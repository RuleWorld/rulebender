...
import rulebender.editors.bngl.BNGLDoubleClickStrategy;
import rulebender.editors.bngl.IBNGLColorConstants;
import rulebender.editors.bngl.NonRuleBasedDamagerRepairer;

public class BNGAConfiguration extends SourceViewerConfiguration 
{
	...
	private BNGAScanner m_bngaScanner;
	private BNGAPartitionScanner m_bnglPartitionScanner;
	
	/**
	 * Create the new Configuration object with a reference to the color manager. 
	 * @param colorManager
	 */
	public BNGAConfiguration(BNGLColorManager colorManager)
	{
	...
	{	
		return new String[] {
			...
			//	BNGAPartitionScanner.BNGA_SPECIES_BLOCK,
			//	BNGAPartitionScanner.BNGA_REACTION_RULES_BLOCK
				};
	}
	 ...
	 * @return
	 */
	public BNGAScanner getBNGAScanner()
	{
		if (m_bngaScanner == null) {
			m_bngaScanner = new BNGAScanner(m_colorManager);
			m_bngaScanner.setDefaultReturnToken(
				new Token(
					new TextAttribute(
						m_colorManager.getColor(IBNGLColorConstants.DEFAULT))));
		}
		
		return m_bngaScanner;
	}

	public BNGAPartitionScanner getBNGAPartitionScanner()
	{
		if (m_bnglPartitionScanner == null) {
			m_bnglPartitionScanner = new BNGAPartitionScanner();
	...
	public IPresentationReconciler getPresentationReconciler(ISourceViewer sourceViewer) 
	{
		...
		//default content
		dr = new DefaultDamagerRepairer(getBNGAScanner());
		...
		NonRuleBasedDamagerRepairer ndr =
			...
			m_reconciler.setDamager(ndr, BNGAPartitionScanner.BNGA_COMMENT);
			m_reconciler.setRepairer(ndr, BNGAPartitionScanner.BNGA_COMMENT);
