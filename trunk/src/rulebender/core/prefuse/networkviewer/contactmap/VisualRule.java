...
public class VisualRule 
{
	...
	{	
		...
		{
			if(!context.containsItem(vis.getVisualItem(group,n)) && productMolecules.contains(n))
			{
				context.addItem(vis.getVisualItem(group, n));
		...
		for(Node n : reactantMolecules)
		{
			if(!context.containsItem(vis.getVisualItem(group,n)) && !center.containsItem(vis.getVisualItem(group,n)))
			{
