...
sub simulate
{
    ...
    # At this point, the simulator seems to be ok.
    #  Go ahead and print out final netfile (if there are new reactons or species)
...
sub generate_hybrid_model
{
    ...
    print $FH $hybrid_model->writeBNGL( {NETfile=>$options->{NETfile}} );
    # writing actions!
    if ( @{$options->{actions}} )
    {
        my $action_string = "\n\n###  model actions  ###\n\n";
