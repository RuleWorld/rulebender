...
struct RxnRule => {
	...
	Rmatches => '@'
	,    # Array of Maps onto SpeciesGraphs selected by Reactants SpeciesGraphs

	Rinclude =>
	  '@',    # Array of patterns to use as context for selecting reactants
	Pinclude =>
	  '@',    # Array of patterns to use as context for selecting products
	...
	RateLaw =>
	  'RateLaw',    # Array of rate laws, 1 if unidirectional, 2 if reversible

	Priority  => '$',
	MultScale => '$'
	,    # Factor by which to scale multiplicity to account for symmetry in rule
	...
	# operation modifiers
	DeleteMolecules => '$'
	, # If True, deleting a molecule in a species will remove only that molecule.
	  # Default is unset, and behavior of deletion is determined automatically.
	MoveConnected => '$'
	,    # If True, transporting a molecule in a species will move all molecules
	 # connected to the first molecule by a path contained in the same compartment.

	ReactionCenter => '@'
	, # array of reaction centers for each reactant pattern in RxnRule  --justin

	RxnLabels =>
	  '%',    # track canonical reaction labels to eliminate reduncant reactions
};
...
sub newRxnRule {
	...
	if ( $string =~ s/CreateSpecies\s*\(\s*//i ) {
		...
		# Read RateLaw
		my ( $rl, $err ) = RateLaw::newRateLaw( \$string, $plist );
		if ($err) { return ( [], $err ); }
	...
	# READ REACTION IN STANDARD SYNTAX
		...
		while ($string) {
			...
			# Checking labels for uniqueness
			my $imol = -1;
			for my $mol ( @{ $g->Molecules } ) {
				...
				if ( $label ne "" ) {
					if ( $labels{$label} ) {
						return ( [],
"Repeated label $label in reactants of reactant rule"
						);
					}
					$labels{$label} = "M";
				...
				for my $comp ( @{ $mol->Components } ) {
					...
					if ( $label ne "" ) {
						if ( $labels{$label} ) {
							return ( [],
"Repeated label $label in reactants of reaction rule"
							);
						}
						$labels{$label} = "C";
		...
		while ($string) {
			...
			# Checking labels for correspondence and uniqueness
			my $imol = -1;
			for my $mol ( @{ $g->Molecules } ) {
				++$imol;
				my $label = $mol->Label;
				if ( $label ne "" ) {
					if ( $labels{$label} eq "M" ) {
						...
						return ( [],
"Repeated label $label in products of reaction rule $name"
						);
					}
					else {
						return ( [],
"Mis- or un-matched label $label in products of reaction rule $name"
						);
					}
...
"Mis- or un-matched label $label in products of reaction rule $name"
		...
		# read rate constants
		my $rl;    # what's this initialized to?  --justin
		my @rate_laws = ();
		if (%rrefs) { setRefs( \%rrefs, '', $plist ); }
		( $rl, $err ) = RateLaw::newRateLaw( \$string, $plist );
		if ($err) { return ( [], $err ); }
		...
		if ($reversible) {
			...
			if ( $string =~ /^\s*[,]/ ) {
				return ( [],
					"Unidirection reaction may have only one rate law" );
			}
		}

		# Set optional priority of rule
		my $priority = 0;
		if ( $string =~ s/priority\s*=\s*(\S+)// ) { $priority = $1; }

		# Set DeleteMolecules attribute
		if ( $string =~ s/DeleteMolecules// ) { $DeleteMolecules = 1; }
  ...
  # are NOT named in the pattern.
		if ( $string =~ s/MoveConnected// ) { $MoveConnected = 1; }

		# Process exclude_reactants
		my @Rexclude = ( () ) x @reac;
		my @Pexclude = ( () ) x @prod;
...
#  cytosis = 0  otherwise
	for my $compartment_change ( @{ $rr->ChangeCompartment } ) {
...
sub toStringSSC {
	...
	for my $molRef ( @{ $rr->MolDel } ) {
		...
		if ( $i != 0 ) { $pstring .= ";"; }
		if ( defined($m) ) {
			$delMol = $rr->Reactants->[$p]->Molecules->[$m]->toStringSSC();
			$pstring .= "destroy ";
			$delMolCase = 1;
		}
		else {
			$delMol = $rr->Reactants->[$p]->toStringSSC();
...
# also checks if reaction is on same molecule, assign it as a different label
	...
	for my $ea ( @{ $rr->EdgeAdd } ) {
		...
		( my $tempstring, my $checkComp1 ) = $rr->Reactants->[$pat1]->Molecules->[$mol1]->toStringSSC();
		( my $tempstring, my $checkComp2 ) = $rr->Reactants->[$pat2]->Molecules->[$mol2]->toStringSSC();
...
#print"\n product permutation !!!for $my_Comp for $addLabel is $tempPString \n";
	...
	if ( $mol_index <= 2 ) {
		...
		while ( defined( $rr->Reactants->[0]->Molecules->[$count] ) ) {
			foreach $key ( keys(%pattern1) ) {
				if ( $key == $count ) {
					$temp = "Mol" . $pattern1{$key} . ":";
					push( @sameReac1, $temp );
					$rstring .= $temp;
				}
			}
	...
	for my $g ( @{ $rr->Reactants } ) {
		...
		++$i;
		( my $num, my $array ) = $g->findConnected2();
		if ( $num > 1 ) {
			$errmsg = 1;
...
#the above statement see that in case of a dangling edge, each edge is seen twice;It stores the pair as a key and value.
...
#this checks if any edge in second pattern is <= seen_bond, it is a case of edge label, and increase all the edges by seen_bond
		...
		while ( ( $key, $value ) = each(%seen_edge) ) {
			if ( $value == 0 ) {
				push( @dangling, $key );
			}
		}
...
sub toXML {
	...
	# RateLaw
	$ostring .= $rr->RateLaw->toXML( $indent2, $id . "_RateLaw" );

	# References
	# TODO This is a hack and should be in RateLaw.pm instead of here
	if ( $rr->RateLaw->Type eq "Function" ) {
		my ( $param, $err ) = $plist->lookup( $rr->RateLaw->Constants->[0] );
		my $fun = $param->Ref;
		$ostring .= $indent3 . "<ListOfArguments>\n";
		for my $arg ( @{ $fun->Args } ) {
			my $ptr = $rr->RRefs->{$arg};
			my $oid = pointer_to_ID( $id . "_R", $ptr );
			$ostring .= $indent4 . "<Argument";
			$ostring .= " id=\"" . $arg . "\"";
			$ostring .= " type=\"ObjectReference\"";
			$ostring .= " value=\"" . $oid . "\"";
			$ostring .= "/>\n";
		}
		$ostring .= $indent3 . "</ListOfArguments>\n";
		$ostring .= $indent2 . "</RateLaw>\n";
	}

	# Map
	$ostring .= $indent2 . "<Map>\n";
	my $index = 1;
	for my $source ( sort keys %{ $rr->MapF } ) {
...
sub findMap {
	...
	# Aggregate reactant and product graphs
	my $rg = SpeciesGraph::copy( @{ $rr->Reactants } );
	my $pg = SpeciesGraph::copy( @{ $rr->Products } );

	#printf "%s -> %s\n", $rg->toString(), $pg->toString();

# Set up mapping between molecules of aggregate graphs and molecules of patterns
	my @aggMapR = ();
	*patterns = $rr->Reactants;
	for my $ipatt ( 0 .. $#patterns ) {
		*molecules = $patterns[$ipatt]->Molecules;
	...
	*patterns = $rr->Products;
	...
	my @maps = $rg->findMaps2($pg);
	unless (@maps) { return "No valid mapping could be found for this rule."; }
	if ( scalar(@maps) > 1 ) { return "Mapping for this rule is ambiguous."; }

	# DEV NOTE: moved symmetry detection to end of routine

	*moleculesR = $rg->Molecules;
	*moleculesP = $pg->Molecules;
	*map        = $maps[0]->MapF;

	# Use mapping to determine graph rewriting steps
	#printf "Found %d mapping(s)\n", scalar(@maps);
	#  printf $maps[0]->toString();

	# Molecules that are destroyed
	# map comments (jhogg): this can be based soley on mapF
	my @dlist = ();
	my @pused = (0) x @moleculesP;

	*patterns = $rr->Reactants;
	my $i_r = 0;
	for my $p_r ( 0 .. $#patterns ) {
		*molecules = $patterns[$p_r]->Molecules;
		my @mol_delete;
		@mol_delete = ();
		for my $m_r ( 0 .. $#molecules ) {
			my $i_p = $map{$i_r};
			if ( $i_p < 0 ) {
				push @mol_delete, $m_r;
			}
...
#printf "Rule %s: Reactant pattern %s is deleted\n", $rr->Name, $rr->Reactants->[$p_r]->toString();
	...
	# map comments (jhogg): this requires mapR
	my @molAdd = ();
	*mapr = $maps[0]->MapR;
	for my $imP ( 0 .. $#pused ) {
...
# List of pointers to reactant components from which to inherit attributes of each component
		...
		if (
			my $err = $mtlist->checkMolecule(
				$mol, { IsSpecies => 1, InheritList => \@ilist }
			)
		  )
		{
			return ( "Molecule created in reaction rule: " . $err );
		}
		push @molAdd, [ ( $ref, @ilist ) ];
	}
	if (@molAdd) {
		$rr->MolAdd( [@molAdd] );
	}
	...
	*adjP   = $pg->Adjacency;
	...
	for my $edgeR (@edgesR) {
		...
		my $p1P = $map{$p1R};
		my $p2P = $map{$p2R};
		if ( defined( $adjP{$p1P}{$p2P} ) ) {
			$eused[ $adjP{$p1P}{$p2P} ] = 1;
			next;
		}
		...
		#printf "Edge $p1R $p2R is deleted (reactant side)\n";
		push @edgeDel, [ $p1R, $p2R ];
	}
	if (@edgeDel) {
		$rr->EdgeDel( [@edgeDel] );
	}
...
# onto the selected graphs
	@edgeAdd = ();
	for my $iedgeP ( 0 .. $#edgesP ) {
		next unless ( $eused[$iedgeP] == 0 );
		my @ps = split( ' ', $edgesP[$iedgeP] );
		next unless ( $#ps == 1 );
		my $p1P = $ps[0];
		my $p2P = $ps[1];
  ...
  #printf "Rule %s: Edge %s %s is added (product side)\n", $rr->Name,$p1P, $p2P;
	...
	# Component state changes
	my @compStateChange = ();
	for my $imR ( 0 .. $#moleculesR ) {
		my $imP = $map{$imR};

		#print "$imR->$map{$imR}\n";
		next unless ( $imP >= 0 );
		*componentsR = $moleculesR[$imR]->Components;
		*componentsP = $moleculesP[$imP]->Components;
		for my $icR ( 0 .. $#componentsR ) {
			next unless ( defined( $componentsR[$icR]->State ) );
			my ( $stateR, $stateP );
			$stateR = $componentsR[$icR]->State;
			my ( $imP, $icP ) = split( '\.', $map{"$imR.$icR"} );
			if ( !defined( $componentsP[$icP]->State ) ) {
				exit_error(
"Component with state attribute defined in reactant pattern cannot map"
					  . " to component with undefined state attribute in product pattern",
					$rr->toString()
				);
			}
			else {
				$stateP = $componentsP[$icP]->State;
			}

			if ( $stateR cmp $stateP ) {
				my $pR = "$aggMapR[$imR].$icR";

	#printf "Component %s changed state from %s to %s\n", $pR, $stateR, $stateP;
				push @compStateChange, [ ( $pR, $stateR, $stateP ) ];
			}
		}
	}
	if (@compStateChange) {
		$rr->CompStateChange( [@compStateChange] );
	}
...
#
# IN PROGRESS
# TO DO:  In readString, add check for species compartment validity.
#         Add bypasses when compartments block is undefined.
#
...
#                 0  otherwise (volume-to-volume species transport and all molecule transport).
	...
	# array which holds compartment change instructions
	my $changeCompartment = [];
...
#  (iii) construct the final map $mapPattR by comparing the forward and reverse maps.
	...
	##
	##  Blah!  parsing problems with compartments in rxn rule

	# First Rewrite forward map entries into form:
	# mapf{pR.mR.cR}= "pP.mP.cP";
	my %mapf_new = ();
	for my $im1 ( 0 .. $#moleculesR ) {
		my $im2 = $map{$im1};
		my $pm1 = $aggMapR[$im1];
		...
		*components = $moleculesR[$im1]->Components;
		for my $ic1 ( 0 .. $#components ) {
			my ( $im2, $ic2 ) = split( '\.', $map{"$im1.$ic1"} );
			if ( $im2 >= 0 ) {
	...
	for my $im1 ( 0 .. $#moleculesP ) {
		...
		#print "map_new: $pm1 -> $pm2\n";
		*components = $moleculesP[$im1]->Components;
		for my $ic1 ( 0 .. $#components ) {
			my ( $im2, $ic2 ) = split( '\.', $mapr{"$im1.$ic1"} );
			if ( $im2 >= 0 ) {
	...
	{
		my @mapPattR = ()
		  ; # map from reactant patterns to product patterns (-1 if deleted, -2 if no consistent map)
		my @mapPattP = ()
		  ; # map from product patterns to reactant patterns (-1 if added, -2 if no consistent map)
...
# first find consistent mappings from each Reactant pattern into a Product pattern (or null pattern).
		...
		for ( my $i_pattR = 0 ; $i_pattR < @{ $rr->Reactants } ; $i_pattR++ ) {
			for (
				my $i_molR = 0 ;
				$i_molR < @{ $rr->Reactants->[$i_pattR]->Molecules } ;
				$i_molR++
			  )
			{
...
# now find consistent mappings from a Product pattern into a Reactant pattern (or null pattern)
		...
		for ( my $i_pattP = 0 ; $i_pattP < @{ $rr->Products } ; $i_pattP++ ) {
			for (
				my $i_molP = 0 ;
				$i_molP < @{ $rr->Products->[$i_pattP]->Molecules } ;
				$i_molP++
			  )
			{
...
#  Pattern mapping is okay if:  $mapPattR[ $mapPattP[i] ] = i  OR  $mapPattP[i] == -1.
 ...
 #  => can't change a species' location if the molecule composition has changed.
   ...
   # }
			...
			# if compartments are not defined, do nothing
			next if ( !defined($compR) and !defined($compP) );

		  # if reactant compartment defined, but not product, then no Transport.
		  # Assume that species compartment is just matching context.
			next if ( defined($compR) and !defined($compP) );
   ...
   # NOTE: this could potentially define a generic transport.
	 ...
	 # }
			...
			unless ( $compR->SpatialDimensions == $compP->SpatialDimensions ) {
				exit_error(
"Reaction Rule attempts to transfer a species between compartments"
			...
			if ( $compR->SpatialDimensions == 3 ) {
				push @{$changeCompartment}, [ $i_pattR, $compR, $compP, 0 ];
			}

			# case 3C: surface to surface
			if ( $compR->SpatialDimensions == 2 ) {

				# only allowed between surfaced connected by volume
				# this is a mimick of endo/exo-cytosis
				my $cytosis = $compR->separated_by_volume($compP);
				unless ($cytosis) {
					exit_error(
"Reaction Rule attempts to transport a membrane species to a membrane"
						  . " which is not connected to the source compartment by a volume.",
						$rr->toString()
					);
				}
				push @{$changeCompartment},
				  [ $i_pattR, $compR, $compP, $cytosis ];
			}
		}
	...
	for ( my $i_molR = 0 ; $i_molR < @{ $rg->Molecules } ; $i_molR++ ) {
		...
		if ( defined($pattR_comp) and defined($molR_comp) ) {
			...
			unless ( $pattR_comp->adjacent($molR_comp) ) {
				exit_error(
"Reaction Rule molecule and pattern compartments are not compatible.",
					$rr->toString()
				);
			}
		}
		...
		my $pattP_comp = $rr->Reactants->[$i_pattP]->Compartment;
...
"Reaction Rule molecule and pattern compartments are not compatible.",
	 ...
	 # (2) error if compartment specified for reactant or product, but not both.
		if ( defined($molR_comp) xor defined($molP_comp) ) {
			exit_error(
				"Reaction Rule specifies compartment for either the reactant or"
			...
			or $molR_comp->separated_by_surface($molP_comp) )
		{
			...
			exit_error(
"Reaction rule moves reactant molecule to a compartment that is not"
	...
	# add compartment changes to rule action list.
	if (@$changeCompartment) {
		$rr->ChangeCompartment($changeCompartment);
	}

# debug: show compartment change operations
# foreach my $op (@$changeCompartment)
# {
#   print STDERR "\n", $rr->toString(), "\n";
...
# TODO: filter reactant automorphisms that permute a pattern flagged by MatchOnce
# (set up a map from the pattern set to aggregated set.)
	...
	#
	# See mapAuto for more notes.
	#my @RuleGroup = ();
	#foreach my $auto ( @r_auto )
	#{
	#   my ($permP, $err) = $auto->get_induced_permutation( $maps[0] );
...
#{
#   # loop over reaction center elements (one array for each reactant pattern)
#   foreach my $Rpatt (@{$rr->ReactionCenter})
#   {
#      CENTER_LOOP:
#      foreach my $center_element (@$Rpatt)
#      {
#         # skip pattern-only elements
#         next CENTER_LOOP unless ($center_element =~ /\./);
#
...
# print $rr->toString(), "\n";
# print join( ' ', 'rg:', scalar(@RuleGroup), 'stab:', scalar(@StabRxnCntr), 'r:', scalar(@r_auto), 'p:', scalar(@p_auto), "\n" );
...
##-----------------------##
{
	...
	my $max_stoich = "";
	my $verbose;
	my ( $rr, @sgs, @matches, @isnew, $new_level, @targets, @reactants );
	my @offsets = (0);
	my $n_new_rxns;
	my ( %mapP,  @rxns );
...
# store matches in @rmatches, where $rmatches[i] is an array of matches to pattern i.
		if ( $pattDepth == -1 ) {
			...
			if (@_) {    # User-provided array
				*species = shift;
			}
			else {       # Array in species list
				*species = $slist->Array;
			}
			...
			for my $patt ( @{ $rr->Reactants } ) {
				...
				for my $match ( $patt->isomorphicToSubgraph(@sgs) ) {
					...
					if ( $targ != $targ_last ) {
						...
						for my $patt_excl ( @{ $rr->Rexclude->[$ipatt] } ) {
	  ...
	  MATCH:
		for my $match ( @{ $rmatches[$pattDepth] } ) {
			...
			# Set mapping of product pattern onto supergraph

            #print "Mapping ", $match->Source->toString(), " onto ", $match->Target->toString(),"\n";
            #for my $m ( 0 .. $#molecules )
			foreach my $im ( 0 .. $#{ $match->Source->Molecules } ) {

				my $pP;
				if ( defined $rr->MapF->{"$pattDepth.$im"} ) {
					$pP = $rr->MapF->{"$pattDepth.$im"};
					next if ( $pP == -1 );
				}
				else
				{   # skip this molecule pattern if it doesn't map into products
					next;
				}
				$mapP{$pP} = $match->mapF( "$im", $offset );

				#print "$pP -> $mapP{$pP}\n";

				foreach my $ic (
					0 .. $#{ $match->Source->Molecules->[$im]->Components } )
				{

					# Get pointer to corresponding component of product pattern
					my $pP = $rr->MapF->{"$pattDepth.$im.$ic"};

					# Set map of product pointer with pointer in Target
					$mapP{$pP} = $match->mapF( "$im.$ic", $offset );

					#print "$pattDepth.$m.$c -> $pP -> $mapP{$pP}\n"
				}
			}
			...
			if ( $pattDepth < $maxDepth ) {
				++$pattDepth;
				$offsets[$pattDepth] =
				  $offset + scalar( @{ $match->Target->Molecules } );
				&applyRule;
			...
			my $using_compartments =
			  defined( $targets[0]->Compartment ) ? 1 : 0;
		   ...
		   # create permutations of identical reactants and process each variant
			my $reactant_permutations =
			  $rr->permute_identical_reactants( \@targets );
			...
			{
			    my ( $g_perm, $rxn_label, $del_list_perm ) =
			        $rr->apply_operations( \@targets, \@offsets, \@matches, \%mapP, $permute );
			    
			    unless ( defined $g )
			    {
			        $g = $g_perm; 
			        @$del_list = @$del_list_perm;       
			    }
			...
			{    # redundant reaction!
				    #print STDERR ">>> Redundant reaction encountered!\n";
				next MATCH;
			}


			# Split aggregate graph into connected subgraphs
			#print STDERR "HERE : split aggregate graph\n";
			my ( $n_subgraphs, $subgraphs ) = $g->findConnected2();

            # Check for correct number of subgraphs
            # NOTE: Jim's updated logic for handling DeleteMolecules and Molecularity Mismatch.
            # This version is equivalent to 2.0.46, which was more or less correct, but is exploitable.
            # In particular, if a rule with the DeleteMolecules keyword also manipulates bonds, it is
            # possible to violate molecularity outside of a deletion operation.
            #print STDERR "HERE : check for correct number of graphs\n";
            #print STDERR "$n_subgraphs : $subgraphs\n";
			my $nprod_patterns = scalar @{ $rr->Products };
			if ( $n_subgraphs != $nprod_patterns )
			{
                # If Molecules are being deleted, it is allowed to have more subgraphs than product patterns
				if ( $rr->DeleteMolecules
					and ( $n_subgraphs > $nprod_patterns ) )
				{
					$verbose
					  and printf "Deleting molecules: %s\n", $g->toString();
				}

				# Otherwise, the reaction shouldn't happen
				else {
					$verbose
					  and printf "Rule %s: n_sub ($n_subgraphs)!= n_prod ($nprod_patterns): %s\n",
					  $rr->Name, $g->toString();
					next MATCH;
				}
			}
			...
			# map of target molecules in supergraph back onto product patterns
			# useful for checking below
			
			#print STDERR "HERE : check for redundant reactions\n";
			my @mapP_reverse = (0) x @$subgraphs;
			for my $p ( keys %mapP ) {
				my $targ = $mapP{$p};
				next if ( $targ =~ /\./ );
				$mapP_reverse[$targ] = $p;
			...
			# Correct $mapP_reverse for deleted molecules
			#print STDERR "HERE : correct mapP for deleted molecules\n";
			for my $mi (@$del_list) {
				$mapP_reverse[$mi] = "d";
			}
			my $it = 0;
			while ( $it <= $#mapP_reverse ) {
				if ( $mapP_reverse[$it] eq "d" ) {
					splice @mapP_reverse, $it, 1;
					next;
				}
				++$it;
			}
			...
			#for my $it (0..$#mapP_reverse){
			#	printf "it=$it p=%s\n", $mapP_reverse[$it];
			#}

			# Loop over product graphs
			#print STDERR "HERE : loop over product graphs\n";
			my @products = ();
			my $new_prod = 0;
			my $iprod;
		  PRODUCT:
			for my $isub ( 1 .. $n_subgraphs )
			{
                #print STDERR "\nHERE : loop over products :i:$isub\n\n";
                # To what product pattern does this subgraph correspond?
                # Check that molecules from only a single product pattern map onto
                # this subgraph, i.e. that molecules of graph partition correctly into products
				my ( $ip, $ip_sav, $im );
				$ip_sav = -1;
				$ip     = -1;
				for my $imol ( 0 .. $#{$subgraphs} )
				{
					next unless ( $subgraphs->[$imol] == $isub );
					my $p = $mapP_reverse[$imol];

					#print "  p=$p\n";
					next unless ( $p ne "0" );
					( $ip, $im ) = split( '\.', $p );

                    #printf "  p=$p isub=$isub imol=$imol ip=$ip ip_sav=$ip_sav pP=$p %s\n", $mapP{$p};
					if ( $ip_sav == -1 ) {
						$ip_sav = $ip;

						# Skip this product if it has Fixed attribute
						# This avoids change of concentration
						if ( $rr->Products->[$ip]->Fixed ) {
                            #printf "isub=$isub imol=$imol ip=$ip ip_sav=$ip_sav pP=$p %s nsub=%d\n", $mapP{$p},$n_subgraphs;
							next PRODUCT;
						}
					}
					elsif ( $ip != $ip_sav ) {
                        # Put informative warning message here
                        #printf "isub=$isub imol=$imol ip=$ip ip_sav=$ip_sav pP=$p %s imax_mol=%d\n", $mapP{$p},$#{$subgraphs};
                        #print $g->toString(),"\n";
						next MATCH;
					}				
				}
				...
				# This might be unnecessary and inefficient
				my $p =
				  ( $n_subgraphs > 1 )
				  ? $g->copySubgraph( $subgraphs, $isub )
				  : $g;
				...
				foreach my $binding_set ( values %edges ) {
					...
					if ( @$binding_set > 2 ) {
						print
						  "WARNING: more than two molecules bound to edge.\n"
						  . ">>", $rr->toString(), "\n";
						next MATCH;
					}
					...
					{    # Both compartments are defined,
						...
						unless ( ( $comp0 == $comp1 )
							or $comp0->adjacent($comp1) )
						{
							print
                              "WARNING: rule application generated a species with edges between"
				...
				foreach my $mol ( @{ $p->Molecules } ) {
					...
					# neighbors are in some disallowed configuration
						...
						print
                          "WARNING: rule application created a species containing invalid "
				...
				if ( $iprod >= 0
				  ) # note sure if iprod check is needed, but it's used below, so I'll follow.
				{
					if ( defined $rr->Products->[$iprod]->Compartment ) {
						next MATCH
						  unless ( $infer_comp ==
							$rr->Products->[$iprod]->Compartment );
					}
				}
				...
				if ( my $err = $p->sortLabel() ) {    # mysterious problem
				...
				if ( $iprod >= 0 ) {
					...
					my $include = 1;
					for my $patt_incl ( @{ $rr->Pinclude->[$iprod] } ) {
				...
				if ( $iprod >= 0 ) {
					if ( defined( $products[$iprod] ) ) {
						push @products, $products[$iprod];
					}
					$products[$iprod] = $spec;
				}
				else { push @products, $spec; }

				#for my $spec (@products){
				#  print " ",$spec->SpeciesGraph->toString();
				#}
				#print "\n";
			}
			...
			my $ri = 0;
			for my $rpatt ( @{ $rr->Reactants } ) {
				if ( $rpatt->Fixed ) {

					#print "Fixed reactant ",$rpatt->toString(),"\n";
					push @products, $reactants[$ri];
				}
				++$ri;
			}

            #print STDERR "\nnreact: ", scalar @reactants, "  |  nprod: ", scalar @products, "\n";
			...
			my $reactant_symmetry = 1;
			{
				...
				{
					$reactant_symmetry /=
					  factorial( $reactant_hash{$reactant} );
				}
			}
		  ...
		  REACTANT_PERMUTATION:
			foreach my $rxn_label (@$rxn_labels) {
				...
				$rxn->RateLaw( $rr->RateLaw );
				$rxn->Priority( $rr->Priority );
				$rxn->RxnRule($rr);

				# deprecated under canonical reaction labeling --justin
				#$rxn->StatFactor( $rr->MultScale );

				$rxn->StatFactor($reactant_symmetry);
				$rxn->Compartments($compartments);

				#print $rxn->toString( $BNGModel::PARAM_LIST ), "\n\n";

				# NOTE: volume-dependent scaling moved to Rxn class --Justin
				# Ultimately this should end up in RateLaw, but ehh...

				# Add reaction to ReactionList
				if ($dlist) { $dlist->add( $rxn, 1 ); }
				else
				{
					$n_new_rxns += $rlist->add($rxn);
					if ( $n_new_rxns == 0 ) {
						printf "Reaction %s was not added\n", $rxn->toString();
					}
				}
...
sub unsetRefs {
	...
	for my $arg ( keys %{$refs} ) { $plist->deleteLocal($arg); }
	return '';
}
...
# ($g, $label, $del_list) = $rr->apply_operations(\@targets,\@offsets,\@matches,\%mapP,\@permute)
#
# Apply rule operations to the reactant set in @targets and return
# an aggregated product graph $g. Also generates a canonical labeling for the reaction
#
# @targets is an array with elements that point to the nth reactant
# @offsets is an array with elements that point to the index of the first molecule in the nth pattern
# @matches is an array with elements that are maps from the nth pattern to the nth reactant
# %mapP is a a hash from pointers in the product pattern to pointers in the $g supergraph
# @permute is an array that defines a permutation of the reactants
{

	#print STDERR "enter apply ops :";
	my ( $rr, $targets, $offsets, $matches, $mapP, $permute ) = @_;

	# Aggregate matching SpeciesGraphs
	my $g = SpeciesGraph::copy(@$targets);
	*mol = $g->Molecules;
	...
	# 13 feb 2009
	{
		...
		for my $comp_change ( @{ $rr->ChangeCompartment } ) {
		 ...
		 # debug:
		 #print STDERR "P: $i_patt  M: $i_mol  C: $i_comp  cytosis: $cytosis\n";
	  ...
	  # to instances in a species graph
			if ( $ic ne '' ) {    # component compartment change
				...
				my ( $im_targ, $ic_targ ) = split '.',
				  $matches->[$i_patt]->mapF( "$im.$ic", $offsets->[$ip] );
				$mol[$im_targ]->Components->[$ic_targ]->Compartment($compartP);

				# save compartment change for canonical ordering
				push @$stack,
				    $canonical_reactant_order->[$ip] . '.'
				  . $matches->[$i_patt]->mapF("$im.$ic") . ','
				  . $compartP->Name;

			}
			elsif ( $im ne '' ) {    # molecule compartment change
				if ( $rr->MoveConnected )
		   ...
		   # (2) exclude molecules named in reactant pattern (aside from $i_mol)
					...
					my @compartment_connected =
					  $matches->[$ip]
					  ->Target->find_compartment_connected($im_targ);
			...
			else {        # species compartment change
				          #print STDERR ">> Species Transport <<\n";

				# save compartment change for canonical ordering
				push @$stack,
				  $canonical_reactant_order->[$ip] . ',' . $compartP->Name;

				if ( $cytosis == 0 ) {    # volume to volume transport
					                      # transport all molecules to $compartP
					for (
						my $im = $offsets->[$ip] ;
						$im < $offsets->[$ip] +
						@{ $matches->[$ip]->Target->Molecules } ;
						$im++
					  )
					{
				...
				else {                    # surface to surface transport
					...
					my $insideR  = $compartR->Inside->[0]
					  ;    # should be only one compartment here
					  ...
					  )
					{
						...
						elsif ( $g->Molecules->[$im]->Compartment == $insideR )
						{
							...
							exit_error(
"surface to surface species transport attempted to move molecule"
		...
		if (@$stack) {
			@$stack = ( sort { $a cmp $b } @$stack );
			$label .= '||CompartmentChange//' . join( '//', @$stack );
		}
	}
...
# Find pointer to molecule and component in $g corresponding to $im.$ic in pattern
	...
	# Apply molecule additions
	{
		...
		foreach my $molAddRef ( @{ $rr->MolAdd } ) {
			...
			$newMol->Label(undef);
			my $nmol = $#{ $g->Molecules } + 1;
			$mapP->{$molRef} = $nmol;
			$ic_newMol = 0;
			for my $comp ( @{ $newMol->Components } ) {
				$comp->Label(undef);
...
#    use the pointers into the supergraph)
			...
			foreach my $map (@$matches) {
				foreach my $ptr ( keys %{ $map->MapF } ) {
					if ( $map->mapF( $ptr, $offsets->[$ip1] ) eq $g_ref1 ) {
						...
						$ref1 =
						  $canonical_reactant_order->[ defined $permute
						  ? $permute->[$ip1]
						  : $ip1 ]
						  . '.'
						  . $map->mapF($ptr);
    ...
    my $del_list = [];   # list of molecules to delete (by index) in the aggregate product
	{
		...
		# compress product graph
		my $im = 0;
		while ( $im < scalar @{ $g->Molecules } ) {
	...
	# Update aggregate graph (and remove dangling edges)
	$g->updateEdges(0);

	# return product graph
	#print STDERR ": exit apply ops\n";
	return ( $g, $label, $del_list );
}
  ...
  # note, edge pointers must be sorted prior to comparison.
{
	...
	# TODO: handle case of different number of nodes per "edge"

	my $cmp;
	while ( @a or @b ) {
		$cmp = cmp_pointer( shift @a, shift @b );
		return $cmp if $cmp;
	}
