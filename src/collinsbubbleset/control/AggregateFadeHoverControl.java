package collinsbubbleset.control;
...
public class AggregateFadeHoverControl extends ControlAdapter {
	...
	public AggregateFadeHoverControl(String aggregateGroup, String nodeGroup,
			String fadeGroup, String action) {
		super();
	...
	public void itemEntered(VisualItem item, MouseEvent evt) {
		if (item.isInGroup(NODES)) {
			AggregateTable at = (AggregateTable) item.getVisualization()
					.getGroup(AGGREGATES);
			Iterator iterator = at.getAggregates(item);
			AggregateItem aggr;

			// cycle through aggregates containing this node, when a hover
			// aggregate is found, set it
			...
			while (iterator.hasNext()) {
				...
				if (!aggr.isHover()) {
					...
					while (aggregateItems.hasNext()) {
						VisualItem aggregatedItem = (VisualItem) aggregateItems
								.next();
						// if item is not also in a hover aggregate, fade it
						Iterator itemAggregates = at
								.getAggregates(aggregatedItem);
						boolean hover = false;
						while (itemAggregates.hasNext()) {
							if (((AggregateItem) itemAggregates.next())
									.isHover())
								hover = true;
						}
						if (!hover)
							aggr.getVisualization().getGroup(FADE)
									.addTuple(aggregatedItem);
					}
				} else {
					Iterator aggregateItems = aggr.items();
					while (aggregateItems.hasNext()) {
		...
		if (item.isInGroup(NODES)) {
			...
			while (iterator.hasNext()) {
				...
				if (aggr.containsItem(item)) {
					...
					item.getVisualization().getFocusGroup(FADE)
							.removeTuple(aggr);
					Iterator aggregateItems = aggr.items();
					while (aggregateItems.hasNext()) {
						VisualItem aggregatedItem = (VisualItem) aggregateItems
