...
/**
 * 
 * Christopher Collins Modified copy of LabelRenderer v 2.2
 * 
 ...
 */
public class BubbleSetLabelRenderer extends AbstractShapeRenderer
{
  ...
  public static final Schema SCHEMA = new Schema(new String[] { ROTATION },
      new Class[] { double.class });

  /**
   * Whether rotations should be restricted to -90 < theta < 90 or not (i.e.
   * keep text upright). True by default.
   */
  ...
  // -1 is the standard invalid row number that item.getRow() returns and we
  // never want to match that
   ...
   */
  public BubbleSetLabelRenderer(String textField)
  {
   ...
   *          field should be a URL, a file within the current classpath, a file
   *          on the filesystem, or null for no image. If the
   *          <code>imageField</code> parameter is null, no images at all will
   *          be drawn.
   */
  public BubbleSetLabelRenderer(String textField, String imageField)
  {
   ...
   * should be a URL, a file within the current classpath, a file on the
   * filesystem, or null for no image. If the <code>imageField</code> parameter
   * is null, no images at all will be drawn.
   * 
   * @param imageField
   ...
   */
  protected static void getAlignedPoint(Point2D p, VisualItem item, double w,
      double h, int xAlign, int yAlign)
  {
  ...
  public void render(Graphics2D g, VisualItem item)
  {
    ...
    if (text != null && ColorLib.alpha(textColor) > 0)
    {
      ...
      for (; end >= 0; y += lh)
      {
        drawString(g, fm, text.substring(start, end), useInt, x, y, tw, item
  ...
  /**
   * Return the graphics space transform applied to this item's shape, based on
   * rotation.
   * 
  ...
  protected AffineTransform getTransform(VisualItem item)
  {
    ...
    if (forceUpright)
    {
      if (((itemRotation > 90) && (itemRotation < 270))
          || ((itemRotation < -90) && (itemRotation > -270)))
        itemRotation += 180;
    }

    itemTransform.rotate(Math.toRadians(itemRotation),
        itemRawShape.getCenterX(), itemRawShape.getCenterY());
  ...
  private final void drawString(Graphics2D g, FontMetrics fm, String text,
      boolean useInt, double x, double y, double w, double cx, double cy,
      double rotation)
  {
  ...
  /**
   * Get the image position, determining where the image is placed with respect
   * to the text. One of {@link Constants#LEFT}, {@link Constants#RIGHT},
  ...
  /**
   * Set the image position, determining where the image is placed with respect
   * to the text. One of {@link Constants#LEFT}, {@link Constants#RIGHT},
   * {@link Constants#TOP}, or {@link Constants#BOTTOM}. The default is left.
   * 
   * @param pos
  ...
  /**
   * Returns the amount of padding in pixels between the content and the border
