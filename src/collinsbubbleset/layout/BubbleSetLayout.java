...
public class BubbleSetLayout extends Layout {
	...
	static {
		Logger.getLogger(BubbleSetLayout.class.getName()).setLevel(
				Level.WARNING);
	}
	...
	/**
	 * whether to have surface follow visible edges contained in aggregate, or
	 * use nearest neighbour connections
	 */
	 ...
	 * 
	 * Speedups: energy only calculated for area of possible influence of items
	 * in aggregate; only aggregates within area of influence of hover items are
	 ...
	 * 
	 * Note that only recalculating hulls affected by the hover item, if there
	 * is one, may cause problems if items are moving but not in the region of
	 * the hover item when there is a hovered item. When there is no hovered
	 * item all hulls are recalculated.
	 * 
	 * @param aggrGroup
	 *            the set of aggregates
	 * @param sourceGroup
	 *            the set of visual items which exert an influence on the
	 *            aggregages (may or may not be members)
	 ...
	 * @param doMorph
	 *            whether to perform edge routing around intersections (ignored
	 *            if using structural edges)
	 ...
	 * @param centroidSort
	 *            if true sort and connect items from centroid out, otherwise
	...
	public BubbleSetLayout(String aggrGroup, String sourceGroup,
		...
		visibleNodePredicate = new AndPredicate(
				new InGroupPredicate(nodeGroup), VisiblePredicate.TRUE);
	...
	public void setAggregateLabelLayout(
			BubbleSetLabelLayout aggregateLabelLayout) {
	...
	public void run(double frac) {
		...
		if (aggregateLabelLayout != null) {
			...
			TupleSet aggregateLabels = m_vis.getGroup(aggregateLabelLayout
					.getGroup());
			Iterator<?> labelIterator = aggregateLabels
					.tuples(VisiblePredicate.TRUE);
	...
	private void run(DecoratorItem di, AggregateItem aitem) {
		...
		bounds.setRect(bounds.getX() - Math.max(edgeR1, nodeR1) - morphBuffer,
				...
				bounds.getHeight() + 2 * Math.max(edgeR1, nodeR1) + 2
						* morphBuffer);
		double[][] potentialArea = new double[(int) (Math.ceil(bounds
		 ...
		 * (float) bounds.getY(); fhull[2] = fhull[0] +
		 * (float)bounds.getWidth(); fhull[3] = fhull[1]; fhull[4] = fhull[2];
		 * fhull[5] = fhull[1] + (float)bounds.getHeight(); fhull[6] = fhull[0];
		 * fhull[7] = fhull[5]; fhull[8] = Float.NaN; //
		 */
		...
		/*
		 * System.out.print("hull: ["); for(int i = 0 ; i < fhull.length-1;
		 * i+=2) System.out.print(fhull[i] + ", " + fhull[i+1]);
		 * System.out.println("]");
		 */
	}
		...
		if (nodeInfluenceFactor != 0) {
			...
			while (aNodeItems.hasNext()) {
				...
				if (item instanceof EdgeItem) {
					...
					Line2D line = new Line2D.Double(((EdgeItem) item)
							.getSourceItem().getX(), ((EdgeItem) item)
							.getSourceItem().getY(), ((EdgeItem) item)
							.getTargetItem().getX(), ((EdgeItem) item)
							.getTargetItem().getY());
					...
					calculateLinesInfluence(potentialArea, a * influenceFactor,
							edgeR1, singleLineArrayList, activeArea);
				} else {

					calculateRectangleInfluence(potentialArea, a
							* influenceFactor, nodeR1, new Rectangle2D.Double(
							item.getBounds().getX() - activeArea.getX(), item
									.getBounds().getY() - activeArea.getY(),
							item.getBounds().getWidth(), item.getBounds()
									.getHeight()));
				}
	 ...
	 * 
	 * 
	 * NodeItems and EdgeItems are both treated equally here, but EdgeItems do
	 * not have their X and Y fields set. I will add the appropriate calls to
	 * set them before they are used. ams292 Aug 4, 2010.
	 * 
	 * @param aitem
	 *            the set to search
	 * @param item
	...
	private ArrayList<Line2D> connectItem(AggregateItem aitem, VisualItem item,
		...
		while (neighbourIterator.hasNext()) {
			...
			if (considerBlockingItems) {
				...
				Line2D completeLine = new Line2D.Double(itemX, itemY, nItemX,
						nItemY);

				Iterator<?> interferenceItems = m_vis.getVisualGroup(
		...
		if ((closestNeighbour != null) && (edgeInfluenceFactor != 0)) {
			...
			if (closestNeighbour instanceof EdgeItem)
				...
				completeLine = new Line2D.Double(itemX, itemY,
						closestNeighbour.getX(), closestNeighbour.getY());
			...
			if (doMorph) {
				...
				while (hasIntersection && iterations < MAX_ROUTING_ITERATIONS) {
					...
					while (!hasIntersection && !linesToCheck.isEmpty()) {
						...
						Line2D line = linesToCheck
								.remove(linesToCheck.size() - 1);
						...
						VisualItem closestItem = getCenterItem(
						...
						if (closestItem != null) {
							...
							if (numIntersections == 2) {
								...
								if ((movePoint != null) && (!foundFirst)
									...
									linesToCheck.add(new Line2D.Double(line
											.getP1(), movePoint));
									linesToCheck.add(new Line2D.Double(
				...
				while (!linesToCheck.isEmpty()) {
					...
					scannedLines
							.add(linesToCheck.remove(linesToCheck.size() - 1));
				}
				...
				if (doMerge) {
					while (!scannedLines.isEmpty()) {
						...
						Line2D line1 = scannedLines
								.remove(scannedLines.size() - 1);

						if (!scannedLines.isEmpty()) {
							// Line2D line2 = scannedLines.pop();
							Line2D line2 = scannedLines.remove(scannedLines
									.size() - 1);

							Line2D mergeLine = new Line2D.Double(line1.getP1(),
									line2.getP2());

							// move virtual edges around nodes, not other edges
							// (routing around edges would be too difficult)
							Iterator<?> interferenceItems = m_vis
		 ...
		 * System.out.println("\tReturned Line:\t(" + l.getX1() + ", " +
	 ...
	 */
	public void calculatePointInfluence(double[][] potentialArea,
			double factor, double r1, double pointx, double pointy) {
	...
	public void calculateLineInfluence(double[][] potentialArea,
		...
		int endY = Math.min(potentialArea[startX].length, Math.max(0,
		...
		while (interferenceItems.hasNext()) {
			...
			if (!currentAggregate.containsItem(interferenceItem)) {
				...
				if ((interferenceItem.getBounds().getMinX() <= (Math.max(
					...
					double distance = fractionToLineEnd(
		...
		while (interferenceItems.hasNext()) {
			...
			if (!currentAggregate.containsItem(interferenceItem)) {
				...
				if ((interferenceItem.getBounds().getMinX() <= (Math.max(
					...
					double distance = fractionToLineCenter(
							interferenceItem.getBounds(), testLine);
					// find closest intersection
					if ((distance != -1) && (distance < minDistance)) {
						closestItem = interferenceItem;
	...
	public void paintPotential(AggregateItem aitem, DecoratorItem di,
		...
		System.err.println("Aggregate info: threshold: " + threshold
				+ " NNIF: " + negativeNodeInfluenceFactor + " NEIF: "
	...
	public Point2D rerouteLine(Line2D line, Rectangle2D rectangle,
			double rerouteBuffer, Intersection[] intersections,
			boolean wrapNormal) {
		Rectangle2D bounds = rectangle.getBounds2D();
	...
	public BubbleSetLabelLayout createLabelLayout(String group,
			String labelField) {
	...
	public class BubbleSetLabelLayout extends ItemAction {
		...
		public void process(VisualItem visualItem, double frac) {
			...
			while (aNodeItems.hasNext()) {
				...
				// item.get(VIRTUAL_EDGES);
				ArrayList<Line2D> edges = (ArrayList<Line2D>) item
						.get(VIRTUAL_EDGES);
	...
	public static Point2D interpolateLine2D(Line2D line, double frac) {
		double newX = line.getP1().getX() * (1 - frac) + line.getP2().getX()
				* frac;
		double newY = line.getP1().getY() * (1 - frac) + line.getP2().getY()
				* frac;
