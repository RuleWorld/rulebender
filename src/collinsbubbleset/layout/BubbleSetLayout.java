package collinsbubbleset.layout;

import java.awt.geom.Line2D;
//import java.awt.geom.Path2D;
import j15support.java.awt.geom.Path2D;
...
import collinsbubbleset.layout.BubbleSetLayout.Intersection.State;
import collinsbubbleset.lib.MathLib;
...
/**
 * Layout algorithm that computes the isocontour surrounding aggregate items and
...
public class BubbleSetLayout extends Layout
{
  ...
  /**
   * Merge consecutive edges resulting from edge routing if there is no obstacle
   * blocking merge.
   */
  public boolean doMerge = true;

  /**
   * Whether to consider blocking items when discovering nearest neighbours for
   * edge routing.
   */
  public boolean considerBlockingItems = true;

  /** The logger for ConcaveHullLayout. */
  private static final Logger LOGGER = Logger.getLogger(BubbleSetLayout.class
      .getName());

  static
  {
    Logger.getLogger(BubbleSetLayout.class.getName()).setLevel(Level.WARNING);
  }
  ...
  static
  {
    ...
    // memberDependentVisibility true means show aggregate if any members
    // visible; false means always hide aggregates
  ...
  private static final Predicate VISIBLE_EDGE_PREDICATE = ExpressionParser
      .predicate("VISIBLE() AND ISEDGE()");

  /**
   * The number of smoothing iterations to calculate after initializing the
   * surface.
   */
  private int smoothingIterations = 10;

  private Predicate visibleNodePredicate = ExpressionParser
      .predicate("VISIBLE() AND ISNODE()");

  String nodeGroup = "component_graph.nodes";

  // skip every N points in the marching squares when making the contour
  private int skip = 10;
  // look at super-pixel groupings of this size
  private int pixelGroup = 2; // 3
   ...
   * energy is 1
   */
  private double nodeR0 = 25;// 15;
  /**
   * the radius at which potential reaches zero -- the extent of energy
   * contribution by the node
   */
  private double nodeR1 = 50;// 50

  /** the distance to morph energy around a node */
  private double morphBuffer = 50;
  /** whether to perform edge routing around intersections */

  // Changed ams292 4/8/10
  private boolean doMorph = false;

  /** whether to connect neighbours left to right or to centroid */
  // Changed ams292 29/7/10
  private boolean centroidSort = false;

  private double nodeInfluenceFactor = 1;
  private double edgeInfluenceFactor = 1;
  private double negativeNodeInfluenceFactor = -0.4;
  private double negativeEdgeInfluenceFactor = 0;// -0.4;

  /**
   * whether to have surface follow visible edges contained in aggregate, or use
   ...
   */
  private BubbleSetLabelLayout aggregateLabelLayout;


  /**
   * Create a concave hull layout for a given group of aggregates and members.
   * 
   * Speedups: energy only calculated for area of possible influence of items in
   * aggregate; only aggregates within area of influence of hover items are
   * recalcuated if there is a hover items; super-pixel sizes are used to
   * calculate energy; some points on the hull are skipped in the rendering.
   * 
   * Note that only recalculating hulls affected by the hover item, if there is
   * one, may cause problems if items are moving but not in the region of the
   * hover item when there is a hovered item. When there is no hovered item all
   * hulls are recalculated.
   * 
   * @param aggrGroup
   *          the set of aggregates
   * @param sourceGroup
   *          the set of visual items which exert an influence on the aggregages
   *          (may or may not be members)
   * @param surfaceDistance
   *          the minimum distance from the aggregate member's edge to the
   *          surface of the hull
   * @param influenceDistance
   *          the maximum influence distance
   * @param threshold
   *          the threshold for inclusion in the hull
   * @param nodeInfluenceFactor
   *          the positive influence of included nodes
   * @param edgeInfluenceFactor
   *          the positive influence of included edges
   * @param negativeNodeInfluenceFactor
   *          the negative influence of non-included nodes
   * @param negativeEdgeInfluenceFactor
   *          the negative influence of non-included edges
   * @param doMorph
   *          whether to perform edge routing around intersections (ignored if
   *          using structural edges)
   * @param useStructuralEdges
   *          whether to have surface follow visible edges contained in
   *          aggregate, or use nearest neighbour connections
   * @param centroidSort
   *          if true sort and connect items from centroid out, otherwise from
   *          left-right, top-bottom
   */
  public BubbleSetLayout(String aggrGroup, String sourceGroup,
      String nodeGroup, double nodeSurfaceDistance,
      double nodeInfluenceDistance, double edgeSurfaceDistance,
      double edgeInfluenceDistance, double nodeInfluenceFactor,
      double edgeInfluenceFactor, double negativeNodeInfluenceFactor,
      double negativeEdgeInfluenceFactor, boolean doMorph,
      boolean useStructuralEdges, boolean centroidSort)
  {
    super(aggrGroup);
    this.sourceGroup = sourceGroup;
    this.nodeGroup = nodeGroup;
    visibleNodePredicate = new AndPredicate(new InGroupPredicate(nodeGroup),
  ...
  public void setAggregateLabelLayout(BubbleSetLabelLayout aggregateLabelLayout)
  {
  ...
  public void setVisualization(Visualization vis)
  {
    ...
    if (aggregateLabelLayout != null)
      aggregateLabelLayout.setVisualization(vis);
  }
  ...
  public void run(double frac)
  {
    ...
    if (aggregateLabelLayout != null)
    {
      ...
      TupleSet aggregateLabels = m_vis
      ...
      while (labelIterator.hasNext())
      {
        ...
        Iterator<? extends VisualItem> itemIterator = aitem
            .items(VisiblePredicate.TRUE); // would VALIDATED false
        // be better?
  ...
  private void run(DecoratorItem di, AggregateItem aitem)
  {
    ...
    while (aggregateMembersIterator.hasNext())
    {
      ...
      if (item.get(VIRTUAL_EDGES) != null)
      {
        ...
        // Deque<Line2D> virtualEdges = (Deque<Line2D>)
        // item.get(VIRTUAL_EDGES);
        ArrayList<Line2D> virtualEdges = (ArrayList<Line2D>) item
            .get(VIRTUAL_EDGES);

        // System.out.println("item " + item.get(VisualItem.LABEL) +
        // " has virtual edges: ");
        for (Line2D l : virtualEdges)
        {
          // System.out.println("\t(" + l.getX1() + ", " + l.getY1() +
          // ")-->(" + l.getX2() + "," + l.getY2()+")");
          bounds.add(l.getBounds2D());
        }
      }
    ...
    bounds.setRect(bounds.getX() - Math.max(edgeR1, nodeR1) - morphBuffer,
        ...
        bounds.getWidth() + 2 * Math.max(edgeR1, nodeR1) + 2 * morphBuffer,
        bounds.getHeight() + 2 * Math.max(edgeR1, nodeR1) + 2 * morphBuffer);
    double[][] potentialArea = new double[(int) (Math.ceil(bounds.getWidth()
        / pixelGroup))][(int) (Math.ceil(bounds.getHeight() / pixelGroup))];
    ...
    if (surface == null)
    {
      surface = new ArrayList<Point2D>((int) bounds.getWidth() * 2
    ...
    while ((!calculateSurface(surface, bounds, aitem, potentialArea))
        && (iterations < MAX_MARCHING_ITERATIONS))
    {
      surface.clear();
      iterations++;
    ...
    /*
     * fhull = new float[9]; fhull[0] = (float) bounds.getX(); fhull[1] =
     * (float) bounds.getY(); fhull[2] = fhull[0] + (float)bounds.getWidth();
     * fhull[3] = fhull[1]; fhull[4] = fhull[2]; fhull[5] = fhull[1] +
     * (float)bounds.getHeight(); fhull[6] = fhull[0]; fhull[7] = fhull[5];
     * fhull[8] = Float.NaN; //
     */
    ...
    /*
     * System.out.print("hull: ["); for(int i = 0 ; i < fhull.length-1; i+=2)
     * System.out.print(fhull[i] + ", " + fhull[i+1]); System.out.println("]");
     */
  }
   ...
   */
  public boolean calculateSurface(ArrayList<Point2D> surface,
      Rectangle2D bounds, AggregateItem aitem, double[][] potentialArea)
  {
   ...
   *         extra items
   */
  public boolean[] testContainment(ArrayList<Point2D> surface,
      Rectangle2D bounds, AggregateItem aitem)
  {
    ...
    while (items.hasNext())
    {
      ...
      if (aitem.containsItem(item))
      {
        ...
        containsAll = (containsAll)
            && (g.getBounds().contains(item.getBounds().getCenterX(), item
                .getBounds().getCenterY()));
        // check precise bounds if rough passes
        containsAll = (containsAll)
            && (g.contains(item.getBounds().getCenterX(), item.getBounds()
                .getCenterY()));
   ...
   */
  public void fillPotentialArea(Rectangle2D activeArea, AggregateItem aitem,
      double[][] potentialArea)
  {
    ...
    {
      Iterator<? extends VisualItem> aNodeItems = aitem
          .items(visibleNodePredicate);
      while (aNodeItems.hasNext())
      {
        ...
        // AMS - Aug 4, 2010
        // calculateRectangleInfluence enumerates through every point
        // in the potential area and checks to see if it is within the
        // r1 distance and then adds the potential if it is.
        //
        // The rectangle2D object is defines the shape from which
        // the distance of all of the points are measured.
        //
        // It seems like edges should get the line influence so I am
        // changing this.
        if (item instanceof EdgeItem)
        {
          // System.out.println("Using rectangle influence on Edge " +
          // item.get("id"));
          // Deque<Line2D> singleLineDeque = (Deque<Line2D>) new
          // ArrayDeque<Line2D>();
          ArrayList<Line2D> singleLineArrayList = (ArrayList<Line2D>) new ArrayList<Line2D>();

          Line2D line = new Line2D.Double(((EdgeItem) item).getSourceItem()
              .getX(), ((EdgeItem) item).getSourceItem().getY(),
              ((EdgeItem) item).getTargetItem().getX(), ((EdgeItem) item)
                  .getTargetItem().getY());

          // singleLineDeque.add(line);
          singleLineArrayList.add(line);

          calculateLinesInfluence(potentialArea, a * influenceFactor, edgeR1,
              singleLineArrayList, activeArea);
        }
        else
        {
          ...
          calculateRectangleInfluence(
              ...
              a * influenceFactor,
              nodeR1,
              new Rectangle2D.Double(item.getBounds().getX()
                  - activeArea.getX(), item.getBounds().getY()
                  - activeArea.getY(), item.getBounds().getWidth(), item
                  .getBounds().getHeight()));
        }

        // add the influence of all the virtual edges
        // ArrayList<Line2D> scannedLines = (Deque<Line2D>)
        // item.get(VIRTUAL_EDGES);
        ArrayList<Line2D> scannedLines = (ArrayList<Line2D>) item
            .get(VIRTUAL_EDGES);
    ...
    while (aEdgeItems.hasNext())
    {
      ...
      if (edgeInfluenceFactor != 0)
      {
        ...
        calculateLineInfluence(potentialArea, a * influenceFactor, edgeR1,
            new Line2D.Double(edge.getSourceItem().getX() - activeArea.getX(),
                edge.getSourceItem().getY() - activeArea.getY(), edge
                    .getTargetItem().getX() - activeArea.getX(), edge
                    .getTargetItem().getY() - activeArea.getY()));
      }
    }
        ...
        || (negativeEdgeInfluenceFactor != 0))
    {
      ...
      while (sourceItems.hasNext())
      {
        ...
        else
        {
          ...
          {
            if ((item.isInGroup(nodeGroup)) && item instanceof NodeItem
                && (negativeNodeInfluenceFactor != 0))
            {
              // subtract influence
                ...
                && (negativeEdgeInfluenceFactor != 0))
            {
              ...
              calculateLineInfluence(potentialArea, a * influenceFactor,
                  edgeR1, new Line2D.Double(edge.getSourceItem().getX()
                      - activeArea.getX(), edge.getSourceItem().getY()
                      - activeArea.getY(), edge.getTargetItem().getX()
                      - activeArea.getX(), edge.getTargetItem().getY()
                      - activeArea.getY()));
            }
          }
        }
   ...
   *          the energy field to fill in
   */
  public void fillPotentialArea(Rectangle2D activeArea,
      DecoratorItem aggregateDecorator, double[][] potentialArea)
  {
    double influenceFactor = 0;
  ...
  private Iterator<?> getSortedNodeIterator(AggregateItem aitem,
      boolean centroidSort)
  {
   ...
   * explicitly connected neighbour, or the best neighbour which is also in the
   * set (see connectItem(aitem, item, visited) for detail on selection of best
   * neighbour. Stores the connections in the VIRTUAL_EDGES deque for each item
   * in the aggregate.
   * 
   * getAdjacentItem
   * 
  ...
  {
    // ("\nCalculating Virtual Edges");

    // Deque<VisualItem> visited = (Deque<VisualItem>) new
    // ArrayDeque<VisualItem>();
    ArrayList<VisualItem> visited = (ArrayList<VisualItem>) new ArrayList<VisualItem>();
    ...
    while (aNodeItems.hasNext())
    {
      ...
      // DEBUG
      // if(item instanceof EdgeItem)
      // System.out.println("\t\tItem (" + item.getX() + ", " +
      // item.getY() + ") is Edge " + item.get("id"));
      // else if(item instanceof NodeItem)
      // System.out.println("\t\tItem (" + item.getX() + ", " +
      // item.getY() + ") is Node " + item.getString(VisualItem.LABEL));

      // check for visible edge structure connecting this node to others
      if ((useStructuralEdges) && (item instanceof NodeItem))
      {
        NodeItem nodeItem = (NodeItem) item;
        Iterator<? extends VisualItem> edgeIterator = aitem
            .items(VISIBLE_EDGE_PREDICATE);
      ...
      else
      {
        ...
        item.set(VIRTUAL_EDGES, new ArrayList<Line2D>());
      }
      visited.add(item);
    }
  }
  ...
  /**
   * Find the sequence of virtual edges which connect a given item to it's best
   ...
   * 
   * 
   * NodeItems and EdgeItems are both treated equally here, but EdgeItems do not
   * have their X and Y fields set. I will add the appropriate calls to set them
   * before they are used. ams292 Aug 4, 2010.
   * 
   * @param aitem
   *          the set to search
   * @param item
   ...
   * @return a deque containing the virtual edges which connect the item to its
   *         best neighbour
   */
  //
  // private Deque<Line2D> connectItem(AggregateItem aitem, VisualItem item,
  // Collection<VisualItem> visited)
  private ArrayList<Line2D> connectItem(AggregateItem aitem, VisualItem item,
      Collection<VisualItem> visited)
  {
    double itemX = 0;
    double itemY = 0;
    ...
    {
      // System.out.println("\n\tconnectItem called on Node "+
      // item.get(VisualItem.LABEL)+" ("+ item.getX() + ", " +
      // item.getY()+")");
      itemX = item.getX();
      itemY = item.getY();
    }
    ...
    {
      // TODO
      // System.out.println("\n\tconnectItem called on Edge "+
      // item.get("id")+" ("+ item.getX() + ", " + item.getY()+")");

      itemX = item.getBounds().getCenterX();
      itemY = item.getBounds().getCenterY();

      // System.out.println("Setting itemX and itemY (" + itemX +", " +
      // itemY + ")");
    }

    VisualItem closestNeighbour = null;
    // Deque<Line2D> scannedLines = (Deque<Line2D>) new
    // ArrayDeque<Line2D>();
    // Deque<Line2D> linesToCheck = (Deque<Line2D>) new
    // ArrayDeque<Line2D>();

    ArrayList<Line2D> scannedLines = new ArrayList<Line2D>();
    ArrayList<Line2D> linesToCheck = new ArrayList<Line2D>();
    ...
    Iterator<VisualItem> neighbourIterator = visited.iterator();
    ...
    while (neighbourIterator.hasNext())
    {
      ...
      if (neighbourItem instanceof EdgeItem)
      {
        // System.out.println("\t\t***************************************\n"+
        // "\t\t* Neighbour Item is EdgeItem " +
        // neighbourItem.get("id"));
        // System.out.println("\t\t* Original Position: (" +
        // neighbourItem.getX() + ", " + neighbourItem.getY() + ")");

        nItemX = neighbourItem.getBounds().getCenterX();
        nItemY = neighbourItem.getBounds().getCenterY();

        // System.out.println("\t\t* Bounds Center: ("+neighbourItem.getBounds().getCenterX()
        // + ", " + neighbourItem.getBounds().getCenterY()+")");
        // System.out.println("\t\t* Using Center: ("+nItemX + ", " +
        // nItemY+")");
        // System.out.println("\t\t***************************************");
      }

      else if (neighbourItem instanceof NodeItem)
      {
        // System.out.println("\t\tNeighbour Item is NodeItem " +
        // neighbourItem.get(VisualItem.LABEL) +"(" +
        // neighbourItem.getX() + ", " + neighbourItem.getY() + ")");
        nItemX = item.getX();
        nItemY = item.getY();
      }

      double distance = Point2D.distance(itemX, itemY, nItemX, nItemY);
      ...
      if (considerBlockingItems)
      {
        ...
        // Line2D completeLine = new Line2D.Double(item.getX(),
        // item.getY(), neighbourItem.getX(), neighbourItem.getY());
        Line2D completeLine = new Line2D.Double(itemX, itemY, nItemX, nItemY);

        Iterator<?> interferenceItems = m_vis.getVisualGroup(sourceGroup)
            .tuples(visibleNodePredicate);
        numberInterferenceItems = countInterferenceItems(interferenceItems,
            aitem, completeLine);
        // add all non interference edges
        /*
         * if (numberInterferenceItems == 0) { scannedLines.push(completeLine);
         * }
         */
      }
    ...
    {
      Line2D completeLine = null;

      if (closestNeighbour instanceof EdgeItem)
        completeLine = new Line2D.Double(itemX, itemY, closestNeighbour
            .getBounds().getCenterX(), closestNeighbour.getBounds()
            .getCenterY());
      else
        completeLine = new Line2D.Double(itemX, itemY, closestNeighbour.getX(),
            closestNeighbour.getY());

      // route the edge around intersecting nodes not in set
      if (doMorph)
      {
        // linesToCheck.pop(completeLine);
        linesToCheck.add(completeLine);
        ...
        while (hasIntersection && iterations < MAX_ROUTING_ITERATIONS)
        {
          ...
          while (!hasIntersection && !linesToCheck.isEmpty())
          {
            ...
            Line2D line = linesToCheck.remove(linesToCheck.size() - 1);
            ...
            VisualItem closestItem = getCenterItem(interferenceItems, aitem,
                line);
            ...
            if (closestItem != null)
            {
              ...
              if (numIntersections == 2)
              {
                ...
                Point2D movePoint = rerouteLine(line, closestItem.getBounds(),
                    tempMorphBuffer, intersections, true);
                // test the movePoint already exists

                boolean foundFirst = (pointExists(movePoint,
                    linesToCheck.iterator()) || pointExists(movePoint,
                    scannedLines.iterator()));
                boolean pointInside = isPointInsideNode(movePoint, m_vis
                    .getVisualGroup(sourceGroup).tuples(visibleNodePredicate),
                    aitem);
                ...
                while ((!foundFirst) && (pointInside) && (tempMorphBuffer >= 1))
                {
                  ...
                  tempMorphBuffer /= 1.5;
                  movePoint = rerouteLine(line, closestItem.getBounds(),
                      tempMorphBuffer, intersections, true);
                  foundFirst = (pointExists(movePoint, linesToCheck.iterator()) || pointExists(
                      movePoint, scannedLines.iterator()));
                  pointInside = isPointInsideNode(
                      movePoint,
                      m_vis.getVisualGroup(sourceGroup).tuples(
                          visibleNodePredicate), aitem);
                }

                if ((movePoint != null) && (!foundFirst) && (!pointInside))
                {
                  // add 2 rerouted lines to check
                  // linesToCheck.push(new
                  // Line2D.Double(line.getP1(), movePoint));
                  // linesToCheck.push(new
                  // Line2D.Double(movePoint, line.getP2()));
                  linesToCheck.add(new Line2D.Double(line.getP1(), movePoint));
                  linesToCheck.add(new Line2D.Double(movePoint, line.getP2()));
                ...
                if (!hasIntersection)
                {
                  ...
                  if ((movePoint != null) && (!foundSecond))
                  {
                    // add 2 rerouted lines to check
                    // linesToCheck.push(new
                    // Line2D.Double(line.getP1(),
                    // movePoint));
                    // linesToCheck.push(new
                    // Line2D.Double(movePoint,
                    // line.getP2()));

                    linesToCheck
                        .add(new Line2D.Double(line.getP1(), movePoint));
                    linesToCheck
                        .add(new Line2D.Double(movePoint, line.getP2()));
        ...
        while (!linesToCheck.isEmpty())
        {
          ...
          scannedLines.add(linesToCheck.remove(linesToCheck.size() - 1));
        }
        ...
        if (doMerge)
        {
          while (!scannedLines.isEmpty())
          {
            ...
            Line2D line1 = scannedLines.remove(scannedLines.size() - 1);

            if (!scannedLines.isEmpty())
            {
              // Line2D line2 = scannedLines.pop();
              Line2D line2 = scannedLines.remove(scannedLines.size() - 1);
              ...
              else
              {
                ...
                linesToCheck.add(line1);
                scannedLines.add(line2);
              }
            }
            else
            {
      ...
      else
      {
        ...
        scannedLines.add(completeLine);
      }
    } // close if ((closestNeighbour != null) && (edgeInfluenceFactor != 0))

    /*
     * System.out.println(); for(Line2D l : scannedLines) {
     * System.out.println("\tReturned Line:\t(" + l.getX1() + ", " + l.getY1() +
     * ", " + l.getX2() + ", " + l.getY2()+")"); } //
     */
    return scannedLines;
  }
   ...
   */
  private boolean isPointInsideNode(Point2D movePoint, Iterator<?> tuples,
      AggregateItem exceptionItem)
  {
   ...
   *         otherwise
   */
  public boolean pointExists(Point2D pointToCheck,
      Iterator<Line2D> linesIterator)
  {
    boolean found = false;
    ...
    while ((linesIterator.hasNext()) && (!found))
    {
      ...
      if (MathLib.doublePointsEqual(checkEndPointsLine.getP1(), pointToCheck))
      {
        found = true;
      }
      if (MathLib.doublePointsEqual(checkEndPointsLine.getP2(), pointToCheck))
      {
        found = true;
      }
    }
   ...
   */
  public void calculatePointInfluence(double[][] potentialArea, double factor,
      double r1, double pointx, double pointy)
  {
   ...
   *          the line source
   */
  public void calculateLineInfluence(double[][] potentialArea,
      double influenceFactor, double r1, Line2D line)
  {
    double tempX, tempY, distance = 0;

    Rectangle2D r = line.getBounds2D();
    int startX = Math.min(Math.max(0, (int) ((r.getX() - r1) / pixelGroup)),
        potentialArea.length - 1);
    int startY = Math.min(Math.max(0, (int) ((r.getY() - r1) / pixelGroup)),
        potentialArea[startX].length - 1);
    int endX = Math.min(potentialArea.length - 1,
        Math.max(0, (int) ((r.getX() + r.getWidth() + r1) / pixelGroup)));
    int endY = Math.min(potentialArea[startX].length,
   ...
   * closest to the P1 endpoint and the item is not in the given aggregate. Note
   * that despite the shape of the rendered VisualItem, the rectangular bounds
   * are used as for this check.
   * 
   ...
   */
  public VisualItem getClosestItem(Iterator<?> interferenceItems,
      AggregateItem currentAggregate, Line2D testLine)
  {
    ...
    while (interferenceItems.hasNext())
    {
      ...
      if (!currentAggregate.containsItem(interferenceItem))
      {
        ...
        if ((interferenceItem.getBounds().getMinX() <= (Math.max(
            ...
            testLine.getX2())) && (interferenceItem.getBounds().getMaxY() >= Math
            .min(testLine.getY1(), testLine.getY2()))))))
        {

          double distance = fractionToLineEnd(interferenceItem.getBounds(),
              testLine);
   ...
   * @return the closest item or null if there are no intersections.
   */
  public VisualItem getCenterItem(Iterator<?> interferenceItems,
      AggregateItem currentAggregate, Line2D testLine)
  {
    double minDistance = Double.MAX_VALUE;
    VisualItem closestItem = null;
    ...
    while (interferenceItems.hasNext())
    {
      ...
      if (!currentAggregate.containsItem(interferenceItem))
      {
            ...
            .min(testLine.getY1(), testLine.getY2()))))))
        {
          ...
          double distance = fractionToLineCenter(interferenceItem.getBounds(),
              testLine);
          // find closest intersection
          if ((distance != -1) && (distance < minDistance))
          {
  ...
  public int countInterferenceItems(Iterator<?> interferenceItems,
      AggregateItem currentAggregate, Line2D testLine)
  {
    int count = 0;
    while (interferenceItems.hasNext())
   ...
   *          the line source
   */
  // public void calculateLinesInfluence(double[][]potentialArea, double
  // influenceFactor, double r1, Deque<Line2D> lines, Rectangle2D bounds) {
  public void calculateLinesInfluence(double[][] potentialArea,
      double influenceFactor, double r1, ArrayList<Line2D> lines,
      Rectangle2D bounds)
  {
    double tempX, tempY, distance = 0;
    double minDistance = Double.MAX_VALUE;
   ...
   *          the line source
   */
  public void calculateRectangleInfluence(double[][] potentialArea,
      double influenceFactor, double r1, Rectangle2D r)
  {
    double tempX, tempY, distance = 0;
    Line2D line;
        ...
        else
        {
          ...
          if ((outcode & Rectangle2D.OUT_TOP) == Rectangle2D.OUT_TOP)
          {
            ...
            if ((outcode & Rectangle2D.OUT_LEFT) == Rectangle2D.OUT_LEFT)
            {
              ...
              distance = Point2D.distance(tempX, tempY, r.getMinX(),
                  r.getMinY());
            }
            ...
            else
            {
              ...
              if ((outcode & Rectangle2D.OUT_RIGHT) == Rectangle2D.OUT_RIGHT)
              {
                ...
                distance = Point2D.distance(tempX, tempY, r.getMaxX(),
                    r.getMinY());
              }
              ...
              else
              {
                ...
                line = new Line2D.Double(r.getMinX(), r.getMinY(), r.getMaxX(),
                    r.getMinY());
                distance = line.ptSegDist(tempX, tempY);
              }
            }
          ...
          else
          {
            ...
            else
            {
              ...
              if ((outcode & Rectangle2D.OUT_LEFT) == Rectangle2D.OUT_LEFT)
              {
                ...
                line = new Line2D.Double(r.getMinX(), r.getMinY(), r.getMinX(),
                    r.getMaxY());
                distance = line.ptSegDist(tempX, tempY);
              }
              ...
              else
              {
                ...
                if ((outcode & Rectangle2D.OUT_RIGHT) == Rectangle2D.OUT_RIGHT)
                {
                  ...
                  line = new Line2D.Double(r.getMaxX(), r.getMinY(),
                      r.getMaxX(), r.getMaxY());
                  distance = line.ptSegDist(tempX, tempY);
                }
              }
   ...
   */
  public void calculateAreaInfluence(double[][] potentialArea,
      double influenceFactor, double r1, Area a, Rectangle2D activeArea)
  {
    double tempX, tempY, distance = 0;

    // create a deque of the lines
    // Deque<Line2D> lines = (Deque<Line2D>) new ArrayDeque<Line2D>();
    ...
    int startY = Math.min(
        ...
        potentialArea[startX].length - 1);
    int endX = Math.min(potentialArea.length - 1, Math.max(0,
   ...
   */
  public boolean march(ArrayList<Point2D> surface, double[][] potentialArea,
      int x, int y)
  {
  ...
  /**
   * Calculate the energy function of any hovered aggregates and visualize it in
   * the given graphics context.
   * 
   * @param g2d
   *          the graphics context into which to visualize the energy function
   */
  ...
  public void paintPotential(AggregateItem aitem, DecoratorItem di,
      Graphics2D g2d)
  {
    ...
    bounds.setRect(bounds.getX() - Math.max(edgeR1, nodeR1), bounds.getY()
        ...
        bounds.getWidth() + 2 * Math.max(edgeR1, nodeR1), bounds.getHeight()
    ...
    System.err.println("Aggregate info: threshold: " + threshold + " NNIF: "
        + negativeNodeInfluenceFactor + " NEIF: " + negativeEdgeInfluenceFactor
        + " NIF: " + nodeInfluenceFactor + " EIF: " + edgeInfluenceFactor);
    ...
    for (int x = 0; x < potentialArea.length - 1; x++)
    {
      for (int y = 0; y < potentialArea[x].length - 1; y++)
      {
        ...
        if (potentialArea[x][y] < 0)
          g2d.setColor(ColorLib.getColor(ColorLib.rgba(20, 20, 150,
              (int) Math.min(255, Math.abs((potentialArea[x][y] * 40))))));
        else
          g2d.setColor(ColorLib.getColor(ColorLib.rgba(150, 20, 20,
              (int) Math.min(255, Math.abs((potentialArea[x][y] * 40))))));
  ...
  public Intersection intersectLineLine(Line2D a, Line2D b)
  {
    ...
    double ua_t = (b.getX2() - b.getX1()) * (a.getY1() - b.getY1())
        - (b.getY2() - b.getY1()) * (a.getX1() - b.getX1());
    double ub_t = (a.getX2() - a.getX1()) * (a.getY1() - b.getY1())
        - (a.getY2() - a.getY1()) * (a.getX1() - b.getX1());
    double u_b = (b.getY2() - b.getY1()) * (a.getX2() - a.getX1())
    ...
    if (u_b != 0)
    {
      ...
      {
        result = new Intersection(a.getX1() + ua * (a.getX2() - a.getX1()),
  ...
  public double fractionToLineCenter(Rectangle2D bounds, Line2D line)
  {
    ...
    // top
    testDistance = fractionAlongLineA(line, new Line2D.Double(bounds.getMinX(),
    ...
    // right
    testDistance = fractionAlongLineA(line, new Line2D.Double(bounds.getMaxX(),
   ...
   * @param intersections
   *          an array of at least 4 intersections where the intersections will
   *          be stored as top, left, bottom, right
   * @return the number of intersection points found (doesn't count coincidental
   *         lines)
   */
  public int testIntersection(Line2D line, Rectangle2D bounds,
      Intersection[] intersections)
  {
    ...
    // top
    intersections[0] = intersectLineLine(line,
        new Line2D.Double(bounds.getMinX(), bounds.getMinY(), bounds.getMaxX(),
            bounds.getMinY()));
    if (intersections[0].state == State.Point)
      countIntersections++;

    // left
    intersections[1] = intersectLineLine(line,
        new Line2D.Double(bounds.getMinX(), bounds.getMinY(), bounds.getMinX(),
            bounds.getMaxY()));
    ...
    // bottom
    intersections[2] = intersectLineLine(line,
        new Line2D.Double(bounds.getMinX(), bounds.getMaxY(), bounds.getMaxX(),
            bounds.getMaxY()));
    if (intersections[2].state == State.Point)
      countIntersections++;

    // right
    intersections[3] = intersectLineLine(line,
        new Line2D.Double(bounds.getMaxX(), bounds.getMinY(), bounds.getMaxX(),
            bounds.getMaxY()));
   ...
   * @param rerouteBuffer
   *          the buffer to place between the rectangle corner and the new point
   *          position
   * @param wrapNormal
   ...
   */
  public Point2D moveEndPoint(Point2D oldPoint, Rectangle2D rectangle,
      double rerouteBuffer, boolean wrapNormal)
  {
    ...
    if (wrapNormal)
    {
      ...
      if (oldPoint.getY() < bounds.getCenterY())
      {
        ...
        {
          // bottom right
          newPoint = new Point2D.Double(bounds.getMaxX() + rerouteBuffer,
              bounds.getMaxY() + rerouteBuffer);
        }
        else
        {
          // bottom left
          newPoint = new Point2D.Double(bounds.getMinX() - rerouteBuffer,
              bounds.getMaxY() + rerouteBuffer);
        }
      }
      else
   ...
   * @param rerouteBuffer
   *          the buffer to place between the selected reroute corner and the
   *          new point
   * @param intersections
   ...
   */
  public Point2D rerouteLine(Line2D line, Rectangle2D rectangle,
      double rerouteBuffer, Intersection[] intersections, boolean wrapNormal)
  {
    Rectangle2D bounds = rectangle.getBounds2D();
    ...
    if (wrapNormal)
    {
      if (leftIntersect.state == State.Point)
      {
        ...
        // top area
        double topArea = bounds.getWidth()
            * (((leftIntersect.getY() - bounds.getY()) + (rightIntersect.getY() - bounds
                .getY())) / 2);
      ...
      else
      {
        ...
        else
        {
          ...
          double totalArea = bounds.getHeight() * bounds.getWidth();
          // top area
          double leftArea = bounds.getHeight()
  ...
  public BubbleSetLabelLayout createLabelLayout(String group, String labelField)
  {
  ...
  /**
   * Positions aggregate labels (DecoratorItems) along the longest virtual edge
   * in the aggregate. Rotates label up to +-45 degress to follow the edge.
   * Label size is modulated to fit within the length of the edge.
   * 
   * @author Christopher
   */
  public class BubbleSetLabelLayout extends ItemAction
  {
    ...
    public BubbleSetLabelLayout(String group, String titleField)
    {
      super(group);
    ...
    public void process(VisualItem visualItem, double frac)
    {
      ...
      if (g == null)
      {
        g = (new BufferedImage(2, 2, BufferedImage.TYPE_4BYTE_ABGR_PRE))
            .getGraphics();
      }
      ...
      if (aNodeItems.hasNext())
      {
        ...
        longestLine = new Line2D.Double(item.getBounds().getX(), item
            .getBounds().getY() - nodeR0 / 2, item.getBounds().getMaxX(), item
            .getBounds().getY() - nodeR0 / 2);
      ...
      longestLine.setLine(longestLine.getP1().getX(), longestLine.getP1()
          .getY() - fm.getDescent(), longestLine.getP2().getX(), longestLine
          .getP2().getY() - fm.getDescent());
      ...
      while (aNodeItems.hasNext())
      {
        ...
        // Deque<Line2D> edges = (Deque<Line2D>)
        // item.get(VIRTUAL_EDGES);
        ArrayList<Line2D> edges = (ArrayList<Line2D>) item.get(VIRTUAL_EDGES);
      ...
      // rotate along line
      double rotation = Math.toDegrees(Math.atan2(longestLine.getY2()
          - longestLine.getY1(), longestLine.getX2() - longestLine.getX1()));
