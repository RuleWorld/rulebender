...
public class BioNetGenUtility 
{
	 ...
	 */
	public static boolean parameterScan(String filePath, ParameterScanData data, String bngPath, String scriptFullPath, String resultsPath)
	{
		ParameterScanJob job = new ParameterScanJob(filePath, bngPath, scriptFullPath, data, resultsPath);
		
		try 
		{
			new ProgressMonitorDialog(Display.getDefault().getActiveShell()).run(true, true, job);
		} 
		...
		{			
			...
			System.out.println("Shell null? " + BNGEditor.getMainEditorShell()==null ? "YES" : "NO");
			MessageBox errorMessage = new MessageBox(BNGEditor.getMainEditorShell(), SWT.ICON_ERROR | SWT.OK);
	 ...
	 * @param filePath
	 */
	public static boolean runBNGLFile(String filePath, String bngFullPath, String resultsPath)
	{
		...
		BNGExecutionJob job = new BNGExecutionJob(filePath, bngFullPath, resultsPath);
