...
public class ResultsFileUtility
{
  ...
  private static String getResultsDirectoryForFile(File selectedFile)
  {
    ...
    String pathToReturn = ConfigurationManager.getConfigurationManager()
        .getWorkspacePath() + System.getProperty("file.separator");

    // Add the results directory
    pathToReturn += "BNGResults" + System.getProperty("file.separator");

    // Add the name of the file
    pathToReturn += selectedFile.getName().substring(0,
        selectedFile.getName().indexOf(".bngl"))
  ...
  {
    return (getResultsDirectoryForFile(selectedFile) + "parascan-"
        + getCurrentDateAndTime() + System.getProperty("file.separator"));
  }


  public static String getSimulationResultsDirectoryForFile(File selectedFile)
  {
    return (getResultsDirectoryForFile(selectedFile) + getCurrentDateAndTime() + System
