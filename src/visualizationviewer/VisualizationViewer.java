...
class VisualizationViewer extends JFrame {
	...
	private JSplitPane mainPanel, cMapAnnotationSplit;
	...
	/**
	 * Private constructor for singleton pattern. The constructor instantiates
	 * all of the gui elements and packs them into the frame.
	 * 
	 * @param control_in
	 *            This argument is the controller that will be sending events
	 *            and commands to the viewer.?
	 */
	private VisualizationViewer(VisualizationViewerController control_in) {
		...
		speciesBrowserLayeredPane = new SpeciesBrowserLayeredPane(
				speciesBrowserSize);
		 ...
		 * 
		 * // Species Browser overview speciesBrowserOverviewJPanel = new
		 * JPanel(); speciesBrowserOverviewJPanel.setBorder(border);
		 * speciesBrowserJPanel.setPreferredSize(new Dimension(400, 125));
		 * speciesBrowserOverviewJPanel.setBackground(Color.WHITE);
		 * 
		 * 
		 * mainBrowserPanel = new JPanel(); mainBrowserPanel.setLayout(new
		 * BoxLayout(mainBrowserPanel, BoxLayout.Y_AXIS));
	...
	/**
	 * This method is used instead of the constructor. Only one instance of the
	 * VisualizationViewer can be created.
	 * 
	 * @return the VisualizationViewer instance.
	 */
	protected static synchronized VisualizationViewer getViewer(
			VisualizationViewerController controller_in) {
		if (viewer == null) {
	...
	/**
	 * Give the VisualizationViewer a Display to use for the contact map.
	 * 
	 * @param d
	 *            A Display instance
	 */
	...
	/**
	 * Give the VisualizationViewer a Display to use for the influence graph.
	 * 
	 * @param d
	 *            A Display instance
	 */
	...
	/**
	 * Give the VisualizationViewer a Display to use for the species browser.
	 * 
	 * @param d
	 *            A Display instance
	 */
	protected void setSpeciesBrowser(Display d) {
		...
		/*
		 * if(speciesBrowserJPanel.getComponentCount() > 0) { // This should
		 * allow the visualization to be garbage collected.
		 * speciesBrowserJPanel.removeAll();
		 * //speciesBrowserJPanel.revalidate(); } if(d != null) {
	...
	/**
	 * Give the VisualizationViewer a Display to use for the over of the cMap
	 * 
	 * @param cMapOverview
	 */
	...
	/**
	 * Give the VisualizationViewer a Display to use for the over of the iGraph
	 * 
	...
	/**
	 * Give the VisualizationViewer a Display to use for the over of the
	 * speciesBrowser.
	 * 
	...
	protected void setSpeciesBrowserOverview(Display od) {
		 ...
		 * 
		 * if (speciesBrowserOverview != null &&
		 * speciesBrowserJPanel.getComponentCount() == 1) {
		 * 
		 * // absolute layout speciesBrowserOverviewJPanel.setLayout(null);
		 * 
		 * Display dis_focus = (Display) speciesBrowserJPanel.getComponent(0);
		 * 
		 * // compute the size of select box int x, y, width, height; double
		 * ratio; // scale of overview over focus
		 * 
		 * double[] result = computeSelectBox(speciesBrowserOverviewJPanel,
		 * speciesBrowserOverview, dis_focus, "component_graph"); x = (int)
		 * result[0]; y = (int) result[1]; width = (int) result[2]; height =
		 * (int) result[3]; ratio = result[4];
		 * 
		 * // get insets of overviewPanel Insets insets =
		 * speciesBrowserOverviewJPanel.getInsets(); // get the size of
		 * overviewPanel Dimension size_panel =
		 * speciesBrowserOverviewJPanel.getSize();
		 * 
		 * // create select box speciesBrowserRectanglePanel = new
		 * RectanglePanel(width, height); // set bounds for select box
		 * speciesBrowserRectanglePanel.setBounds(x, y, width, height); // set
		 * select box be transparent
		 * speciesBrowserRectanglePanel.setOpaque(false); // add select box to
		 * panel speciesBrowserOverviewJPanel.add(speciesBrowserRectanglePanel);
		 * 
		 * // set bounds for overview display
		 * speciesBrowserOverview.setBounds(insets.left, insets.top,
		 * size_panel.width, size_panel.height); // add overview display to
		 * panel speciesBrowserOverviewJPanel.add(speciesBrowserOverview); //
		 * speciesBrowserOverviewJPanel.revalidate();
		 * 
		 * // add mouse listener for select box speciesBrowserSelectBoxControl =
		 * new SelectBoxControl(x, y, width, height,
		 * speciesBrowserRectanglePanel, dis_focus, ratio);
	...
	public void updateSpeciesBrowserSelectBox() {
		 ...
		 * 
		 * double[] result = computeSelectBox(speciesBrowserOverviewJPanel,
		 * (Display) speciesBrowserOverviewJPanel.getComponent(1), (Display)
		 * speciesBrowserJPanel.getComponent(0), "component_graph");
		 * 
		 * speciesBrowserSelectBoxControl.updateInfo((int) result[0], (int)
		 * result[1], (int) result[2], (int) result[3], result[4]); }
		 */
	...
	protected void setAnnotationPanel(TableModel tm) {
		...
		// When this property is true the table uses the entire height of the
		// container,
	 ...
	 */
	/*
	 * private double[] computeSelectBox(JPanel overviewPanel, Display
	 * overviewDis, Display focusDis, String graphName) { double[] result = new
	 * double[5];
	 * 
	 * // get insets of overviewPanel Insets insets = overviewPanel.getInsets();
	 * 
	 * // compute the size of select box int x, y, width, height;
	 * 
	 * // focus display width height Display dis_focus = focusDis; double
	 * scale_focus = dis_focus.getScale(); int dis_focus_width =
	 * dis_focus.getVisibleRect().width; int dis_focus_height =
	 * dis_focus.getVisibleRect().height;
	 * 
	 * // compute ratio double scale_overview = overviewDis.getScale(); double
	 * ratio = scale_overview / scale_focus;
	 * 
	 * // set width and height for select box width = (int)(dis_focus_width *
	 * ratio); height = (int)(dis_focus_height * ratio);
	 * 
	 * // compute the position of select box
	 * 
	 * // focus display x y double dis_focus_x = dis_focus.getDisplayX(); double
	 * dis_focus_y = dis_focus.getDisplayY();
	 * 
	 * // focus vis x y Visualization vis_focus= dis_focus.getVisualization();
	 * Rectangle2D visbounds_focus = vis_focus.getBounds(graphName); double
	 * vis_focus_x = visbounds_focus.getX(); double vis_focus_y =
	 * visbounds_focus.getY();
	 * 
	 * // compute distance int distance_x = (int)((dis_focus_x - vis_focus_x) *
	 * ratio); int distance_y = (int)((dis_focus_y - vis_focus_y) * ratio);
	 * 
	 * // overview dis x y double dis_overview_x = overviewDis.getDisplayX();
	 * double dis_overview_y = overviewDis.getDisplayY();
	 * 
	 * // overview vis x y Visualization vis_overview=
	 * overviewDis.getVisualization(); Rectangle2D visbounds_overview =
	 * vis_overview.getBounds(graphName); double vis_overview_x =
	 * visbounds_overview.getX() * ratio; double vis_overview_y =
	 * visbounds_overview.getY() * ratio;
	 * 
	 * // set x and y for select box upper-left corner x = (int)vis_overview_x +
	 * (insets.left - (int)dis_overview_x) + distance_x; y = (int)vis_overview_y
	 * + (insets.top - (int)dis_overview_y) + distance_y;
	 * 
	 * result[0] = x; result[1] = y; result[2] = width; result[3] = height;
	 * result[4] = ratio; return result; }
	 */
	/*
	...
	/*
	 * public void resizeSpeciesBrowserOverviewDisplay() { Dimension size =
	 * speciesBrowserOverviewJPanel.getSize();
	 * 
	 ...
	 */
	/*
	 * public void resizeSpeciesBrowserDisplay() { Dimension size =
	 * speciesBrowserJPanel.getSize();
	 * 
	 * if(speciesBrowserJPanel.getComponentCount() == 1) {
	...
	public void resizeSpeciesBrowser() {
		speciesBrowserLayeredPane.resizeSpeciesBrowser(speciesBrowserJP
				.getSize());
	}
