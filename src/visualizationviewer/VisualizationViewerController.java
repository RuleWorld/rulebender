...
public class VisualizationViewerController {
	...
	//they will update the screen but only for the first time that you see them.
	...
	public static synchronized VisualizationViewerController loadVisualizationViewController()
	{
		if(control == null)
		{
			...
			// Tell the LinkHub to use the AnnotationPanelController class for all of the 
			// linking of the annotation panel.
			// The control object is passed so that the AnnotationPanelController can pass
			// back a tablemodel to be displayed in the view.
			LinkHub.getLinkHub().registerLinkedViewsListener(new AnnotationPanelControllerNoModelGuide(control));
		}
	...
	public void fileBecomesFocus(CurrentFile currentFile_in) 
	{
		...
		System.out.println("Sending null to the visviewer controller because a new file is selected");
	...
	private void updateCMap()
	{
		...
		{	
			//DEBUG
			//System.out.println("currentFile : " + (currentFile == null ? "null" : "not null"));
			//System.out.println("currentFile.getCMapVisualModel() : " + (currentFile.getCMapVisualModel() == null ? "null" : "not null"));
			//System.out.println("currentFile.getCMapVisualModel().getDisplay() : " + (currentFile.getCMapVisualModel().getDisplay() == null ? "null" : "not null"));
	...
	{
		//System.out.println("Informed of new CMap Model from "+"for currentFile = " + currentFile.getFileName() );
		if (viewer.isVisible())
		{
				updateCMap();
		}
	...
	{	
		...
		/* I'm pretty sure this is unnecesary because if it is null then nothing will happen
		 * and the previous one will be in the window still.
		 * 
		else {
