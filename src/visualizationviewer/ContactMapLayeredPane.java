...
public class ContactMapLayeredPane extends JLayeredPane 
{
	...
	public void setCMapOverview(Display od) 
	{
		...
		if(od != null && cMapJPanel.getComponentCount() == 1)
		{
			...
			cMapSelectBoxControl = new SelectBoxControl(0, 0, 0, 0, cMapRectanglePanel, dis_focus, 0);
	...
	{	
		...
		if(cMapJPanel != null && cMapOverviewJPanel != null)
		{
			...
			cMapOverviewJPanel.setBounds(0, size.height-OVERVIEW_HEIGHT, OVERVIEW_WIDTH-BORDER_WIDTH, OVERVIEW_HEIGHT-BORDER_WIDTH);
			
			if(cMapJPanel.getComponentCount() == 1 && cMapOverviewJPanel.getComponentCount() > 1)
			{
				((Display) cMapJPanel.getComponent(0)).setSize(new Dimension(currentSize.width-BORDER_WIDTH*2, currentSize.height-BORDER_WIDTH*2));
				((Display) cMapOverviewJPanel.getComponent(1)).setBounds(BORDER_WIDTH, BORDER_WIDTH, OVERVIEW_WIDTH-BORDER_WIDTH*3, OVERVIEW_HEIGHT-BORDER_WIDTH*2);
				((Display) cMapOverviewJPanel.getComponent(1)).setSize(new Dimension(OVERVIEW_WIDTH-BORDER_WIDTH*3, OVERVIEW_HEIGHT-BORDER_WIDTH*2));
	...
	private double[] computeSelectBox(JPanel overviewPanel, Display overviewDis, Display focusDis, String graphName) 
	{
		...
		// TODO bug here, the overviewDis.getScale is returning 1, when the overview window should have a smaller scale. 
