...
public class InfluenceGraphLayeredPane extends JLayeredPane
{
	...
	public void setIGraphOverview(Display od) 
	{
		...
		if(od != null && iGraphJPanel.getComponentCount() == 1)
		{
			...
			double[] result = computeSelectBox(iGraphOverviewJPanel, od, dis_focus, "igraph");
			x = (int)result[0];
			y = (int)result[1];
			width = (int)result[2];
			height = (int)result[3];
	        ...
	        //od.setBounds(insets.left, insets.top, size_panel.width, size_panel.height);
	...
	{		
		...
		if(iGraphJPanel != null && iGraphOverviewJPanel != null)
		{
			...
			iGraphOverviewJPanel.setBounds(0, currentSize.height-OVERVIEW_HEIGHT, OVERVIEW_WIDTH-BORDER_WIDTH, OVERVIEW_HEIGHT-BORDER_WIDTH);
			
			if( iGraphJPanel.getComponentCount() == 1 && iGraphOverviewJPanel.getComponentCount() > 1)
			{
				((Display)iGraphJPanel.getComponent(0)).setSize(new Dimension(size.width-BORDER_WIDTH*2, size.height-BORDER_WIDTH*2));
				((Display) iGraphOverviewJPanel.getComponent(1)).setBounds(BORDER_WIDTH, BORDER_WIDTH, OVERVIEW_WIDTH-BORDER_WIDTH*3, OVERVIEW_HEIGHT-BORDER_WIDTH*2);
				((Display) iGraphOverviewJPanel.getComponent(1)).setSize(new Dimension(OVERVIEW_WIDTH-BORDER_WIDTH*3, OVERVIEW_HEIGHT-BORDER_WIDTH*2));			
			}
		}		
	 ...
	 * 
	 * TODO this should be a part of the rectangle control or rectangle panel class, but I don't have the 
