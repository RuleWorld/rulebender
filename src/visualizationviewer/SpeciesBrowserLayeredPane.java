	...
	{ 
		...
		public void setSpeciesBrowserOverview(Display od) 
		{
			...
			if(od != null && speciesBrowserJPanel.getComponentCount() == 1)
			{
				...
				double[] result = computeSelectBox(speciesBrowserOverviewJPanel, od, dis_focus, "component_graph");
				x = (int)result[0];
				y = (int)result[1];
				width = (int)result[2];
				height = (int)result[3];
				...
				//od.setBounds(insets.left, insets.top, size_panel.width/2, size_panel.height);
		        //od.setBounds(BORDER_WIDTH, BORDER_WIDTH, OVERVIEW_WIDTH-BORDER_WIDTH*2, OVERVIEW_HEIGHT-BORDER_WIDTH);
				...
				speciesBrowserSelectBoxControl = new SelectBoxControl(x, y, width, height, speciesBrowserRectanglePanel, dis_focus, ratio);
		...
		{
			return new Dimension(speciesBrowserJPanel.getSize().width  - border.getBorderInsets(speciesBrowserJPanel).left
															 - border.getBorderInsets(speciesBrowserJPanel).right,
								 speciesBrowserJPanel.getSize().height - border.getBorderInsets(speciesBrowserJPanel).top
															 - border.getBorderInsets(speciesBrowserJPanel).bottom);
		}
		
		public Dimension getSpeciesBrowserOverviewSize() 
		{	
			return new Dimension(speciesBrowserOverviewJPanel.getSize().width  - border.getBorderInsets(speciesBrowserOverviewJPanel).left
																	 - border.getBorderInsets(speciesBrowserOverviewJPanel).right,
								 speciesBrowserOverviewJPanel.getSize().height - border.getBorderInsets(speciesBrowserOverviewJPanel).top
		...
		{	
			...
			if(speciesBrowserJPanel != null && speciesBrowserOverviewJPanel != null)
			{
				...
				speciesBrowserJPanel.setBounds(0, 0, currentSize.width, currentSize.height);
				speciesBrowserOverviewJPanel.setBounds(0, size.height-OVERVIEW_HEIGHT, OVERVIEW_WIDTH-BORDER_WIDTH, OVERVIEW_HEIGHT-BORDER_WIDTH);
				
				if(speciesBrowserJPanel.getComponentCount() == 1 && speciesBrowserOverviewJPanel.getComponentCount() > 1)
				{
					((Display) speciesBrowserJPanel.getComponent(0)).setSize(new Dimension(currentSize.width-BORDER_WIDTH*2, currentSize.height-BORDER_WIDTH*2));
					((Display) speciesBrowserOverviewJPanel.getComponent(1)).setBounds(BORDER_WIDTH, BORDER_WIDTH, OVERVIEW_WIDTH-BORDER_WIDTH*3, OVERVIEW_HEIGHT-BORDER_WIDTH*2);
					((Display) speciesBrowserOverviewJPanel.getComponent(1)).setSize(new Dimension(OVERVIEW_WIDTH-BORDER_WIDTH*3, OVERVIEW_HEIGHT-BORDER_WIDTH*2));
