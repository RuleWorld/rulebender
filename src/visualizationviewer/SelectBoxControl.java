...
public class SelectBoxControl implements MouseListener, MouseMotionListener {
	 ...
	 * update is called, the painted values are always the same, so the problem
	 * is in the passed values.
	 * 
	 * @param x
	 * @param y
	 * @param width
	...
	public void updateInfo(int x, int y, int width, int height, double result) {
		...
		this.ratio = result;
		/*
		 * } else { upperleft_x = 10; upperleft_y = 10; this.width = 50;
		 * this.height = 50; this.ratio = result; }
		 */
		selectbox.setBounds((int) upperleft_x, (int) upperleft_y,
