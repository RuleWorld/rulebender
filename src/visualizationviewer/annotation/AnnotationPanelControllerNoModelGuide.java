...
public class AnnotationPanelControllerNoModelGuide implements
	 ...
	 * @param viewer
	 */
	public AnnotationPanelControllerNoModelGuide(
			VisualizationViewerController viewer) {
		visviewer = viewer;
	}
	 ...
	 * ArrayList<String> stateList = (ArrayList) item.get("states"); if
	 * (stateList != null) { for (int i = 0; i < stateList.size() - 1; i++) {
	 * states = states + stateList.get(i) + ", "; } states = states +
	 ...
	 * {"Comments", ""}};
	 * 
	 * if (item.canGet("annotation", TableModel.class)) { // names =
	 * {"Attribute", "Value"} as default this.updateAnnotationTable(item,
	...
	private void setAnnotationForAggregate(VisualItem item) {
		...
		Object[][] data = {
				...
				// {"Online resources", ""},
				{ "UniProt", "" }, { "PathwayCommons", "" }, { "HPRD", "" },
				{ "Reactome", "" }, { "UCSD-Nature", "" }, { "InterPro", "" },
				{ "PROSITE", "" }, { "KEGG", "" }, { "ChEBI", "" },
				{ "PubChem", "" } };

		if (item.canGet("annotation", TableModel.class)) {
			// names = {"Attribute", "Value"} as default
			this.updateAnnotationTable(item,
					(TableModel) item.get("annotation"), null, data);
		} else {
			this.updateAnnotationTable(item, null, null, data);
		}
	}
	 ...
	 * table model, and then the table model updates the annotation panel and is
	...
	public void componentSelectedInContactMap(VisualItem componentItem) {
		...
		Object[][] data = {
				...
				{ "Molecule",
						componentItem.getString("molecule_expression").trim() },
				{ "Location", "" },
		// {"Comments", ""}
		};
	...
	public void moleculeSelectedInText(String moleculeText) {
		// System.out.println("moleculeSelectedInText called in AnnotationPanelController for "
		// + moleculeText);
	...
	public void ruleSelectedInText(String ruleText) {
		// System.out.println("ruleSelectedInText called in AnnotationPanelController for "
		// + ruleText);
