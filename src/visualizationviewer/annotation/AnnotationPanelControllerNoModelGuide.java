...
public class AnnotationPanelControllerNoModelGuide implements LinkedViewsReceiverInterface 
{
	...
	/**
	 * Constructor takes a VisualizationViewerController so it can send the TableModel
	 * to the correct controller.
	 * @param viewer
	 */
	public AnnotationPanelControllerNoModelGuide(VisualizationViewerController viewer) 
	{
		visviewer = viewer;
	}
	...
	private void setAnnotationForNode(VisualItem item) 
	{
		...
		if (type.equals("state")) {
			...
			updateAnnotationTableForStateNode(item, rules);
		...
		// component node
			...
			Object[][] data = {{"Name of component", ((String) item.get(VisualItem.LABEL)).trim()},
					{"Internal states", states.trim()},
					{"Molecule", item.getString("molecule_expression").trim()},
					{"Location", ""},
					{"Comments", ""}};
			
			if (item.canGet("annotation", TableModel.class)) {
				// names = {"Attribute", "Value"} as default
				this.updateAnnotationTable(item, (TableModel)item.get("annotation"), null, data);
			}
			else {
...
*/	
	...
	/**
	 * Handles when aggregates (molecules or compartments) are selected in the contact map.
	 * TODO separate the molecules and compartments and create a LinkHub event for compartments.
	 * @param item
	 */
	 ...
	 * @param rowIndex_rule
	 */
	private void parseRulesForAnnotation(ArrayList<VisualRule> rules, Object[][] data, int n_col, int rowIndex_rule) 
	{
		...
		for (; i < rules.size() + rowIndex_rule; i++) {
			...
			// constraints
			if (wholerule.indexOf("include")!=-1 || wholerule.indexOf("exclude")!=-1
	...
	/**
	 * This method is for visual items with an associated arraylist of rules.  
	 * The item and rules are passed in and the rules are used to build the 
	 * table model, and then the table model updates the annotation panel and
	 * is attached to the item.
	 * 
	 * @param item
	 * @param rules
	 */
	private void updateAnnotationTableForEdges(VisualItem item, ArrayList<VisualRule> rules) 
	{
	 ...
	 * @param rules
	 */
	private void updateAnnotationTableForStateNode(VisualItem item, ArrayList<VisualRule> rules) 
	{
		// number of items in annotation table
		int n_items = 4;
		// number of columns in annotation table
	 ...
	 * @param data
	 */
	private void updateAnnotationTable(VisualItem item, TableModel tableModel, String[] names, Object[][] data) 
	{
		if (tableModel != null) 
		{
		...
		if (names == null) 
		{
			if (data == null) 
			{
				System.out.println("Sending null to the visviewer controller because names and data are null");
	...
	/*
	 * Begin LinkedViewsReceiverInterface implementations.
	 * TODO A lot of these just call a single method that is implemented above.  I can move 
	 * those implementations down here.  
	 */
	...
	{
		// show details in the table in the annotation panel
		String[] names = {"ID", "BNGL", "Rate", "Constraints"};
		...
		// whole rule
		// A() + B() <-> C() rate1, rate2
		String wholerule = (String) ruleItem.getName();
		...
		// update table
		TableModel tm = new AnnotationTableModel(names, data);
		System.out.println("tm in iGraph click control is " + (tm==null ? "null" : "not null"));
		visviewer.updateAnnotationTable(tm);
	...
	public void componentSelectedInContactMap(VisualItem componentItem) 
	{
		...
		// show details in the table in the annotation panel
		Object[][] data = {{"Name of component", ((String) componentItem.get(VisualItem.LABEL)).trim()},
				{"Internal states", states.trim()},
		...
		if (componentItem.canGet("annotation", TableModel.class)) {
			// names = {"Attribute", "Value"} as default
			this.updateAnnotationTable(componentItem, (TableModel)componentItem.get("annotation"), null, data);
		}
		else {
	...
	{
		//System.out.println("moleculeSelectedInText called in AnnotationPanelController for " + moleculeText);
		//Visualization vis = visviewer.getContactMapVisualization();
		
		// TODO Get the Visualization from the controller and do a search for the VisualItem.
		// Then send the VisualItem to the same method that handles it for the contact map.
	}

	public void ruleSelectedInText(String ruleText) 
	{
		// System.out.println("ruleSelectedInText called in AnnotationPanelController for " + ruleText);
		// Visualization vis = visviewer.getInfluenceGraphVisualization();
		
		// TODO Get the Visualization from the controller and do a search.
		// Then send the VisualItem to the same method that handles it for the influence graph.
	}
