 ...
 */
public class CustomizedChartComposite extends Composite implements
	 ...
	 */
	public CustomizedChartComposite(Composite comp, int style,
			JFreeChart chart, boolean properties, boolean save, boolean print,
	...
	public void mouseUp(MouseEvent event) {
		...
		if (this.zoomRectangle == null) {
			 ...
			 * event.x, event.y); displayPopupMenu(pt.x, pt.y); }
			 */

			// new code
			if (this.popup != null) {
				if (event.button == 3
						|| (event.button == 1 && (event.stateMask & SWT.CTRL) != 0)) {
					org.eclipse.swt.graphics.Point pt = this.canvas.toDisplay(
							event.x, event.y);
