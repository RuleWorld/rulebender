...
public class SpeciesGraph
{
  ...
  private void parseSpecies(int id, String speciesExp)
  {
    ...
    for (int i = 0; i < molecules.length; i++)
    {
      ...
      String moleculeName = curMolecule.substring(0, curMolecule.indexOf("("));

      // create a molecule using the name
      SMolecule mole = new SMolecule(i + 1, moleculeName);

      // parse the components
      String componentExp = curMolecule.substring(curMolecule.indexOf("(") + 1,
          curMolecule.lastIndexOf(")"));
      ...
      for (int j = 0; j < components.length; j++)
      {
        ...
        for (int t = 0; t < curComponent.length(); t++)
        {
          if (curComponent.charAt(t) == '!')
          {
            ...
            {
              bondIdStr += Character.toString(curComponent.charAt(t));
              t++;
            ...
            if (species.getBondById(bondId) != null)
            {
              ...
              {
                String state = curComponent.substring(curTilde + 1,
                    curExclamation);
              ...
              {
                // add right part of bond
                bond.setRightPart(mole.getId(), comp.getId(), -1);
              }
            }
            else
            {
              ...
              if (curTilde != -1 && curTilde + 1 < curExclamation)
              {
                ...
                bond = new SBond(bondId, mole.getId(), comp.getId(),
                    comp.getStateId(state));
              }
              // no state
              else
              {
                bond = new SBond(bondId, mole.getId(), comp.getId(), -1);
              }
  ...
  private void createGraph()
  {
    ...
    /*
     * 
     * // add the expression of the species Node exp = comp_graph.addNode(); if
     * (species.getId() != -1) { // species exp.setString(VisualItem.LABEL,
     * Integer.toString(species.getId())+ " " + species.getExpression()); } else
     * { // not a species, just expression exp.setString(VisualItem.LABEL,
     * species.getExpression()); }
     */
    ...
    for (int i = 0; i < molecules.size(); i++)
    {
      ...
      else
      {
        ...
        AggregateItem aggregateForMolecule = aggregates.get(tmole.getId() + "");
        ...
        for (int j = 0; j < tmole.getComponents().size(); j++)
        {
          ...
          // If it has states
          if (tcomp.getStates() != null && tcomp.getStates().size() != 0)
          {
            ...
            for (int k = 0; k < tcomp.getStates().size(); k++)
            {
              ...
              // set its ID
              n_state.set("ID", tmole.getId() + "." + tcomp.getId() + "." + k);
              ...
              // add it to the hashtable
              nodes.put(tmole.getId() + "." + tcomp.getId() + "." + k, n_state);
              // add it to the aggregate
    ...
    for (int l = 0; l < bonds.size(); l++)
    {
      ...
      Node leftnode = nodes.get(tbond.getLeftM() + "." + tbond.getLeftC());
      Node rightnode = nodes.get(tbond.getRightM() + "." + tbond.getRightC());
      ...
      if (tbond.getRightS() != -1)
      {
        ...
        rightnode = nodes.get(tbond.getRightM() + "." + tbond.getRightC() + "."
            + tbond.getRightS()); // state
        // node
        if (rightnode != null)
          rightnode.set("hasedge", true);
      }
