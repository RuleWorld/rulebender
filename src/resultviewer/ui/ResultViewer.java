...
public class ResultViewer extends ApplicationWindow
{
    ...
    file_tv.addDoubleClickListener(new IDoubleClickListener()
    {
      public void doubleClick(DoubleClickEvent e)
      {
                ...
                .getName().endsWith(".scan")))
        {
          ...
          data.setExpandedElements(element_tv_gdat.getExpandedElements());
  ...
  private void initTextFolder()
  {
    ...
    textFolder.addSelectionListener(new SelectionAdapter()
    {
      ...
      public void widgetSelected(SelectionEvent event)
      {
        ...
        {
          // CDAT & GDAT & SCAN
          int chartIndex = ((Composite) item.getControl()).getChildren().length - 1;
  ...
  private void createNETOutline()
  {
    ...
    element_tv_net.getTree().setLayoutData(new GridData(GridData.FILL_BOTH));
    ...
    element_tv_net.addSelectionChangedListener(new ISelectionChangedListener()
    {
      ...
      public void selectionChanged(SelectionChangedEvent event)
      {
        ...
        NETItemNode node = (NETItemNode) selection.getFirstElement();
  ...
  private void createCDATOutline()
  {
    ...
    element_tv_cdat.addSelectionChangedListener(new ISelectionChangedListener()
    {
  ...
  private void createGDATOutline()
  {
    ...
    element_tv_gdat.addCheckStateListener(new ICheckStateListener()
    {
      ...
      public void checkStateChanged(CheckStateChangedEvent event)
      {
        ...
        if (event.getChecked() == true)
        {
          ...
          {
            // ObservableNode
            data.addCheckedObservable((ObservableNode) event.getElement());
            setStatus("Checked:" + node.getName());
          }
        ...
        else if (event.getChecked() == false)
        {
          ...
          {
            // ObservableNode
            data.removeCheckedObservable((ObservableNode) event.getElement());
            setStatus("Unchecked:" + node.getName());
          }
          else if (node.getNodeType().equalsIgnoreCase("SpeciesFolderNode"))
          {
    ...
    element_tv_gdat.addSelectionChangedListener(new ISelectionChangedListener()
    {
      ...
      public void selectionChanged(SelectionChangedEvent event)
      {
        ...
        if (node.getNodeType().equalsIgnoreCase("ObservableNode"))
        {
          ...
          if (!labelName.equals(data.getSelectedObservableName()))
          {
            // store the name of the selected species
  ...
  private void createCompareOutline()
  {
    ...
    for (int i = 0; i < 2; i++)
    {
      ...
      // select all button
      final Button selectAllBnt = new Button(outlineCmp_compare, SWT.CHECK);
      selectAllBnt.setText("Check/Uncheck All");
      gridData = new GridData();
        ...
        public void widgetSelected(SelectionEvent arg0)
        {
          ...
          if (selected == true)
          {
            ...
            for (int i = 0; i < elements.length; i++)
            {
              ...
              if (node.getNodeType().equalsIgnoreCase("SpeciesNode"))
              {
                data.addCheckedSpecies((Integer) cur_compare.getData("index"),
                    (SpeciesNode) elements[i]);
              }
            }
            // set allChecked be true
            data.setAllChecked((Integer) cur_compare.getData("index"), true);
            // set some nodes be checked
            cur_compare.setCheckedElements(data
                .getCheckedElements((Integer) cur_compare.getData("index")));
            // plot the chart
            curChartPanel.setChart(DATComparisonChart.plotChart(data,
                chartTypeList));
            curChartPanel.setChart(DATComparisonChart.plotChart(
          ...
          else
          {
            ...
            // set allChecked be false
            data.setAllChecked((Integer) cur_compare.getData("index"), false);
            // set some nodes be checked
            cur_compare.setCheckedElements(data
                .getCheckedElements((Integer) cur_compare.getData("index")));
            // plot the chart
      ...
      cur_compare.getTree().addMouseListener(new MouseListener()
      {
        ...
        public void mouseDown(MouseEvent event)
        {
          ...
          if (item == null)
          {
            ...
            cur_compare.setSelection(null);
            DATComparisonChart.resetChart(curChartPanel.getChart(), cur_index); // reset
                                                                                // chart
          }
        }
      ...
      cur_compare.addCheckStateListener(new ICheckStateListener()
      {
        ...
        public void checkStateChanged(CheckStateChangedEvent event)
        {
          ...
          if (data.getSelectedSpeciesName(cur_index) != null)
          {
            ...
            if (key != null)
            {
              DATComparisonChart.markSelectedLine(curChartPanel.getChart(),
                  cur_index, key);

              cur_compare.setSelection(
                  new StructuredSelection(findSpeciesNode(cur_compare,
                      labelName)), true);
      ...
      cur_compare.addSelectionChangedListener(new ISelectionChangedListener()
      {
        ...
        public void selectionChanged(SelectionChangedEvent event)
        {
          ...
          if (node.getNodeType().equalsIgnoreCase("ObservableNode"))
          {
            ...
            DATComparisonChart.markSelectedLine(curChartPanel.getChart(),
                cur_index, labelName);
            setStatus("Selected: " + data.getFileName(cur_index) + " "
                + labelName);
          }
          else
          {
            ...
            if (data.getFileName(cur_index).endsWith(".cdat"))
            {
              // CDAT
  ...
  private void initExplorerMenu()
  {
    ...
    compareCurMenuItem.addSelectionListener(new SelectionAdapter()
    {
      public void widgetSelected(SelectionEvent event)
      {
        ...
        if (curFileName.contains("Compare")
            || !(curFileName.endsWith(".cdat") || curFileName.endsWith(".gdat") || curFileName
                .endsWith(".scan")))
        {
        ...
        String type2 = node.getName()
            .substring(node.getName().indexOf(".") + 1);

        // get DATFileData
        DATFileData data1 = (DATFileData) curFileNode.getFileData();
        DATFileData data2 = (DATFileData) ((FileNode) node).getFileData();

        // create a compare data object
        DATComparisonData compData = new DATComparisonData(type1, data1, type2,
            data2);
    ...
    compareTwoMenuItem.addSelectionListener(new SelectionAdapter()
    {
      public void widgetSelected(SelectionEvent event)
      {
        ...
        // get DATFileData
        DATFileData data1 = (DATFileData) ((FileNode) node1).getFileData();
        DATFileData data2 = (DATFileData) ((FileNode) node2).getFileData();
        ...
        sash_form_top.setMaximizedControl(null);
        // create a new compare tab item
        createComparisonItem((FileNode) node1, (FileNode) node2, compData);
    ...
    deleteMenuItem.addSelectionListener(new SelectionAdapter()
    {
      ...
      public void widgetSelected(SelectionEvent event)
      {
        ...
        MessageBox deleteBox = new MessageBox(getShell(), SWT.ICON_WARNING
            | SWT.OK | SWT.CANCEL);
  ...
  private void initTextMenu()
  {
    ...
    speciesIDItem.addSelectionListener(new SelectionAdapter()
    {
      public void widgetSelected(SelectionEvent arg0)
      {
        ...
        {
          idStr += Character.toString(data.getFileContent().charAt(i));
        }
    ...
    speciesExpItem.addSelectionListener(new SelectionAdapter()
    {
      ...
      public void widgetSelected(SelectionEvent arg0)
      {
        ...
        for (int i = p.x; i < p.x + p.y; i++)
        {
          expression += Character.toString(data.getFileContent().charAt(i));
        }
        ...
        else
        {
          ...
          String item = "species\t" + id.toString() + " " + expression;
          itemList.add(item);
          showGraph(itemList);
        }
      }
  ...
  private void createChartItem(FileNode fNode)
  {
    ...
    JFreeChart chart = DATChart.plotChart((DATFileData) fNode.getFileData(),
    ...
    if (fNode.getName().endsWith(".gdat") || fNode.getName().endsWith(".scan"))
    {
      ...
      public void widgetDisposed(DisposeEvent event)
      {
        ...
        if (curFileList.size() == 0)
        {
          ...
          if (elementOutline != null && elementOutline.isDisposed() == false)
          {
            ...
            // maximize explorerFolder
            sash_form_top.setMaximizedControl(sash_form_top.getChildren()[0]);
          }
        }
      }
  ...
  private void createLOGItem(FileNode fNode)
  {
    ...
    // use SourceViewer to control the NET file
    final SourceViewer sv = new SourceViewer(textFolder, null, SWT.READ_ONLY
      ...
      DATComparisonData data)
  {
    ...
    xAxisBnt.addSelectionListener(new SelectionAdapter()
    {
      public void widgetSelected(SelectionEvent e)
      {
        ...
        DATComparisonData data = (DATComparisonData) curFileNode.getFileData(); // plot
    ...
    for (int i = 0; i < data.getDATDataCount(); i++)
    {
      ...
      ((Button) outlineCmp_compare.getChildren()[2 + 5 * i]).setSelection(true);
  ...
  private void updateOutline()
  {
    ...
    if (curFileNode.getName().contains("Compare"))
    {
      ...
      for (int i = 0; i < element_tv_compare.size(); i++)
      {
        ...
        {
          String selectedObservableName = data.getSelectedObservableName(i);
          if (selectedObservableName != null)
          {
            cur_compare.setSelection(new StructuredSelection(
                findObservableNode(cur_compare, selectedObservableName)), true);
          }
          else
          {
            cur_compare.setSelection(null);
      ...
      try
      {
        SpeciesGraph sg = new SpeciesGraph(id, expression,
            VisualizationViewerController.loadVisualizationViewController()
                .getSpeciesBrowserSize(), VisualizationViewerController
  ...
  public void openDefaultFile(String expandFilePath, String expandFileType)
  {
    ...
    file_tv.expandAll();
    Object[] firstLevel = ((TreeNode) file_tv.getInput()).getChildrenArray();
    for (int i = 0; i < firstLevel.length; i++)
    {
      ...
      if (curFirst.getName().equals(modelName))
      {
        ...
        for (int j = 0; j < secondLevel.length; j++)
        {
          ...
          {
            // find time label
            Object[] thirdLevel = ((TreeNode) curSecond).getChildrenArray();
            ...
            for (int k = 0; k < thirdLevel.length; k++)
            {
              ...
              if (expandFileType.equals(".gdat")
                  && curThird.getName().endsWith(expandFileType))
              {
                found = true;
              }
              ...
              if (found == true)
              {
                ...
                // set selection
                file_tv.setSelection(new StructuredSelection(curThird), true);

                // set status line
                setStatus("Opened file " + curFileNode.getName());
