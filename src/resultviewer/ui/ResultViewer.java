...
public class ResultViewer extends ApplicationWindow {
	...
	private void initExplorerFolder() {
		...
		file_tv.addSelectionChangedListener(new ISelectionChangedListener() {
			...
			public void selectionChanged(SelectionChangedEvent event) {
				...
				setStatus("Selected: " + ((TreeNode) selection.getFirstElement()).getName());
				...
				if (curFileNode.getName().endsWith(".net")) {
					...
					for (int i = 0; i < textFolder.getItemCount(); i++) {
						...
						if (item.getText().equals(curFileNode.getName())) {
							// set selection on this matched item
							textFolder.setSelection(item);
							curChartPanel = (ChartComposite) ((Composite)item.getControl()).getChildren()[2];
	...
	private void initTextFolder() {
		...
		textFolder.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent event) {
				...
				if (fileName.endsWith(".cdat") || fileName.endsWith(".gdat") || fileName.endsWith(".scan")) {
					// CDAT & GDAT & SCAN
					curChartPanel = (ChartComposite) ((Composite)item.getControl()).getChildren()[2];
	...
	private void initOutlineFolder() {
		...
		selectAllBnt.setText("Select/Deselect All");

		// element tree viewer
		element_tv = new CheckboxTreeViewer(outlineCmp, SWT.NONE);
		...
		selectAllBnt.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent arg0) {
				...
				if (selected == true) {
					...
					if (curFileNode.getName().endsWith(".cdat")) {
					...
					} else if (curFileNode.getName().endsWith(".gdat") || curFileNode.getName().endsWith(".scan")) {
						// GDAT & SCAN
						DATFileData data = (DATFileData) curFileNode
								.getFileData();
						...
						for (int i = 0; i < elements.length; i++) {
							...
							if (node.getNodeType().equalsIgnoreCase(
									"ObservableNode")) {
								data
										.addCheckedObservable((ObservableNode) elements[i]);
							}
						}
						// set some nodes be checked
					...
					if (curFileNode.getName().endsWith(".cdat")) {
						...
						for (int i = 0; i < elements.length; i++) {
							data
									.removeCheckedSpecies((SpeciesNode) elements[i]);
						}
						// set some nodes be checked
						element_tv.setCheckedElements(data.getCheckedSpecies());
						curChartPanel.setChart(DATChart.plotXYLineChart(data, dataType));
						...
						for (int i = 0; i < elements.length; i++) {
							...
							if (node.getNodeType().equalsIgnoreCase(
									"ObservableNode")) {
								data
										.removeCheckedObservable((ObservableNode) elements[i]);
							}
						}
						// set some nodes be checked
						element_tv.setCheckedElements(data
			...
			public void checkStateChanged(CheckStateChangedEvent event) {
				...
				if (event.getChecked() == true) {
					...
					if (node.getNodeType().equalsIgnoreCase("SpeciesNode")) {
						...
						data
								.addCheckedSpecies((SpeciesNode) event
										.getElement());
						curChartPanel.setChart(DATChart.plotXYLineChart(data, dataType));
	...
	/*
	 * Create a menu to control tab items on textFolder.
	 *  - Close All
	 */
	...
	/*
	 * Create a menu to control text for NET file.
	 * Show graph based on selected Text.
	 *  - Species ID
	 *  - Species Expression
	 */
	...
	private void initTextMenu() {
		...
		speciesIDItem.addSelectionListener(new SelectionListener(){
			...
			public void widgetSelected(SelectionEvent arg0) {
				...
				try {
					...
					if (exp == null) {
						setStatus("\"" + idStr +"\"" + " has no correspoding species.");
						return;
					}
					// show the graph
					...
					setStatus("\"" + idStr +"\"" + " is not an Integer number.");
				}
		...
		speciesExpItem.addSelectionListener(new SelectionListener(){
			...
			public void widgetSelected(SelectionEvent arg0) {
				...
				if (id == null) {
					setStatus("\"" + expression +"\"" + " is not a species expression");
					return;
				}
	...
	private void createChartItem(FileNode fNode) {
		...
		JFreeChart chart = DATChart.plotXYLineChart((DATFileData) fNode
				.getFileData(), dataType);
		...
		/*
		 * Set initial status be selecting all the nodes
		 */
		selectAllBnt.setSelection(true);
		element_tv.setAllChecked(true);
	...
	private void createNetItem(FileNode fNode) {
		...
		sv.addPostSelectionChangedListener(new ISelectionChangedListener() {
			...
			public void selectionChanged(SelectionChangedEvent event) {
				...
				// set background color for all the text which is the same as the selected one
				...
				while(selectOffset != -1) {
					...
					selectOffset = fileContent.substring(subStringOffset).indexOf(selected);
	...
	private void showGraph(FileData data, int id, String speciesExp) {
		...
		if (dis == null
				|| !dis.getName().equals(speciesExp)) {
			dis = new SpeciesGraph(id, speciesExp)
					.getDisplay();
			dis.setName(speciesExp);
			data.setDisplay(dis);
	...
	public static void main(String[] args) {
		ResultViewer w = new ResultViewer();
		w.setBlockOnOpen(true);
		w.open();

		if (w.jf != null && w.jf.isShowing()) {
			w.jf.dispose(); // close the graph
		}
