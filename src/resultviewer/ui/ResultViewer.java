...
public class ResultViewer extends ApplicationWindow {
	...
	private ArrayList<String> chartTypeList; // for compare file
	...
	private void initExplorerFolder() {
		...
		// file_tv.getTree().setLinesVisible(true);
				...
				.addSelectionChangedListener(new ISelectionChangedListener() {
					...
					public void selectionChanged(SelectionChangedEvent event) {
						...
						if (node.getNodeType().equalsIgnoreCase(
							...
							// TODO example code to show multiple species

							/*
							 * if (node.getNodeType().equalsIgnoreCase(
							 * "SpeciesFolderNode")) { // show all the children
							 * Object[] children = node.getChildrenArray(); if
							 * (children != null) { for (int i = 0; i <
							 * children.length; i++) { TreeNode curChild =
							 * (TreeNode)children[i]; String childitem =
							 * "species\t" + curChild.getName();
			...
			cur_compare.getTree().addMouseListener(new MouseListener() {
				...
				public void mouseDown(MouseEvent event) {
					...
					if (item == null) {
						...
						DATComparisonData data = (DATComparisonData) curFileNode
								.getFileData();
						data.setSelectedObservableName(cur_index, null);
						data.setSelectedSpeciesName(cur_index, null);
						*/
						cur_compare.setSelection(null);
...
//		jscrollP.setVerticalScrollBarPolicy(JScrollPane.VERTICAL_SCROLLBAR_NEVER);
	...
	private boolean addItemToGraph(String type, String itemExp) {
		if (type.equals("species") || type.equals("pattern")) {
			...
			/*
			JTextArea label = new JTextArea(type + ": "
			...
			*/
			...
			try {
				...
				Dimension dim = jf.getPreferredSize();
				dim.height -= label.getSize().height;
				dis.setSize(dim);
				dis.pan(40, 40);
				panel.add(dis);
			} catch (Exception e) {
