...
public class DATFileData extends FileData{
	...
	public XYSeriesCollection getSeriesCollection() {
		...
		} else if (this.fileName.endsWith(".gdat") || this.fileName.endsWith(".scan")) {
			//GDAT & SCAN
			for (int i = 0; i < seriesList.size(); i++) {
	...
	/*
	 * Return the data set in log scale to draw a chart
	 * SCAN file only
	 */
	...
	protected void readData(File file) {
		...
		try {
			...
			while (in.hasNext()) {
				...
				else logTime= Math.log(time) / Math.log(10); // log time
				...
				while (!line.equals("")) {
					...
					while (!line.equals("")) {
						...
						// log scale
						XYDataItem logitem = new XYDataItem(logTime, concentration);
						logSeriesList.get(count).add(logitem);
	 ...
	 */
	private void readSpeciesFromNETFile(File file) {
		String filePath = file.getAbsolutePath();
		String fileName = file.getName();
		String fileType = fileName.substring(fileName.lastIndexOf("."), fileName.length());
		String netFilePath = filePath.substring(0, filePath.indexOf(fileType)) + ".net";;
		...
		if (fileType.equals(".scan")) {
			...
			netFilePath = file.getParent() + slash + prefixName + slash + prefixName + ".net";
		}
	...
	private void readObservableFromNETFile(File file) {
		...
		try {
					...
					while(!line.equalsIgnoreCase("end groups")) {
						...
						for (int i = 0; i < tmp2.length; i++) {
							...
							if (speciesID.contains("*")) {
								speciesID = speciesID.substring(speciesID.indexOf("*")+1, speciesID.length());
							}
			...
			observableFolder = new ObservableFolderNode("Observables", info, componentsList);
			in.close();
		} catch (FileNotFoundException e) {
			e.printStackTrace();
		}
