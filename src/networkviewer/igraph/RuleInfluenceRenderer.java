 ...
 */
public class RuleInfluenceRenderer extends AbstractShapeRenderer
{
   ...
   */
  public RuleInfluenceRenderer(int edgeType)
  {
   ...
   */
  public RuleInfluenceRenderer(int edgeType, int arrowType)
  {
    this();
  ...
  protected Shape getRawShape(VisualItem item)
  {
    ...
    /*
     * //set end point based on edge influence type Integer activate =
     * (Integer)edge.get("activation"); Integer inhibit =
     * (Integer)edge.get("inhibition");
     * 
     * if (activate == 1 || activate == 0) { m_xAlign2 = Constants.LEFT;
     * m_yAlign2 = Constants.LEFT; }
     * 
     * if (inhibit == 1 || inhibit == 0) { m_xAlign2 = Constants.RIGHT;
     * m_yAlign2 = Constants.RIGHT; }
     */
  ...
  public boolean locatePoint(Point2D p, VisualItem item)
  {
    ...
    else
    {
      ...
      return s.intersects(p.getX() - halfWidth, p.getY() - halfWidth, width,
          width);
    }
  }
   ...
   * determination, however, the preferred method is to change the item size
   * value itself.
   * 
   * @param item
   ...
   * by the current line width determined by the
      ...
      double y1, double x2, double y2)
  {
    ...
    // offset
    double offset = Math.abs(dx) > Math.abs(dy) ? Math.abs(dx) : Math.abs(dy);

    // dynamically assign curve height
    if (offset >= 50 && offset < 100)
    {
      offset *= 0.55;
    }
    else if (offset >= 100 && offset < 900)
    {
      offset *= 0.45;
    }
    else if (offset >= 900 && offset < 2000)
    {
      offset *= 0.35;
    }
    else if (offset >= 2000)
    ...
    // activation edge
    if ((Integer) eitem.get("activation") != -1)
    {
      offset *= 1.2;
    }
    ...
    if (Math.abs(dy) < 0.1)
    {
      ...
      cp[0].setLocation(centerx, centery + sign * offset);
      cp[1].setLocation(centerx, centery + sign * offset);

      return;
    }
    // same x OR both x and y are the same
    if (Math.abs(dx) < 0.1)
    {
      sign = 1;
      // determine the direction of the arc
      ...
      cp[0].setLocation(centerx + offset, sign * centery);
      cp[1].setLocation(centerx + offset, sign * centery);
      return;
    }
    ...
    // compute position
    x = centerx + sign_x * offset * Math.cos(angle);
    y = centery + sign_y * offset * Math.sin(angle);
  ...
  public void setArrowType(int type)
  {
    if (type < 0 || type >= Constants.EDGE_ARROW_COUNT)
      throw new IllegalArgumentException("Unrecognized edge arrow type: "
          + type);
    m_edgeArrow = type;
  }
