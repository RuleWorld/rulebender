...
public class SetBNGPathDialogue
{
  ...
  public SetBNGPathDialogue(final Shell mainEditorShell, BNGEditor editorIn)
  {
    ...
    // Create the shell
    setBNGPathDialogue = new Shell(parentShell, SWT.DIALOG_TRIM
        ...
        && ConfigurationManager.getConfigurationManager().getBNGFName() != null)
    {
      ...
      String path = ConfigurationManager.getConfigurationManager()
          .getBNGFPath();
    ...
    okButton.addSelectionListener(new SelectionListener()
    {
      ...
      public void widgetSelected(SelectionEvent arg0)
      {
        ...
        while (completedPath.substring(completedPath.length() - 1,
            completedPath.length()).equals("\"")
            || completedPath.substring(completedPath.length() - 1,
                completedPath.length()).equals("/"))
        {
        ...
        {
          MessageBox mb = new MessageBox(setBNGPathDialogue,
        ...
        else
        {
          if (ConfigurationManager.getConfigurationManager().getOSType() == 1)
          {
            ConfigurationManager.getConfigurationManager().setBNGFPath(
                completedPath.substring(0, completedPath.lastIndexOf('\\')));
            ConfigurationManager.getConfigurationManager().setBNGFName(
                completedPath.substring(completedPath.lastIndexOf('\\') + 1,
