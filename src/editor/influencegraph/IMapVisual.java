...
import networkviewer.CustomizedZoomToFitControl;
import networkviewer.FitOverviewListener;
import networkviewer.cmap.ReverseWheelZoomControl;
import networkviewer.igraph.IMAPLayout;
import networkviewer.igraph.RuleInfluenceRenderer;
...
public class IMapVisual
{
    ...
    for (int i = 0; i < model.getInfluences().size(); i++)
    {
      ...
      if (model.getInfluences().get(i).getActivation() != -1)
      {
        Edge e_a = iGraph.addEdge(
            nodeList.get(model.getInfluences().get(i).getStartrulenodeindex()),
            nodeList.get(model.getInfluences().get(i).getEndrulenodeindex()));
  ...
  private String parseTwoDirectionRule(String ruleText, int forward)
  {
    ...
    else if (forward == 0)
    {
      ...
      if (ruleText.indexOf(">") != -1)
      {
        ...
        {
          secondrate = res.substring(res.lastIndexOf(",") + 1, res.length())
              .trim();
          res = res.substring(0, res.lastIndexOf(",")).trim();
          if (res.lastIndexOf(" ") != -1)
          {
      ...
      if (ruleText.indexOf("<") != -1)
      {
        res += ruleText.substring(0, ruleText.indexOf("<")).trim();
      }
      // add second rate
  ...
  private void setUpActions()
  {
    ...
    ColorAction fill = new ColorAction("igraph.nodes", VisualItem.FILLCOLOR,
        ColorLib.rgb(225, 225, 225));

    // ColorActions color the given group a certain color.
    ColorAction text = new ColorAction("igraph.nodes", VisualItem.TEXTCOLOR,
        ColorLib.rgb(0, 0, 0));
    ...
    Predicate activate1 = (Predicate) ExpressionParser.parse("activation == 1");
    edgeStroke.add(activate1, ColorLib.rgba(77, 146, 33, alpha));
    Predicate activate0 = (Predicate) ExpressionParser.parse("activation == 0");
    edgeStroke.add(activate0, ColorLib.rgba(77, 146, 33, alpha));
    Predicate inhibit1 = (Predicate) ExpressionParser.parse("inhibition == 1");
    edgeStroke.add(inhibit1, ColorLib.rgba(197, 27, 125, alpha));
    Predicate inhibitn0 = (Predicate) ExpressionParser.parse("inhibition == 0");
  ...
  public void setUpMainDisplay(Dimension mainDisplaySize_in)
  {
    ...
    mainDisplay.addControlListener(new InfluenceGraphClickControlDelegate(vis));
