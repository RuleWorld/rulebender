...
public class IMapModel
{
    ...
    while (scan.hasNext())
    {
      ...
      while (TBProcessed.charAt(TBProcessed.length() - 1) == '\\')
      {
        ...
        if (TBProcessed.indexOf('#') >= 0)
          TBProcessed = TBProcessed.substring(0, TBProcessed.indexOf('#'))
              .trim();
      }
      if (TBProcessed.indexOf(' ') != -1)
      {
        ...
        try
        {
          ...
          parsemolecule(TBProcessed.substring(TBProcessed.indexOf(' ') + 1,
              TBProcessed.length()).trim());
        }
  ...
  void parsespecies(String molestr)
  {
    ...
    while (scan.hasNext())
    {
      ...
      if (TBProcessed.indexOf(' ') != -1)
      {
        ...
        {
          Integer.parseInt(tempstr1);
          TBProcessed = TBProcessed.substring(TBProcessed.indexOf(' ') + 1,
              TBProcessed.length()).trim();
        }
        catch (NumberFormatException e)
        {
      ...
      {
        tempstr1 = TBProcessed.substring(TBProcessed.lastIndexOf(' ') + 1,
            TBProcessed.length()).trim();
        if (tempstr1.indexOf('(') == -1 && tempstr1.indexOf(')') == -1
            ...
            && tempstr1.indexOf('?') == -1 && tempstr1.indexOf('~') == -1
            && tempstr1.indexOf('{') == -1 && tempstr1.indexOf('}') == -1)
          TBProcessed = TBProcessed.substring(0, TBProcessed.lastIndexOf(' '))
              .trim();
        else
          break;
      }
  ...
  void parsemolecule(String molecule)
  {
    ...
    {
      parsecomponent(tempmole, component.substring(0, component.indexOf(','))
          .trim());
  ...
  boolean validatemole(IMolecule tempmole)
  {
    ...
    // names
    {
      ...
      else
      {
        for (int j = 0; j < tempmole.getComponents().get(i).getStates().size(); j++)
          if (tempmole.getComponents().get(i).getStates().get(j).getName()
              .trim().length() == 0)
          {
    ...
    for (int i = 0; i < tempmole.getComponents().size(); i++)
      if (!validatename(tempmole.getComponents().get(i).getName()))
        ...
        for (int j = 0; j < tempmole.getComponents().get(i).getStates().size(); j++)
          if (!validatename(tempmole.getComponents().get(i).getStates().get(j)
              .getName()))
            return false;
    return true;
  }
    ...
    while (scan.hasNext())
    {
      ...
      while (m.find())
      {
        TBProcessed = TBProcessed.substring(0, m.start()).trim() + " "
            + TBProcessed.substring(m.end(), TBProcessed.length()).trim();
  ...
  void parserule(IRule temprule, String rulestr)
  {
    ...
    if (colonIndex >= 0 && ((cabbageIndex < 0) || (colonIndex < cabbageIndex)))
    {
    ...
    if (rulestr.indexOf('<') != -1)
    {
      ...
      while (rulestr.lastIndexOf(' ') != -1)
      {
        ...
        // contain the rates.
        tempstr1 = rulestr
            .substring(rulestr.lastIndexOf(' '), rulestr.length()).trim();
  ...
  void parserule2pattern(IRule temprule, String rulestr)
  {
    ...
    products = rulestr.substring(rulestr.indexOf('>') + 1, rulestr.length())
        .trim();
    ...
    if (ruleparsevalid)
    {
      for (int i = 0; i < temprule.getReactantpatterns().size(); i++)
        for (int j = 0; j < temprule.getReactantpatterns().get(i).getBonds()
            .size(); j++)
          add2bondactions(temprule.getBondactions(), temprule
              .getReactantpatterns().get(i).getBonds().get(j), false);
      for (int i = 0; i < temprule.getProductpatterns().size(); i++)
        for (int j = 0; j < temprule.getProductpatterns().get(i).getBonds()
            .size(); j++)
          add2bondactions(temprule.getBondactions(), temprule
              .getProductpatterns().get(i).getBonds().get(j), true);
  ...
  void parsepatterns(ArrayList<IRulePattern> patternlist, String patternstr)
  {
    ...
    {
      parsemolepattern(temprp, patternstr.substring(0, patternstr.indexOf('.'))
          .trim());
        ...
        if (pbonds.get(i).getSites().size() == 2)
        {
          ...
          for (int j = 0; j < temprp.getMolepatterns().size(); j++)
            for (int k = 0; k < temprp.getMolepatterns().get(j)
              ...
              if (temprp.getMolepatterns().get(j).getComppatterns().get(k)
                  .getPbondlist().contains(pbonds.get(i).getName()))
              {
                temprp.getMolepatterns().get(j).getComppatterns().get(k)
                    .getBondlist().add(bondindex);
              }
        }
    }
    /*
     * for(int j = 0; j<temprp.getMolepatterns().size(); j++) for(int k=0;
     * k<temprp.getMolepatterns().get(j).getComppatterns().size(); k++) temprp
  ...
  void parsemolepattern(IRulePattern temprp, String molestr)
  {
    ...
    {
      molename = molestr.substring(0, molestr.indexOf('(')).trim();
      component = molestr.substring(molestr.indexOf('(') + 1, molestr.length())
          .trim();
    }
    if (!leftparenthis && !rightparenthis)
    {
    ...
    while (component.indexOf(',') != -1)
    {
      parsecomppattern(tempmp, component.substring(0, component.indexOf(','))
          .trim());
      component = component.substring(component.indexOf(',') + 1,
          component.length()).trim();
    }
  ...
  void parsecomppattern(IMoleculePattern tempmp, String compstr)
  {
    ...
    {
      statestr = compstr.substring(compstr.indexOf('~') + 1, compstr.length())
          .trim();
    ...
    if (compstr.indexOf('!') != -1)
    {
      compbond = compstr.substring(compstr.indexOf('!') + 1, compstr.length())
          .trim();
      compstr = compstr.substring(0, compstr.indexOf('!')).trim();
      if (compstr.indexOf('%') != -1)
        compstr = compstr.substring(0, compstr.indexOf('%')).trim();
    ...
    {
      BNGEditor.displayOutput("\nError! Molecule: "
          + molecules.get(tempmp.getMoleindex()).getName() + ", Component: "
          + compstr + " NOT declared!");
      ruleparsevalid = false;
      return;
    }
      ...
      if (tempcp.getStateindex() == -1 && !statestr.equals("*"))
      {
        if (!flexiblestate)
        {
          BNGEditor.displayOutput("\nError! Molecule: "
              ...
              + ", Component: " + compstr + ", State: " + statestr
              + " NOT declared!");
          ruleparsevalid = false;
          return;
        }
    ...
    if (compbond.length() != 0)
    {
      while (compbond.indexOf('!') != -1)
      {
        ...
        else
        {
          add2pbonds(
              tempstr,
              new ISite(tempmp.getMoleindex(), tempcp.getCompindex(), tempcp
                  .getStateindex()));
    ...
    for (int i = 0; i < temprule.getReactantpatterns().size(); i++)
          ...
          .getMolepatterns().size(); j++)
      {
        ...
        for (int k = 0; k < temprule.getReactantpatterns().get(i)
          ...
          temprule.getReactantpatterns().get(i).getMolepatterns().get(j)
              .getComppatterns().get(k).setMatched(false);
      }
    for (int i = 0; i < temprule.getProductpatterns().size(); i++)
      for (int j = 0; j < temprule.getProductpatterns().get(i)
          .getMolepatterns().size(); j++)
      {
        productmoles.add(temprule.getProductpatterns().get(i).getMolepatterns()
            .get(j));
        for (int k = 0; k < temprule.getProductpatterns().get(i)
            .getMolepatterns().get(j).getComppatterns().size(); k++)
          temprule.getProductpatterns().get(i).getMolepatterns().get(j)
              .getComppatterns().get(k).setMatched(false);
      }
      ...
      for (int i = 0; i < in1.getComppatterns().size(); i++)
        for (int j = 0; j < in2.getComppatterns().size(); j++)
              ...
              && !in2.getComppatterns().get(j).isMatched())
          {
            ...
            if (!comparecomppattern(in1.getComppatterns().get(i), in2
                .getComppatterns().get(j)))
            {
  ...
  void generateImap()
  {
    ...
    for (int i = 0; i < rules.size(); i++)
      ...
      {
        rulenodes.add(new IRuleNode(rulenodeindex, i, true, rules.get(i)
            .getName(), rules.get(i).getLabel()));
        rulenodeindex++;
        rulenodes.add(new IRuleNode(rulenodeindex, i, false, rules.get(i)
            .getName(), rules.get(i).getLabel()));
        rulenodeindex++;
      }
      else
  ...
  void determineinfluence(Influence tempinfluence, IRuleNode rn1, IRuleNode rn2)
  {
    ...
    if (rn2.isForward() && rn1.isForward())
    {
      ...
      // half of reaction1
      for (int i = 0; i < rules.get(rn2.getIruleindex()).getReactantpatterns()
          .size()
          && !canbreak; i++)
        for (int j = 0; j < rules.get(rn1.getIruleindex())
          ...
          switch (patternmatching(rules.get(rn2.getIruleindex())
              .getReactantpatterns().get(i), rules.get(rn1.getIruleindex())
      ...
      for (int i = 0; i < rules.get(rn2.getIruleindex()).getReactantpatterns()
          .size()
          && !canbreak; i++)
        for (int j = 0; j < rules.get(rn1.getIruleindex()).getProductpatterns()
            .size()
            && !canbreak; j++)
          switch (patternmatching(rules.get(rn2.getIruleindex())
              .getReactantpatterns().get(i), rules.get(rn1.getIruleindex())
              .getProductpatterns().get(j)))
          {
          // partial match and r1 reactioncenter
            case 0:
              // possible activation
    ...
    {
      canbreak = false;
      for (int i = 0; i < rules.get(rn2.getIruleindex()).getProductpatterns()
          .size()
          && !canbreak; i++)
        for (int j = 0; j < rules.get(rn1.getIruleindex())
            .getReactantpatterns().size()
            && !canbreak; j++)
          switch (patternmatching(rules.get(rn2.getIruleindex())
              .getProductpatterns().get(i), rules.get(rn1.getIruleindex())
              .getReactantpatterns().get(j)))
          {
            case 0:
              // possible inhibition
          ...
          for (int k = 0; k < tempmp1.getComppatterns().size(); k++)
          {
            ...
            for (int l = 0; l < tempmp2.getComppatterns().size(); l++)
              ...
              if (tempmp1.getComppatterns().get(k).getCompindex() == tempmp2
                  ...
                  && !tempmp2.getComppatterns().get(l).isMatched()
                  && !tempmp1.getComppatterns().get(k).isMatched())
              {
                tempmp2.getComppatterns().get(l).setMatched(true);
                tempmp1.getComppatterns().get(k).setMatched(true);
                Collections.sort(tempmp1.getComppatterns().get(k)
                    .getPbondlist());
                Collections.sort(tempmp2.getComppatterns().get(l)
                    .getPbondlist());

                // different states or different bond status
                if ((tempmp1.getComppatterns().get(k).getStateindex() != tempmp2
                    .getComppatterns().get(l).getStateindex())
                    || !samebondstatus(tempmp1.getComppatterns().get(k),
                ...
                if (canmatch
                    && tempmp2.getComppatterns().get(l).isReactioncenter())
                  reactioncenter = true;
              }
            if (canmatch)
              // match or conflict
