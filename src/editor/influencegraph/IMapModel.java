...
public class IMapModel {
		...
		while (scan.hasNext()) {
			...
			if (TBProcessed.indexOf('#') != -1)
				TBProcessed = TBProcessed
						.substring(0, TBProcessed.indexOf('#')).trim();
			while (TBProcessed.charAt(TBProcessed.length() - 1) == '\\') {
				if (scan.hasNext())
					TBProcessed = TBProcessed.substring(0,
							TBProcessed.length() - 1)
							+ ' '
							+ scan.nextLine().trim();
				else
					TBProcessed = TBProcessed.substring(0,
				...
				if (TBProcessed.indexOf('#') >= 0)
					TBProcessed = TBProcessed.substring(0,
							TBProcessed.indexOf('#')).trim();
			}
			if (TBProcessed.indexOf(' ') != -1) {
				tempstr1 = TBProcessed.substring(0, TBProcessed.indexOf(' '))
						.trim();
				try {
					Integer.parseInt(tempstr1);
					parsemolecule(TBProcessed.substring(
							TBProcessed.indexOf(' ') + 1, TBProcessed.length())
							.trim());
	...
	void parsespecies(String molestr) {
		...
		while (scan.hasNext()) {
			...
			if (TBProcessed.indexOf(' ') != -1) {
				...
				try {
					Integer.parseInt(tempstr1);
					TBProcessed = TBProcessed.substring(
							TBProcessed.indexOf(' ') + 1, TBProcessed.length())
							.trim();
				} catch (NumberFormatException e) {
				}
			...
			while (TBProcessed.lastIndexOf(' ') != -1) {
				tempstr1 = TBProcessed.substring(
						TBProcessed.lastIndexOf(' ') + 1, TBProcessed.length())
						.trim();
	...
	void parsemolecule(String molecule) {
		...
		if (leftparenthis && rightparenthis) {
			tempmole.setName(molecule.substring(0, molecule.indexOf('('))
					.trim());
			component = molecule.substring(molecule.indexOf('(') + 1,
					molecule.indexOf(')')).trim();
		}
		...
		if (!leftparenthis && rightparenthis) {
			BNGEditor.displayOutput("\nError parsing molecule : " + molecule
					+ " , molecule not added !");
			return;
		}

		while (component.indexOf(',') != -1) {
			parsecomponent(tempmole,
					component.substring(0, component.indexOf(',')).trim());
			component = component.substring(component.indexOf(',') + 1,
					component.length()).trim();
		}
	...
	void parsecomponent(IMolecule tempmole, String component) {
		...
		if (component.indexOf('~') == -1) {
			...
			state = component.substring(component.indexOf('~') + 1,
					component.length()).trim();
			component = component.substring(0, component.indexOf('~'));
			if (component.indexOf('!') != -1)
				component = component.substring(0, component.indexOf('!'))
	...
	boolean validatemole(IMolecule tempmole) {
		for (int i = 0; i < tempmole.getComponents().size(); i++)// delete the
																	// entries
																	// with
																	// empty
																	// string
																	// names
		{
			if (tempmole.getComponents().get(i).getName().trim().length() == 0) {
				...
				for (int j = 0; j < tempmole.getComponents().get(i).getStates()
						.size(); j++)
					if (tempmole.getComponents().get(i).getStates().get(j)
		...
		for (int i = 0; i < tempmole.getComponents().size(); i++)
			if (!validatename(tempmole.getComponents().get(i).getName()))
				...
				for (int j = 0; j < tempmole.getComponents().get(i).getStates()
						.size(); j++)
					if (!validatename(tempmole.getComponents().get(i)
							.getStates().get(j).getName()))
						return false;
		return true;
	}
		...
		while (scan.hasNext()) {
			...
			if (TBProcessed.indexOf(' ') != -1) {
				...
				tempstr1 = TBProcessed.substring(0, TBProcessed.indexOf(' '))
						.trim();
				try {
					// Get an integer from the temp string ... but do nothing
					// with it. I think
			...
			while (m.find()) {
				TBProcessed = TBProcessed.substring(0, m.start()).trim()
						+ " "
						+ TBProcessed.substring(m.end(), TBProcessed.length())
								.trim();
			...
			if (TBProcessed.contains("include_reactants"))
				TBProcessed = TBProcessed.substring(0,
						TBProcessed.indexOf("include_reactants")).trim();
			if (TBProcessed.contains("exclude_reactants"))
				TBProcessed = TBProcessed.substring(0,
						TBProcessed.indexOf("exclude_reactants")).trim();
			if (TBProcessed.contains("include_products"))
				TBProcessed = TBProcessed.substring(0,
						TBProcessed.indexOf("include_products")).trim();
			if (TBProcessed.contains("exclude_products"))
				TBProcessed = TBProcessed.substring(0,
						TBProcessed.indexOf("exclude_products")).trim();

			parserule(temprule, TBProcessed);
			if (ruleparsevalid)
				rules.add(temprule);
			else
				BNGEditor.displayOutput("\nError parsing rule : " + TBProcessed
						+ " , rule not added !");
		}
	}
	...
	void parserule(IRule temprule, String rulestr) {
		...
		if (colonIndex >= 0
			...
			rulestr = rulestr.substring(rulestr.indexOf(":") + 1,
					rulestr.length()).trim();
		...
		if (rulestr.indexOf('<') != -1) {
			...
			while (rulestr.lastIndexOf(' ') != -1) {
				// Get the substring from the last space to the end. This should
				// contain the rates.
				tempstr1 = rulestr.substring(rulestr.lastIndexOf(' '),
						rulestr.length()).trim();
				...
				if (tempstr1.indexOf('(') == -1 && tempstr1.indexOf(')') == -1
					...
					if (tempstr1.indexOf(',') != -1) {
						// Set a temp string to the substring from after the
						// comma to the end. For reverse rate.
						tempstr2 = tempstr1.substring(
								tempstr1.indexOf(',') + 1, tempstr1.length())
								.trim();

						// Set a temp string to the substring from the beginning
						// to the comma. For forward rate.
						tempstr1 = tempstr1.substring(0, tempstr1.indexOf(','))
								.trim();

						// Validate both of the strings (either empty or not a
						// parsed name so far).
						// If they are valid...
						if (validaterate(tempstr1) && validaterate(tempstr2)) {
							...
							// Set the reverse rate as the forward rate if the
							// reverse rate is empty.
							else
								temprule.setRate2(temprule.getRate1());

							// Set the forward rate
							// TODO Shouldn't this be above the previous if
							// blocks?
							temprule.setRate1(tempstr1);

							// Remove the rates from the rule string.
							rulestr = rulestr.substring(0,
									rulestr.lastIndexOf(' ')).trim();
						}
					...
					// If there was a special character in the rulestring
					// between the last space and the end.
					else {
						// If the first temp string is valid.
						if (validaterate(tempstr1)) {
							// If the forward rate for the temp rule is not null
							// and not empty,
							// then set the reverse rate as the forward rate.
							if (temprule.getRate1() != null
									&& !(temprule.getRate1().length() == 0))
								temprule.setRate2(temprule.getRate1());
			...
			// Send the rule string and the partially completed IRule object to
			// the
	...
	/**
	 * Validates a rate string. The string can be empty, or if the string is not
	 * empty it must not match the name of a molecule or component that has been
	 * previously parsed.
	 * 
	 * @param rate
	 * @return
		...
		for (int i = 0; i < molecules.size(); i++) {
			...
			// If the name of the molecule does not match the rate
				...
				for (int j = 0; j < molecules.get(i).getComponents().size(); j++) {
					...
					// rate is invalid.
					if (molecules.get(i).getComponents().get(j).getName()
							.equals(rate))
						return false;
				}
			}
	...
	void parserule2pattern(IRule temprule, String rulestr) {
		...
		while (m.find()) {
			...
			rulestr = rulestr.substring(0, m.start()).trim() + "!$"
					+ rulestr.substring(m.end(), rulestr.length()).trim();
		...
		while (products.indexOf('+') != -1) {
			...
			parsepatterns(temprule.getProductpatterns(),
					products.substring(0, products.indexOf('+')).trim());

			// Remove from the beginning until after the first '+'.
			products = products.substring(products.indexOf('+') + 1,
					products.length()).trim();
		}
		...
		if (ruleparsevalid) {
			for (int i = 0; i < temprule.getReactantpatterns().size(); i++)
				for (int j = 0; j < temprule.getReactantpatterns().get(i)
						.getBonds().size(); j++)
					add2bondactions(temprule.getBondactions(), temprule
							.getReactantpatterns().get(i).getBonds().get(j),
							false);
			for (int i = 0; i < temprule.getProductpatterns().size(); i++)
				for (int j = 0; j < temprule.getProductpatterns().get(i)
						.getBonds().size(); j++)
					add2bondactions(temprule.getBondactions(), temprule
							.getProductpatterns().get(i).getBonds().get(j),
							true);
			...
			if (temprule.isBidirection())
				for (int i = 0; i < temprule.getBondactions().size(); i++)
					bonds.get(temprule.getBondactions().get(i).getBondindex())
							.setCanGenerate(true);
			else
				for (int i = 0; i < temprule.getBondactions().size(); i++)
					if (temprule.getBondactions().get(i).getAction() > 0)
						bonds.get(
								temprule.getBondactions().get(i).getBondindex())
								.setCanGenerate(true);
		}
	}
	...
	void parsepatterns(ArrayList<IRulePattern> patternlist, String patternstr) {
		...
		while (patternstr.indexOf('.') != -1) {
			parsemolepattern(temprp,
					patternstr.substring(0, patternstr.indexOf('.')).trim());
			patternstr = patternstr.substring(patternstr.indexOf('.') + 1,
					patternstr.length()).trim();
		}
		...
		if (ruleparsevalid) {
			for (int i = 0; i < pbonds.size(); i++)
				...
				if (pbonds.get(i).getSites().size() == 2) {
					tempbond = new IBond();
					tempbond.setMolecule1(pbonds.get(i).getSites().get(0)
							.getMolecule());
					tempbond.setComponent1(pbonds.get(i).getSites().get(0)
							.getComponent());
					tempbond.setState1(pbonds.get(i).getSites().get(0)
							.getState());
					tempbond.setMolecule2(pbonds.get(i).getSites().get(1)
							.getMolecule());
					tempbond.setComponent2(pbonds.get(i).getSites().get(1)
							.getComponent());
					tempbond.setState2(pbonds.get(i).getSites().get(1)
							.getState());
					bondindex = add2bonds(tempbond);
					temprp.getBonds().add(bondindex);
					for (int j = 0; j < temprp.getMolepatterns().size(); j++)
						for (int k = 0; k < temprp.getMolepatterns().get(j)
								.getComppatterns().size(); k++)
							if (temprp.getMolepatterns().get(j)
									.getComppatterns().get(k).getPbondlist()
									.contains(pbonds.get(i).getName())) {
								temprp.getMolepatterns().get(j)
										.getComppatterns().get(k).getBondlist()
										.add(bondindex);
							}
				}
		}
		/*
		 * for(int j = 0; j<temprp.getMolepatterns().size(); j++) for(int k=0;
		 * k<temprp.getMolepatterns().get(j).getComppatterns().size(); k++)
		 * temprp
		 * .getMolepatterns().get(j).getComppatterns().get(k).getPbondlist(
		 * ).clear();
		 */
	...
	boolean equalbond(IBond in1, IBond in2) {
		if (in1.getMolecule1() == in2.getMolecule1()
				&& in1.getMolecule2() == in2.getMolecule2()
				&& in1.getComponent1() == in2.getComponent1()
				&& in1.getComponent2() == in2.getComponent2()
				&& in1.getState1() == in2.getState1()
				&& in1.getState2() == in2.getState2())
			return true;
		if (in1.getMolecule1() == in2.getMolecule2()
				&& in1.getMolecule2() == in2.getMolecule1()
				&& in1.getComponent1() == in2.getComponent2()
				&& in1.getComponent2() == in2.getComponent1()
				&& in1.getState1() == in2.getState2()
				&& in1.getState2() == in2.getState1())
			return true;
		return false;
	}
	...
	void parsemolepattern(IRulePattern temprp, String molestr) {
		...
		if (leftparenthis && rightparenthis) {
			...
			component = molestr.substring(molestr.indexOf('(') + 1,
					molestr.indexOf(')')).trim();
		}
		if (leftparenthis && !rightparenthis) {
			molename = molestr.substring(0, molestr.indexOf('(')).trim();
			component = molestr.substring(molestr.indexOf('(') + 1,
	...
	void parsecomppattern(IMoleculePattern tempmp, String compstr) {
		...
		if (compstr.indexOf('~') != -1) {
			statestr = compstr.substring(compstr.indexOf('~') + 1,
					compstr.length()).trim();
		...
		if (compstr.indexOf('!') != -1) {
			compbond = compstr.substring(compstr.indexOf('!') + 1,
					compstr.length()).trim();
			compstr = compstr.substring(0, compstr.indexOf('!')).trim();
			if (compstr.indexOf('%') != -1)
				compstr = compstr.substring(0, compstr.indexOf('%')).trim();
		...
		if (tempcp.getCompindex() == -1) {
			BNGEditor.displayOutput("\nError! Molecule: "
					+ molecules.get(tempmp.getMoleindex()).getName()
					+ ", Component: " + compstr + " NOT declared!");
			ruleparsevalid = false;
			return;
		}
		if (statestr.length() != 0) {
			if (statestr.indexOf('!') != -1) {
				statebond = statestr.substring(statestr.indexOf('!') + 1,
						statestr.length()).trim();
				statestr = statestr.substring(0, statestr.indexOf('!')).trim();
				if (statestr.indexOf('%') != -1)
					statestr = statestr.substring(0, statestr.indexOf('%'))
							.trim();
				tempcp.setStateindex(getstateindex(tempmp.getMoleindex(),
						tempcp.getCompindex(), statestr));
			} else {
				if (statestr.indexOf('%') != -1)
					statestr = statestr.substring(0, statestr.indexOf('%'))
							.trim();
			...
			if (tempcp.getStateindex() == -1 && !statestr.equals("*")) {
				if (!flexiblestate) {
					BNGEditor.displayOutput("\nError! Molecule: "
							+ molecules.get(tempmp.getMoleindex()).getName()
							+ ", Component: " + compstr + ", State: "
							+ statestr + " NOT declared!");
					ruleparsevalid = false;
					return;
				} else {
					tempcp.setStateindex(molecules.get(tempmp.getMoleindex())
							.getComponents().get(tempcp.getCompindex())
							.getStates().size());
					IState tempstate = new IState();
					tempstate.setName(statestr);
					molecules.get(tempmp.getMoleindex()).getComponents()
							.get(tempcp.getCompindex()).getStates()
							.add(tempstate);
				}
			}
		}
		...
		if (compbond.length() != 0) {
			while (compbond.indexOf('!') != -1) {
				...
				compbond = compbond.substring(compbond.indexOf('!') + 1,
						compbond.length()).trim();
				...
				if (tempstr.equals("$")) {
					if (tempcp.getWildcards() != 0)
						...
						BNGEditor.displayOutput("\nError! Molecule: "
								+ molecules.get(tempmp.getMoleindex())
										.getName() + ", Component: " + compstr
								+ ", State: " + statestr
								+ " has multiple bond status!");
						ruleparsevalid = false;
						return;
					}
					...
					add2pbonds(
							tempstr,
							new ISite(tempmp.getMoleindex(), tempcp
									.getCompindex(), tempcp.getStateindex()));
			...
			if (tempstr.equals("$")) {
				if (tempcp.getWildcards() != 0)
					...
					BNGEditor.displayOutput("\nError! Molecule: "
							+ molecules.get(tempmp.getMoleindex()).getName()
							+ ", Component: " + compstr + ", State: "
							+ statestr + " has multiple bond status!");
					ruleparsevalid = false;
					return;
				}
			} else if (tempstr.equals("?")) {
				...
				add2pbonds(tempstr,
						new ISite(tempmp.getMoleindex(), tempcp.getCompindex(),
								tempcp.getStateindex()));
				tempcp.getPbondlist().add(tempstr);
			}
		}
		if (statebond.length() != 0) {
			while (statebond.indexOf('!') != -1) {
				...
				if (tempstr.indexOf('%') != -1)
					tempstr = tempstr.substring(0, tempstr.indexOf('%')).trim();
				statebond = statebond.substring(statebond.indexOf('!') + 1,
						statebond.length()).trim();
				if (tempstr.equals("$")) {
					if (tempcp.getWildcards() != 0)
						tempcp.setWildcards(1);
					else {
	...
	void getreactioncenter(IRule temprule) {
		...
		for (int i = 0; i < temprule.getReactantpatterns().size(); i++)
			for (int j = 0; j < temprule.getReactantpatterns().get(i)
					.getMolepatterns().size(); j++) {
				reactantmoles.add(temprule.getReactantpatterns().get(i)
						.getMolepatterns().get(j));
				for (int k = 0; k < temprule.getReactantpatterns().get(i)
						.getMolepatterns().get(j).getComppatterns().size(); k++)
					temprule.getReactantpatterns().get(i).getMolepatterns()
							.get(j).getComppatterns().get(k).setMatched(false);
			}
		for (int i = 0; i < temprule.getProductpatterns().size(); i++)
			for (int j = 0; j < temprule.getProductpatterns().get(i)
					.getMolepatterns().size(); j++) {
				productmoles.add(temprule.getProductpatterns().get(i)
						.getMolepatterns().get(j));
				for (int k = 0; k < temprule.getProductpatterns().get(i)
						.getMolepatterns().get(j).getComppatterns().size(); k++)
					temprule.getProductpatterns().get(i).getMolepatterns()
							.get(j).getComppatterns().get(k).setMatched(false);
			}
		...
		for (int i = 0; i < reactantmoles.size(); i++)
			if (!reactantmoles.get(i).isMatched())
				for (int j = 0; j < reactantmoles.get(i).getComppatterns()
						.size(); j++)
					reactantmoles.get(i).getComppatterns().get(j)
							.setReactioncenter(true);
		for (int i = 0; i < productmoles.size(); i++)
			if (!productmoles.get(i).isMatched())
				for (int j = 0; j < productmoles.get(i).getComppatterns()
						.size(); j++)
					productmoles.get(i).getComppatterns().get(j)
							.setReactioncenter(true);
	}
		...
		if (in1.getMoleindex() == in2.getMoleindex()) {
			...
			for (int i = 0; i < in1.getComppatterns().size(); i++)
				for (int j = 0; j < in2.getComppatterns().size(); j++)
					if (in1.getComppatterns().get(i).getCompindex() == in2
							.getComppatterns().get(j).getCompindex()
							&& !in1.getComppatterns().get(i).isMatched()
							&& !in2.getComppatterns().get(j).isMatched()) {
						in1.getComppatterns().get(i).setMatched(true);
						in2.getComppatterns().get(j).setMatched(true);
						if (!comparecomppattern(in1.getComppatterns().get(i),
								in2.getComppatterns().get(j))) {
							in1.getComppatterns().get(i)
									.setReactioncenter(true);
							in2.getComppatterns().get(j)
									.setReactioncenter(true);
						}
	...
	boolean comparecomppattern(IComponentPattern in1, IComponentPattern in2) {
		...
		if (in1.getStateindex() == in2.getStateindex()
				&& in1.getWildcards() == in2.getWildcards()
				&& in1.getBondlist().equals(in2.getBondlist()))
			return true;
		return false;
	}
	...
	void generateImap() {
		...
		for (int i = 0; i < rules.size(); i++)
			if (rules.get(i).isBidirection()) {
				rulenodes.add(new IRuleNode(rulenodeindex, i, true, rules
						.get(i).getName(), rules.get(i).getLabel()));
				rulenodeindex++;
				rulenodes.add(new IRuleNode(rulenodeindex, i, false, rules.get(
						i).getName(), rules.get(i).getLabel()));
				rulenodeindex++;
			} else {
				rulenodes.add(new IRuleNode(rulenodeindex, i, true, rules
						.get(i).getName(), rules.get(i).getLabel()));
		...
		for (int i = 0; i < rulenodes.size(); i++)
			for (int j = 0; j < rulenodes.size(); j++)
				if (i != j) {
					...
					determineinfluence(tempinfluence, rulenodes.get(i),
							rulenodes.get(j));
					if (tempinfluence.getActivation() != -1
							|| tempinfluence.getInhibition() != -1)
						influences.add(tempinfluence);
				}
	}

	void determineinfluence(Influence tempinfluence, IRuleNode rn1,
			IRuleNode rn2) {

		boolean canbreak;
		if (rn2.isForward() && rn1.isForward()) {
			canbreak = false;
			// pattern matching from all first half of reaction2 to all first
			// half of reaction1
			for (int i = 0; i < rules.get(rn2.getIruleindex())
					.getReactantpatterns().size()
					&& !canbreak; i++)
				for (int j = 0; j < rules.get(rn1.getIruleindex())
						.getReactantpatterns().size()
						&& !canbreak; j++)
					switch (patternmatching(rules.get(rn2.getIruleindex())
							.getReactantpatterns().get(i),
							rules.get(rn1.getIruleindex())
									.getReactantpatterns().get(j))) {
			...
			for (int i = 0; i < rules.get(rn2.getIruleindex())
					...
					&& !canbreak; i++)
				for (int j = 0; j < rules.get(rn1.getIruleindex())
						.getProductpatterns().size()
						&& !canbreak; j++)
					switch (patternmatching(rules.get(rn2.getIruleindex())
							.getReactantpatterns().get(i),
							rules.get(rn1.getIruleindex()).getProductpatterns()
									.get(j))) {
					// partial match and r1 reactioncenter
					case 0:
						// possible activation
	...
	int patternmatching(IRulePattern in1, IRulePattern in2) {
		...
		for (int i = 0; i < in1.getMolepatterns().size(); i++) {
			...
			for (int j = 0; j < in1.getMolepatterns().get(i).getComppatterns()
					.size(); j++)
				in1.getMolepatterns().get(i).getComppatterns().get(j)
						.setMatched(false);
		}
		for (int i = 0; i < in2.getMolepatterns().size(); i++) {
			in2.getMolepatterns().get(i).setMatched(false);
			for (int j = 0; j < in2.getMolepatterns().get(i).getComppatterns()
					.size(); j++)
				in2.getMolepatterns().get(i).getComppatterns().get(j)
						.setMatched(false);
		}
		...
		for (int i = 0; i < in1.getMolepatterns().size(); i++)
			for (int j = 0; j < in2.getMolepatterns().size(); j++)
				...
				if (in1.getMolepatterns().get(i).getMoleindex() == in2
						.getMolepatterns().get(j).getMoleindex()
						&& !in2.getMolepatterns().get(j).isMatched()
						&& !in1.getMolepatterns().get(i).isMatched()) {
					...
					for (int k = 0; k < tempmp1.getComppatterns().size(); k++) {
						...
						for (int l = 0; l < tempmp2.getComppatterns().size(); l++)
							...
							if (tempmp1.getComppatterns().get(k).getCompindex() == tempmp2
									.getComppatterns().get(l).getCompindex()
									&& !tempmp2.getComppatterns().get(l)
											.isMatched()
									&& !tempmp1.getComppatterns().get(k)
											.isMatched()) {
								tempmp2.getComppatterns().get(l)
										.setMatched(true);
								tempmp1.getComppatterns().get(k)
										.setMatched(true);
								Collections.sort(tempmp1.getComppatterns()
										.get(k).getPbondlist());
								Collections.sort(tempmp2.getComppatterns()
										.get(l).getPbondlist());

								// different states or different bond status
								if ((tempmp1.getComppatterns().get(k)
										.getStateindex() != tempmp2
										.getComppatterns().get(l)
										.getStateindex())
										|| !samebondstatus(tempmp1
												.getComppatterns().get(k),
												tempmp2.getComppatterns()
														.get(l))) {
									// conflict
									return -1;
								} else {
