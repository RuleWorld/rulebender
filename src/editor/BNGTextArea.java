...
public class BNGTextArea extends Composite 
{
	...
	private void initialize() {
		...
		sv.addPostSelectionChangedListener(new ISelectionChangedListener() {
			...
			public void selectionChanged(SelectionChangedEvent event) {
				...
				// get selected text
				String selected = textarea.getSelectionText();
		...
		textarea.setMenu(BNGEditor.getPopmenu());
		// Windows
		...
		// OSX
			...
			textarea.setFont(new Font(Display.getCurrent(), "Monaco", 12, SWT.NONE));
		// Other
		else
			textarea.setFont(new Font(Display.getCurrent(), "Monospace", 12, SWT.NONE));
				...
				if(found)
				{
					...
					if(i!=BNGEditor.getBlocknum()*2) {
						if (i == BNGEditor.getBlocknum() * 3) {
							// molecule names
							ranges.add(new StyleRange(m.start(1)-1 + textarea.getOffsetAtLine(temp[0]), m.group(1).length(), Display.getCurrent().getSystemColor(SWT.COLOR_BLUE),null, SWT.NULL));
						}
						else
							// keywords
							ranges.add(new StyleRange(m.start(1)-1 + textarea.getOffsetAtLine(temp[0]), m.group(1).length(), color_purple,null, SWT.NULL));
					}
					// comment
					else {
						ranges.add(new StyleRange(m.start(1)-1 + textarea.getOffsetAtLine(temp[0]), m.group(1).length(), color_PaleGreen4,null, SWT.NULL));
					}
					/*
	...
	void Undo()
	{
		if (!textchanges.empty()) 
		{
			...
			int textchangeLineIndex = textarea.getLineAtOffset(textchange.start);
			...
			if (textchangeLineIndex < topIndex || textchangeLineIndex > bottomIndex) {
				textarea.setTopIndex(textchangeLineIndex - (bottomIndex - topIndex) / 2);
			}
			ignoreUndo = false;
		}
	}
		...
		if(temp.size() == 0)
		{
			if (line < textarea.getLineCount() - 1) {
				...
				textarea.setSelection(textarea.getOffsetAtLine(line),
						textarea.getOffsetAtLine(line + 1)
								- textarea.getLineDelimiter().length());
			} else {
				// the last line
				textarea.setSelection(
						textarea.getOffsetAtLine(line),
						textarea.getOffsetAtLine(line)
