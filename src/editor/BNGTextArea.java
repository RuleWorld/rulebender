...
public class BNGTextArea extends Composite implements LinkedViewsReceiverInterface
{
	...
	private void initialize() {
		...
		sv.addTextListener(new ITextListener() {
			...
			public void textChanged(TextEvent event) {
				...
				if ((event.getText() == null || event.getText().equals("")) && event.getReplacedText() == null) {
					return ;
				}
				
				// first open
				if (event.getReplacedText() != null && event.getReplacedText().equals("abcdefg")) {
					return ;
				}
				
				//empty selection
				if (event.getText() == null || (event.getText().equals("") && event.getReplacedText().equals(""))) {
					return ;
				}
		...
		if(ConfigurationManager.getConfigurationManager().getOSType() == 1)
			styledTextArea.setFont(new Font(Display.getCurrent(), "SEGOE UI", 10, SWT.NONE));
		// OSX
		else if (ConfigurationManager.getConfigurationManager().getOSType() == 2)
			styledTextArea.setFont(new Font(Display.getCurrent(), "Monaco", 12, SWT.NONE));
		// Other
		else
			styledTextArea.setFont(new Font(Display.getCurrent(), "Monospace", 12, SWT.NONE));
	...
	void SyntaxHighlight(int in1, int in2, String in3, boolean global)
	{
		...
		if(!global)
		{
			...
			tempstr = styledTextArea.getText() + styledTextArea.getLineDelimiter();
		tempstr = " " + tempstr;
				...
				if(found)
				{
					...
					if(i!=BNGEditor.getBlocknum()*2) {
						if (i == BNGEditor.getBlocknum() * 3) {
							// molecule names
							ranges.add(new StyleRange(m.start(1)-1 + styledTextArea.getOffsetAtLine(temp[0]), m.group(1).length(), Display.getCurrent().getSystemColor(SWT.COLOR_BLUE),null, SWT.NULL));
						}
						else
							// keywords
							ranges.add(new StyleRange(m.start(1)-1 + styledTextArea.getOffsetAtLine(temp[0]), m.group(1).length(), color_purple,null, SWT.NULL));
					}
					// comment
					else {
						ranges.add(new StyleRange(m.start(1)-1 + styledTextArea.getOffsetAtLine(temp[0]), m.group(1).length(), color_PaleGreen4,null, SWT.NULL));
					}
					/*
		...
		ranges = tempArray;
		if(global)
			styledTextArea.replaceStyleRanges(0, tempstr.length()-styledTextArea.getLineDelimiter().length()-1, (StyleRange[]) ranges.toArray(new StyleRange[0]));
		else
			styledTextArea.replaceStyleRanges(styledTextArea.getOffsetAtLine(temp[0]), tempstr.length()-styledTextArea.getLineDelimiter().length()-1, (StyleRange[]) ranges.toArray(new StyleRange[0]));
	}
	...
	int[] getlinenums(int in1, int in2, String in3)
	{
		...
		else
		{
			...
			if(in1+in2<=styledTextArea.getCharCount()-1)
				tempstr = tempstr+styledTextArea.getText(in1+in2, styledTextArea.getCharCount()-1);
			tempdoc.set(tempstr);
			try {
	...
	void Undo()
	{
		if (!textchanges.empty()) 
		{
			...
			ignoreUndo = true;
			styledTextArea.replaceTextRange(textchange.start, textchange.length, textchange.replacedText);
			styledTextArea.setCaretOffset(textchange.start);
			int textchangeLineIndex = styledTextArea.getLineAtOffset(textchange.start);
			int topIndex = sv.getTopIndex();
			int bottomIndex = sv.getBottomIndex();
			// change top index if the changed text if not in display area
			if (textchangeLineIndex < topIndex || textchangeLineIndex > bottomIndex) {
				styledTextArea.setTopIndex(textchangeLineIndex - (bottomIndex - topIndex) / 2);
			}
			ignoreUndo = false;
		}
	}
			...
			for(int k=0;k<researchitem.size();k++)
			{
				...
				if(researchitem.get(k)%2 == 0)
				{
					...
					while(found)
					{
						...
						if(found && validateCaret(m.start(1)-1))
						{
							...
							blockinfo[researchitem.get(k)/2].line = styledTextArea.getLineAtOffset(Doc2Prodoc(tcaret1));
							found = false;
						}		
					}
				}
				else
				{
					...
					if(tcaret1 > blockinfo[researchitem.get(k)/2].caretend1)
					{
						...
						blockinfo[researchitem.get(k)/2].lineend = styledTextArea.getLineAtOffset(Doc2Prodoc(tcaret1));
					}
				}
	...
	boolean validateCaret(int caretin)
	{
		try {
			...
			Matcher m = BNGEditor.getKeywords().get(BNGEditor.getBlocknum()*2).matcher(tempstr);
	...
	public void find()
	{
		if(styledTextArea.getCaretOffset() == styledTextArea.getText().length())
	...
	public void replace()
	{
		if(styledTextArea.getCaretOffset() == styledTextArea.getText().length())
			styledTextArea.setSelection(0, 0);
		replaceShell = new Shell(BNGEditor.getMainEditorShell(), SWT.DIALOG_TRIM);
		...
		replacebutton2.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
                ...
                if(styledTextArea.getSelectionText()!= null)
                {
                	...
                	{
                		styledTextArea.replaceTextRange(styledTextArea.getCaretOffset()-strB.length(), strB.length(), replacetext2.getText());
                		return;
                	}
                }
                resetHighlights();
                ...
                if(StartFindPos-a!=0)
                	styledTextArea.replaceTextRange(a, b, replacetext2.getText());
			}
		});
		replacebutton2.setEnabled(false);		
		...
		while(temp2.size()!=0)
		{
			if(line>blockinfo[temp2.get(0)].line)
			{
				...
				else
				{
					...
					UpdateProdoc(false,0);
					styledTextArea.setSelection(styledTextArea.getOffsetAtLine(line), styledTextArea.getOffsetAtLine(line+1)-styledTextArea.getLineDelimiter().length());
					arc1.redraw();
					arc2.redraw();
					return;
				}
	...
	{	
		...
		String reverse_bi 	= forward_bi.substring(forward_bi.indexOf('>')+1, forward_bi.length()) + 
	...
	{	
		resetHighlights();
		styledTextArea.setSelection(styledTextArea.getCaretOffset(), styledTextArea.getCaretOffset());
	}
	...
	public void componentSelectedInContactMap(VisualItem componentItem) 
	{
		...
		System.out.println(moleculeName + "\\(^[\\)]" +name +"^[\\)]" + "\\)" );
		
		findFromRegExp(moleculeName +"\\s*\\((\\s|[^\\)])*"+ name + "([^\\)]|\\s)*\\)");
