    ...
    LinkedViewsReceiverInterface
{
  ...
  private void initialize()
  {
    ...
    sv.addTextListener(new ITextListener()
    {
      ...
      public void textChanged(TextEvent event)
      {
        ...
        if (event.getText() == null
            || (event.getText().equals("") && event.getReplacedText()
                .equals("")))
        {
          return;
        }
    ...
    sv.addPostSelectionChangedListener(new ISelectionChangedListener()
    {
      ...
      public void selectionChanged(SelectionChangedEvent event)
      {
        ...
        while (selectOffset != -1)
        {
          ...
          selectOffset = fileContent.substring(subStringOffset).indexOf(
              selected);
    ...
    if (ConfigurationManager.getConfigurationManager().getOSType() == 1)
      styledTextArea.setFont(new Font(Display.getCurrent(), "SEGOE UI", 10,
          SWT.NONE));
    // OSX
    ...
    // Other
      ...
      styledTextArea.setFont(new Font(Display.getCurrent(), "Monospace", 12,
          SWT.NONE));
  ...
  class CodeFoldRuler extends AbstractRulerColumn
  {
    CodeFoldRuler(int width)
    {
      ...
      this.setFont(new Font(Display.getCurrent(), "Courier New", 12, SWT.BOLD
          | SWT.COLOR_BLACK));
    }
    ...
    if (undobycollapse)
    {
      ...
      for (int k = 0; k < BNGEditor.getBlocknum(); k++)
        if (blockinfo[k].statechange == false && blockinfo[k].folded)
          try
          {
            prodoc.removeMasterDocumentRange(blockinfo[k].caretbegin2,
                blockinfo[k].caretend2 - blockinfo[k].caretbegin2);
          }
          catch (BadLocationException e)
          {
          }
      ...
      if (blockinfo[i].statechange)
      {
        try
        {
          ...
          if (blockinfo[i].folded)
          {
            try
            {
              prodoc.removeMasterDocumentRange(blockinfo[i].caretbegin2,
                  blockinfo[i].caretend2 - blockinfo[i].caretbegin2);
              if (popneeded)
              {
                TextChanges textchange = textchanges.pop();
                textchanges.push(new TextChanges(-1, -1, "", true,
                    blockinfo[i].caretbegin2 - offset, blockinfo[i].caretend2
                        - offset, true, i));
                if (Prodoc2Doc(textchange.start) > blockinfo[i].caretbegin1)
                  textchange.start = textchange.start - blockinfo[i].caretend2
                      + blockinfo[i].caretbegin2;
                textchanges.push(textchange);
              }
              else
                textchanges.push(new TextChanges(-1, -1, "", true,
                    blockinfo[i].caretbegin2, blockinfo[i].caretend2, true, i));
            }
            catch (BadLocationException e)
            {
            }
          ...
          else
          {
            try
            {
              prodoc.addMasterDocumentRange(blockinfo[i].caretbegin2,
                  blockinfo[i].caretend2 - blockinfo[i].caretbegin2);
              if (popneeded)
              {
                TextChanges textchange = textchanges.pop();
                ...
                textchanges
                    .push(new TextChanges(-1, -1, "", true,
                        blockinfo[i].caretbegin2, blockinfo[i].caretend2,
                        false, i));
            }
            catch (BadLocationException e)
            {
  ...
  void SyntaxHighlight(int in1, int in2, String in3, boolean global)
  {
    ...
    for (int i = 0; i < keywords.size(); i++)
    {
      ...
      while (found)
      {
        ...
        if (found)
        {
          ...
          if (i != BNGEditor.getBlocknum() * 2)
          {
            if (i == BNGEditor.getBlocknum() * 3)
            {
              ...
              ranges.add(new StyleRange(m.start(1) - 1
                  + styledTextArea.getOffsetAtLine(temp[0]), m.group(1)
                  .length(), Display.getCurrent()
                  .getSystemColor(SWT.COLOR_BLUE), null, SWT.NULL));
            }
            else
              // keywords
              ranges.add(new StyleRange(m.start(1) - 1
                  + styledTextArea.getOffsetAtLine(temp[0]), m.group(1)
                  .length(), color_purple, null, SWT.NULL));
          }
          // comment
          else
          {
            ranges.add(new StyleRange(m.start(1) - 1
                + styledTextArea.getOffsetAtLine(temp[0]), m.group(1).length(),
                color_PaleGreen4, null, SWT.NULL));
          }
          /*
           * if(i!=BNGEditor.blocknum*2) ranges.add(new StyleRange(m.start(1)-1
           ...
           * textarea.getOffsetAtLine(temp[0]), m.group(1).length(), Display
           * .getCurrent().getSystemColor(SWT.COLOR_DARK_GRAY),null, SWT.NULL));
           */
        }
      }
    }
    ...
    while (ranges.size() != 0)// sort
    {
      ...
      if (tempArray.size() == 0
          || ranges.get(int1).start >= tempArray.get(tempArray.size() - 1).start
  ...
  class TextChanges// To be used together with Undo
  {
    ...
    public TextChanges(int start, int length, String replacedText,
        boolean fold, int foldstart, int foldend, boolean expand, int blockindex)
    {
      this.start = start;
      this.length = length;
  ...
  void Undo()
  {
    if (!textchanges.empty())
    {
      ...
      if (textchange.fold)
      {
        try
        {
          ...
          if (!textchange.expand)
          {
            try
            {
              prodoc.removeMasterDocumentRange(textchange.foldstart,
      ...
      ignoreUndo = true;
      styledTextArea.replaceTextRange(textchange.start, textchange.length,
          textchange.replacedText);
  ...
  boolean validateCaret(int caretin)
  {
    try
    {
      ...
      Matcher m = BNGEditor.getKeywords().get(BNGEditor.getBlocknum() * 2)
          .matcher(tempstr);
  ...
  int Doc2Prodoc(int in)// caret
  {
    ...
    for (int i = 0; i < temp2.size(); i++)
    {
      ...
      if (in >= blockinfo[temp2.get(i)].caretend2
          && blockinfo[temp2.get(i)].folded)
        tempint = tempint
            - (blockinfo[temp2.get(i)].caretend2 - blockinfo[temp2.get(i)].caretbegin2);
    }
    return tempint;
  }
    ...
    replacebutton2.addSelectionListener(new SelectionAdapter()
    {
      public void widgetSelected(SelectionEvent e)
      {
        ...
        if (styledTextArea.getSelectionText() != null)
        {
          ...
          {
            styledTextArea.replaceTextRange(styledTextArea.getCaretOffset()
                - strB.length(), strB.length(), replacetext2.getText());
            return;
          }
        }
        resetHighlights();
  ...
  void selectErrorLine(int in)
  {
    ...
    while (temp2.size() != 0)
    {
      ...
      {
        if (line + blockinfo[temp2.get(0)].line
            - blockinfo[temp2.get(0)].lineend > blockinfo[temp2.get(0)].line)
        {
