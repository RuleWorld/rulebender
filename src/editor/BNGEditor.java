...
public class BNGEditor 
{
	...
	static String DOTfpath = null,DOTfname = null;
	...
	private void createSShell() {
		...
		sShell.setText("BNG Editor 0.2.80");
		createToolBar();
    	...
    	if(!pathfile.exists())
			...
			else
			{
				try {
				    BufferedReader br1 = new BufferedReader(new FileReader(pathfile));
				    DOTfpath = br1.readLine();
				    DOTfname = br1.readLine();
		    	} catch (FileNotFoundException e1) {} catch (IOException e1) {}
			}
				...
				okbutton.addMouseListener(new MouseListener()
				{
					...
					public void mouseUp(MouseEvent arg0) {
						...
						if(emptyfile.getSelection())
							tempfile = new CurrentFile(temp,newfiletext.getText(),ostype,true);
						else
							tempfile = new CurrentFile(temp,newfiletext.getText(),ostype,false);
		...
		mtf3.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				else
				{
					...
					if(!fileexist)
					{
						tempfile = new CurrentFile(fpath,fname,ostype,false);
						inputfiles.add(tempfile);
						fileselection = inputfiles.size()-1;
						textfolder.setSelection(fileselection);	
						if(ostype==1)
	...
	public static void setdotpath()
	{
		...
		if(DOTfpath !=null && DOTfname != null )
			if(ostype == 1)
				DOTPathtext1.setText(DOTfpath + "\\" + DOTfname);
			else
				DOTPathtext1.setText(DOTfpath + "/" + DOTfname);
		else
			if(ostype != 1)
			...
			public void widgetSelected(SelectionEvent arg0) {
				...
				else
				{
					...
					try {
						...
						if(ostype == 1)
						{
							DOTfpath = temppath.substring(0, temppath.lastIndexOf('\\'));
							DOTfname = temppath.substring(temppath.lastIndexOf('\\')+1, temppath.length());
						}
						else
						{
							DOTfpath = temppath.substring(0, temppath.lastIndexOf('/'));
