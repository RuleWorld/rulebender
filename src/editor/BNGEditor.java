...
//		MenuItem separator7 = new MenuItem(mvsub, SWT.SEPARATOR);
		...
		mo1.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
			    ...
			    if (fontData != null) {
			      ...
			      getInputfiles().get(getFileselection()).getBNGTextArea().getStyledTextArea().setFont(font);
					getInputfiles().get(getFileselection()).getBNGTextArea().arc1.redraw();
					getInputfiles().get(getFileselection()).getBNGTextArea().arc2.redraw();
			    }
			}
			public void widgetDefaultSelected(org.eclipse.swt.events.SelectionEvent e) {
			}
		...
		//mte3.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_C, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()).getKeyCode());
		...
		*/
		...
		mtf9.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				if (printer == null)
					printer = new Printer();
				getInputfiles().get(getFileselection()).getBNGTextArea().getStyledTextArea().print(printer).run();
			}
			public void widgetDefaultSelected(org.eclipse.swt.events.SelectionEvent e) {
			}
		});
		...
		//mtf6.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_Q, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()).getKeyCode());
	...
	private void setupEditorContextMenu()
	{
		...
		/*
		// TODO definitely can set more advanced checking here to see if the selected text is a 
		// molecule or rule.
		if(!getInputFiles().isEmpty() && !getInputfiles().get(getFileselection()).getBNGTextArea().getStyledTextArea().getSelectionText().equals(""))
		{
			selectMolecule.setEnabled(true);
		}
		else
		{
		...
		*/
		/*
		selectMolecule.addSelectionListener(new org.eclipse.swt.events.SelectionListener() 
		{
			// The method that is called when the menu item is selected.
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) 
			{
				// If there are no open files, then do nothing.
				if(textfolder.getSelection() == null)
					return;
				
				// Get the selected text.
				String selected = getInputfiles().get(getFileselection()).getBNGTextArea().getStyledTextArea().getSelectionText();
				
				// TODO this is going to be slower than necessary until we parse to make sure 
				// that it is a valid molecule before we send it.
		...
		*/
		/*
		 selectRule.addSelectionListener(new org.eclipse.swt.events.SelectionListener() 
	...
	{	
		System.out.println("My PID: " + ManagementFactory.getRuntimeMXBean().getName());
		...
		getShowfilepath().addFocusListener(new FocusAdapter()
		{
			public void focusGained(FocusEvent e)
			{
				if(getFileselection()!=-1)
					getInputfiles().get(getFileselection()).getBNGTextArea().getStyledTextArea().setFocus();
				else
					textfolder.setFocus();
			}
		});
