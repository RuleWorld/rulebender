...
public class BNGEditor {
	...
	private void setupMenuBar() {
		...
		setBNGPathMenuItem
				.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
					public void widgetSelected(
						...
						(new SetBNGPathDialogue(mainEditorShell, editor))
								.show();
					}
		...
		setWorkspaceMenuItem
				.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
					public void widgetSelected(
							org.eclipse.swt.events.SelectionEvent e) {
						(new SetWorkspaceDialogue(mainEditorShell, editor, true))
								.show();
					}
		...
		mth1.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				mb.setMessage("RuleBender 1.3.515 was written by\n\nAdam Smith, Wen Xu, Yao Sun\nDept. of Computer Science\nUniv. of Pittsburgh");
		 ...
		 * mte8.setText("&Delete All\tCtrl+D"); mte8.setAccelerator(SWT.MOD1 +
		 * 'D'); mte8.addSelectionListener(new
		 * org.eclipse.swt.events.SelectionListener() { public void
		 * widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
		 * if(textfolder.getSelection() == null) return;
		 * inputfiles.get(fileselection).getStyledTextArea().deleteall(); }
		 * public void
		 * widgetDefaultSelected(org.eclipse.swt.events.SelectionEvent e) { }
		 * });
		 */
		...
		mtf7.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int i = 0; i < getInputfiles().size(); i++)
					if (getInputfiles().get(i).tabitem.equals(textfolder
							.getSelection())) {
						setFileselection(i);
				...
				if (getInputfiles().get(getFileselection()).getFilepath() == null)
					getShowfilepath().setText("");
				else if (ConfigurationManager.getConfigurationManager()
						.getOSType() == 1)
					getShowfilepath().setText(
	...
	void AddKeyWords() {
		...
		getKeywords()
				.add(Pattern
						.compile("\\s+(begin[ \\t\\x0B\\f]+compartments)[#\\s]")); // 7
		getKeywords().add(
		...
		getKeywords().add(
				Pattern.compile("\\s+(end[ \\t\\x0B\\f]+actions)[#\\s]")); // 16
		...
		getKeywords().add(
				Pattern.compile("\\s+(begin[ \\t\\x0B\\f]+model)[#\\s]")); // 30
		getKeywords().add(
				Pattern.compile("\\s+(end[ \\t\\x0B\\f]+model)[#\\s]")); // 31
		getKeywords()
				.add(Pattern.compile("[\\s+\\)\\d\\w](->)[\\s+\\(\\d\\w]")); // 32
		getKeywords().add(
				Pattern.compile("[\\s+\\)\\d\\w](<->)[\\s+\\(\\d\\w]")); // 33
	...
	private void createToolBar() {
		 ...
		 * actionButton.addSelectionListener(new SelectionListener() { public
		 * void widgetSelected(SelectionEvent e) { if(textfolder.getSelection()
		 * == null) return;
		 * 
		 ...
		 * widgetSelected(SelectionEvent e) { if(textfolder.getSelection() ==
		 * null) return;
		 * 
		 * //BNGEditor.getEditor().getInputFiles().get(BNGEditor.getEditor().
		 * getFileSelection()).produceNewModels();
	...
	private void setupEditorContextMenu() {
		 ...
		 * org.eclipse.swt.events.SelectionListener() { // The method that is
		 * called when the menu item is selected. public void
		 * widgetSelected(org.eclipse.swt.events.SelectionEvent e) { // If there
		 * are no open files, then do nothing. if(textfolder.getSelection() ==
		 * null) return;
		 * 
		 ...
		 * 
		 * // TODO this is going to be slower than necessary until we parse to
		 * make sure // that it is a valid molecule before we send it.
		 ...
		 * there are no open files, then do nothing.
		 * if(textfolder.getSelection() == null) return;
		 * 
		 * // Get the selected text. String selected =
		 * getInputfiles().get(getFileselection
	...
	private void createTextfolder() {
		...
		textfolder.addMouseListener(new MouseListener() {
			...
			public void mouseDoubleClick(MouseEvent arg0) {
				String filename = "untitled" + emptyFileCount.toString()
						+ ".bngl";
	...
	static void init() {
		...
		textfolder.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent arg0) {
				...
				psController.fileSelectionChange(getInputfiles().get(
						getFileselection()));
			}
		});
	...
	public static void setResultViewerPath() {
		 ...
		 * 
		 * // parentDir String parentDir = System.getProperty("user.dir");
		 ...
		 * = appDir.substring(0, appDir.lastIndexOf("/")); resultsFolderPath =
		 * appDir + tmpSlash + "BNGResults"; } }
		 */

		String resultsFolderPath = ConfigurationManager
	...
	public static void replaceInputfiles(CurrentFile oldF, CurrentFile newF) {
		...
		if (ConfigurationManager.getConfigurationManager().getOSType() == 1) {
			getShowfilepath().setText(
					newF.getFilepath() + "\\" + newF.getFileName());
		} else {
			getShowfilepath().setText(
					newF.getFilepath() + "/" + newF.getFileName());
		}
