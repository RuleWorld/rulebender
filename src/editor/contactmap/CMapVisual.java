...
public class CMapVisual {
	...
	public CMapVisual(CMapModel model_in, Dimension cMapSize,
		...
		// DEBUG
		System.out.println("Molecules: " + model_in.getMolecules().size()
				+ "Bonds: " + model_in.getBonds().size() + "Rules: "
				+ model_in.getRules().size());

		System.out.println("\nMolecules: ");
		for (Molecule m : model_in.getMolecules()) {
			System.out.println("\t" + m.getExpression());
		}

		System.out.println("\nBonds: ");
		for (Bond b : model_in.getBonds()) {
			System.out.println("\t" + b.toString());
		}

		System.out.println("\nRules: ");
		for (Rule r : model_in.getRules()) {
		...
		// The VisualRule object that can create an edge. Arraylist of
		// VisualRule objects.
		...
		CMapClickControlDelegate cctrldelegate = new CMapClickControlDelegate(
				nv.getVisualization());
		...
		// This is how to get a hub node based on a Node set (all the involved
		// nodes in a rule)
		...
		for (int i = 0; i < model.getMolecules().size(); i++) {
			...
			// It has components
				...
				// for making invisible edges between components in the same
				// molecule.
				otherCompsInMol = new ArrayList<Node>();

				// Get the aggregate so you can add nodes to it.
				AggregateItem aggregateForMolecule = (AggregateItem) at
						.addItem();
				...
				// set invisible
				VisualItem nItem = vis.getVisualItem(COMPONENT_GRAPH,
						specialCompNode);
				nItem.setVisible(false);

				// Add it to the compartment
				if (compartment != null) {
					addItemToCompartmentAgg(cmptAggList, specialCompNode,
							compartment);
				}
				...
				for (int j = 0; j < tmole.getComponents().size(); j++) {
					...
					aggregateForMolecule.addItem(vis.getVisualItem(
							COMPONENT_GRAPH, n));
					aggregateForMoleculeCompartment.addItem(vis.getVisualItem(
							COMPONENT_GRAPH, n));
					...
					if (tcomp.getStates().size() != 0) {
						...
						for (int k = 0; k < tcomp.getStates().size(); k++) {
							...
							Node n_state = makeStateNode(tmole, tcomp, tstate,
									i, j, k, n);

							// Add it to the compartment
							if (compartment != null) {
								addItemToCompartmentAgg(cmptAggList, n_state,
										compartment);
							}
							// add it to the molecule
							aggregateForMolecule.addItem(vis.getVisualItem(
									COMPONENT_GRAPH, n_state));
							aggregateForMoleculeCompartment.addItem(vis
									.getVisualItem(COMPONENT_GRAPH, n_state));
		...
		for (int l = 0; l < model.getRules().size(); l++) {
			...
			// map the rest of rules which can not be mapped to bonds or changed
			// state
			identifyMoleLevelReaction(thisRule, r_comp);
		}

		// TODO I'm not sure what this is for, but I'm going to ignore it for
		// now.
		// if(tbond.CanGenerate == true)
		// {
		/*
		 * if(tbond.state1 != -1) {
		 * outfile.print("struct"+model.getMolecules().get
		 * (tbond.molecule1).name+
		 * "_"+model.getMolecules().get(tbond.molecule1).components
		 * .get(tbond.component1
		 * ).name+tbond.component1+":"+model.getMolecules().
		 * get(tbond.molecule1).
		 * components.get(tbond.component1).states.get(tbond.state1).name+"->");
		 * } else {
		 * outfile.print("struct"+model.getMolecules().get(tbond.molecule1
		 * ).name+
		 * "_"+model.getMolecules().get(tbond.molecule1).components.get(tbond
		 * .component1).name+tbond.component1+"->"); } if(tbond.state2 != -1) {
		 * outfile
		 * .println("struct"+model.getMolecules().get(tbond.molecule2).name
		 * +"_"+model
		 * .getMolecules().get(tbond.molecule2).components.get(tbond.component2
		 * ).
		 * name+tbond.component2+":"+model.getMolecules().get(tbond.molecule2).
		 * components
		 * .get(tbond.component2).states.get(tbond.state2).name+"[arrowhead=none];"
		 * ); } else {
		 * outfile.println("struct"+model.getMolecules().get(tbond.molecule2
		 * ).name
		 * +"_"+model.getMolecules().get(tbond.molecule2).components.get(tbond
		 * .component2).name+tbond.component2+"[arrowhead=none];"); }
		 */
		...
		/*
		 * } else { if(tbond.state1 != -1) {
		 * outfile.print("struct"+model.getMolecules
		 * ().get(tbond.molecule1).name+
		 * "_"+model.getMolecules().get(tbond.molecule1
		 * ).components.get(tbond.component1
		 * ).name+tbond.component1+":"+model.getMolecules
		 * ().get(tbond.molecule1).
		 * components.get(tbond.component1).states.get(tbond.state1).name+"->");
		 * } else {
		 * outfile.print("struct"+molecules.get(tbond.molecule1).name+"_"
		 * +molecules
		 * .get(tbond.molecule1).components.get(tbond.component1).name+
		 * tbond.component1+"->"); } if(tbond.state2 != -1) {
		 * outfile.println("struct"
		 * +molecules.get(tbond.molecule2).name+"_"+molecules
		 * .get(tbond.molecule2
		 * ).components.get(tbond.component2).name+tbond.component2
		 * +":"+molecules
		 * .get(tbond.molecule2).components.get(tbond.component2).states
		 * .get(tbond.state2).name+"[arrowhead=none,color=grey];"); } else {
		 * outfile
		 * .println("struct"+molecules.get(tbond.molecule2).name+"_"+molecules
		 * .get
		 * (tbond.molecule2).components.get(tbond.component2).name+tbond.component2
		 * +"[arrowhead=none,color=grey];"); } }
		 */
		...
		// CMapAnnotationController cmapAnnotation = new
		// CMapAnnotationController();

		// This is where all of those updates are coming from when a
		// visualization is loaded.
		// The only reason that this is necessary would be if there are
		// initializations that
		// have to happen.
		// nodes and edges
		 ...
		 * 
		 * 
		 * while (iter.hasNext()) { VisualItem item = (VisualItem) iter.next();
		 * if (item instanceof NodeItem) {
		 * cmapAnnotation.setAnnotationForNode(item); } else if (item instanceof
		 * EdgeItem) { cmapAnnotation.setAnnotationForEdge(item); } }
		 * 
		 * 
		 * // aggregates iter = vis.items(AGG); while (iter.hasNext()) {
		 * VisualItem item = (VisualItem) iter.next(); if (item instanceof
		 * AggregateItem) { cmapAnnotation.setAnnotationForAggregate(item); } }
		 */
	}
	...
	private Node makeComponentNode(Molecule tmole, Component tcomp,
		...
		System.out
				.println("Molecule: (" + tmole.getName() + "," + moleIndex
						+ ")\t\tComponent: (" + tcomp.getName() + ","
						+ compIndex + ")");
	...
	private void createEdgesForBonds() {
		...
		for (int b = 0; b < model.getBonds().size(); b++) {
			...
			// Declare two type of Edges, to show the CMAP in two mode: no
			// states, with states
			Edge e_comp, e_state;
			...
			Node leftnode = nodes.get(tbond.getMolecule1() + "."
					+ tbond.getComponent1());
			Node rightnode = nodes.get(tbond.getMolecule2() + "."
					+ tbond.getComponent2());
			...
			// TODO For multiple components with same name bug: This is where we
			// are creating the edges.
			// We use the bonds that are in the data structure so I am pretty
			// sure that it is in the parser.
			...
			// instantiate the ArrayList of Rule objects that will be stored
			// with the edge.
			...
			if (tbond.getState1() != -1) {
				...
				leftnode = nodes.get(tbond.getMolecule1() + "."
						+ tbond.getComponent1() + "." + tbond.getState1()); // state
																			// node
				if (leftnode != null)
					leftnode.set("hasedge", true);
			}
			...
			if (tbond.getState2() != -1) {
				...
				rightnode = nodes.get(tbond.getMolecule2() + "."
						+ tbond.getComponent2() + "." + tbond.getState2()); // state
																			// node
				if (rightnode != null)
					rightnode.set("hasedge", true);
			}
		...
		for (RulePattern rp : thisRule.getReactantpatterns()) {
			...
			for (MoleculePattern mp : rp.getMolepatterns()) {
				...
				if (mp.getComppatterns().size() == 0) {
					...
					r_state.addReactantMoleNode(nodes.get(""
							+ mp.getMoleindex()));
				}
				...
				for (ComponentPattern cp : mp.getComppatterns()) {
					if (cp.getStateindex() != -1) {
						// has certain state
						r_state.addReactantCompNode(nodes.get(mp.getMoleindex()
								+ "." + cp.getCompindex() + "."
								+ cp.getStateindex()));
					}

					r_comp.addReactantCompNode(nodes.get(mp.getMoleindex()
							+ "." + cp.getCompindex()));
					r_state.addReactantCompNode(nodes.get(mp.getMoleindex()
							+ "." + cp.getCompindex()));
				}
		...
		for (RulePattern pp : thisRule.getProductpatterns()) {
			...
			for (MoleculePattern mp : pp.getMolepatterns()) {
				...
				for (ComponentPattern cp : mp.getComppatterns()) {
					if (cp.getStateindex() != -1) {
						// has certain state
						r_state.addProductCompNode(nodes.get(mp.getMoleindex()
								+ "." + cp.getCompindex() + "."
								+ cp.getStateindex()));
					}

					r_comp.addProductCompNode(nodes.get(mp.getMoleindex() + "."
							+ cp.getCompindex()));
					r_state.addProductCompNode(nodes.get(mp.getMoleindex()
							+ "." + cp.getCompindex()));
				}
								...
								for (ComponentPattern pcp : pmp
									...
									if (rcompId == pcompId) {
										...
										if (rstateId != pstateId) {
											Node pstateNode = nodes.get(pmp
													.getMoleindex()
													+ "."
													+ pcp.getCompindex()
													+ "."
													+ pcp.getStateindex());
											Node rstateNode = nodes.get(rmp
													.getMoleindex()
													+ "."
													+ rcp.getCompindex()
													+ "."
											...
											// set component "statechange" =
											// true
											Node compNode = nodes.get(pmp
													.getMoleindex()
													+ "."
													+ pcp.getCompindex());
	...
	private void identifyMoleLevelReaction(Rule thisRule, VisualRule r_comp) {
		...
		// for two different rules, if they have same NodeSet, there will be
		// only one hub node
			...
			for (RulePattern rp : rpl) {
				for (MoleculePattern mp : rp.getMolepatterns()) {
					if (mp.getComppatterns().size() != 1) {
						...
						// component node
						compNode = nodes.get("" + mp.getMoleindex() + "."
								+ cp.getCompindex());
						if (cp.getStateindex() != -1) {
							// state node
							stateNode = nodes.get(mp.getMoleindex() + "."
									+ cp.getCompindex() + "."
									+ cp.getStateindex());
							if (stateNode != null) {
		...
		// exists, add current rule to the hub node
			...
			ArrayList<VisualRule> tmplist = (ArrayList<VisualRule>) n
					.get("rules");
			tmplist.add(r_comp);
		}
					...
					if (mp.getComppatterns().size() != 1) {
						...
						if (compNode != null) {
							...
							if (hasState && stateNode != null) {
								...
								// set invisible
								EdgeItem eitem = (EdgeItem) vis.getVisualItem(
										COMPONENT_GRAPH + ".edges", e_state);
								eitem.setVisible(false);
	...
	private void addItemToCompartmentAgg(ArrayList<AggregateItem> aggList,
		...
		for (int i = cmptList.size() - 1; i >= 0; i--) {
			...
			if (cmptStr.equals(cmptName)
					|| cmptTable.isChild(cmptStr, cmptName)) {
				// if (cmptStr.equals(cmptName)) {
