...
public class CMapVisual
{
      ...
      Dimension cMapOverviewSize)
  {
      ...
      if (tmole.getComponents().size() == 0)
      {
        ...
        if (compartment != null)
        {
          if (compartmentsFirstNode.containsKey(compartment))
          {
            ...
            EdgeItem ei = (EdgeItem) vis.getVisualItem(COMPONENT_GRAPH
                + ".edges", te);
            ei.setVisible(false);
          }
          else
        ...
        for (int j = 0; j < tmole.getComponents().size(); j++)
        {
          ...
          if (tcomp.getStates().size() != 0)
          {
            ...
            aggregateForComponent.setString(AGG_COMP_LABEL, tmole.getName()
                + "." + tcomp.getName());
            aggregateForComponent
                .addItem(vis.getVisualItem(COMPONENT_GRAPH, n));
            ...
            for (int k = 0; k < tcomp.getStates().size(); k++)
            {
              ...
              aggregateForMolecule.addItem(vis.getVisualItem(COMPONENT_GRAPH,
                  n_state));
              aggregateForMoleculeCompartment.addItem(vis.getVisualItem(
                  COMPONENT_GRAPH, n_state));
              // add it to the component
              aggregateForComponent.addItem(vis.getVisualItem(COMPONENT_GRAPH,
                  n_state));

              ((ArrayList<Node>) n.get("state_nodes")).add(n_state);

              // set invisible
              NodeItem nitem = (NodeItem) vis.getVisualItem(COMPONENT_GRAPH
                  + ".nodes", n_state);
              nitem.setVisible(false);
    ...
    // {
    /*
     * if(tbond.state1 != -1) { outfile.print("struct"+model.getMolecules().get
     * (tbond.molecule1).name+
     * "_"+model.getMolecules().get(tbond.molecule1).components
     * .get(tbond.component1 ).name+tbond.component1+":"+model.getMolecules().
     * get(tbond.molecule1).
     * components.get(tbond.component1).states.get(tbond.state1).name+"->"); }
     * else { outfile.print("struct"+model.getMolecules().get(tbond.molecule1
     * ).name+
     * "_"+model.getMolecules().get(tbond.molecule1).components.get(tbond
     * .component1).name+tbond.component1+"->"); } if(tbond.state2 != -1) {
     * outfile .println("struct"+model.getMolecules().get(tbond.molecule2).name
     * +"_"+model
     ...
     * outfile.print("struct"+model.getMolecules ().get(tbond.molecule1).name+
     * "_"+model.getMolecules().get(tbond.molecule1
     * ).components.get(tbond.component1
     * ).name+tbond.component1+":"+model.getMolecules ().get(tbond.molecule1).
     * components.get(tbond.component1).states.get(tbond.state1).name+"->"); }
     * else { outfile.print("struct"+molecules.get(tbond.molecule1).name+"_"
     * +molecules .get(tbond.molecule1).components.get(tbond.component1).name+
     * tbond.component1+"->"); } if(tbond.state2 != -1) {
     * outfile.println("struct"
     * +molecules.get(tbond.molecule2).name+"_"+molecules .get(tbond.molecule2
     * ).components.get(tbond.component2).name+tbond.component2 +":"+molecules
     * .get(tbond.molecule2).components.get(tbond.component2).states
     * .get(tbond.state2).name+"[arrowhead=none,color=grey];"); } else { outfile
     * .println("struct"+molecules.get(tbond.molecule2).name+"_"+molecules .get
     ...
     * 
     * 
     * while (iter.hasNext()) { VisualItem item = (VisualItem) iter.next(); if
     * (item instanceof NodeItem) { cmapAnnotation.setAnnotationForNode(item); }
     * else if (item instanceof EdgeItem) {
     * cmapAnnotation.setAnnotationForEdge(item); } }
     * 
     * 
     * // aggregates iter = vis.items(AGG); while (iter.hasNext()) { VisualItem
     * item = (VisualItem) iter.next(); if (item instanceof AggregateItem) {
  ...
  private void createEdgesForBonds()
  {
    ...
    for (int b = 0; b < model.getBonds().size(); b++)
    {
      ...
      if (tbond.getState1() != -1)
      {
        ...
        leftnode = nodes.get(tbond.getMolecule1() + "." + tbond.getComponent1()
            + "." + tbond.getState1()); // state
        // node
        if (leftnode != null)
          leftnode.set("hasedge", true);
      }
              ...
              for (ComponentPattern rcp : rmp.getComppatterns())
              {
                ...
                for (ComponentPattern pcp : pmp.getComppatterns())
                {
                  ...
                  if (rcompId == pcompId)
                  {
                    ...
                    {
                      Node pstateNode = nodes.get(pmp.getMoleindex() + "."
                          + pcp.getCompindex() + "." + pcp.getStateindex());
                      Node rstateNode = nodes.get(rmp.getMoleindex() + "."
                      ...
                      // true
                      Node compNode = nodes.get(pmp.getMoleindex() + "."
                          + pcp.getCompindex());
      ...
      for (RulePattern rp : rpl)
      {
        for (MoleculePattern mp : rp.getMolepatterns())
        {
          ...
          else
          {
            ...
            {
              // state node
              stateNode = nodes.get(mp.getMoleindex() + "." + cp.getCompindex()
                  + "." + cp.getStateindex());
              if (stateNode != null)
              {
