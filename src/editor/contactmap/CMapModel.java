...
public class CMapModel {
	...
	void parsemolecule(String molecule) {
		...
		tempmole.setExpression(molecule);
		...
		// If there is no ) then pull the name out and get the components from (
		// to end.
		if (leftparenthis && !rightparenthis) {
			tempmole.setName(molecule.substring(0, molecule.indexOf('('))
					.trim());
		...
		if (validatemole(tempmole)) {
			...
			if (this.getmoleindex(tempmole.getName()) == -1) {
				...
				// Handle compartment
				if (molecule.indexOf("@") != -1) {
					String compartment = molecule.substring(molecule
							.indexOf("@") + 1);
	...
	/**
	 * This method runs a few checks on the Molecule data structure to determine
	 * its validity
	 * 
	 * -ams292 5/1/10
	 * 
	...
	/**
	 * This method parses the rules between molecules. If the rule is valid then
	 * it is added to the rules arraylist.
	 * 
	 * -ams292 5/1/10
	 * 
	 * @param rulestr
		...
		if (compbond.length() != 0) {
			while (compbond.indexOf('!') != -1) {
				...
				if (tempstr.equals("$")) {
					...
					add2pbonds(
							tempstr,
							new Site(tempmp.getMoleindex(), tempcp
			...
			if (tempstr.equals("$")) {
				...
				add2pbonds(tempstr,
						new Site(tempmp.getMoleindex(), tempcp.getCompindex(),
