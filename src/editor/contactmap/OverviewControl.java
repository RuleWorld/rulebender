...
public class OverviewControl extends ControlAdapter implements PaintListener {
	 ...
	 * insets.bottom);
	 * 
	 * // Gets rectangle coordinates. Point rectStart = new Point(); Point
	 * rectEnd = new Point(); getRectangleCoordinates(display, rectStart,
	 * rectEnd);
	 * 
	 * int dx = (rectEnd.x - rectStart.x) / 2; int dy = (rectEnd.y -
	 * rectStart.y) / 2;
	 * 
	...
	private void drawClearRectangle(Graphics2D g) {
		...
		g.setClip(insets.left, insets.top, overview.getWidth() - insets.left
				- insets.right, overview.getHeight() - insets.bottom
				- insets.top);
		...
		// Bottom.
		g.fillRect(insets.left, rectangleEnd.y, overview.getWidth()
				- insets.left - insets.right, overview.getHeight()
				- insets.bottom - rectangleEnd.y);

		// Left.
		g.fillRect(insets.left, rectangleStart.y, rectangleStart.x
				- insets.left, rectangleEnd.y - rectangleStart.y);

		// Right.
		g.fillRect(rectangleEnd.x, rectangleStart.y, overview.getWidth()
				- rectangleEnd.x - insets.right, rectangleEnd.y
				- rectangleStart.y);
	...
	/*
	 * private void drawCross(Graphics g, Point start, Point end) { Color
	 * anterior = g.getColor(); g.setColor(Color.RED);
	 * 
