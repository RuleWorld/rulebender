...
public class OverviewControl extends ControlAdapter implements PaintListener
{
  ...
  /*
   * private void fixBounds(Point point) { Point boundsStart = new
   * Point(insets.left, insets.top); Point boundsEnd = new
   * Point(overview.getWidth() - insets.right, overview.getHeight() -
   * insets.bottom);
   * 
   * // Gets rectangle coordinates. Point rectStart = new Point(); Point rectEnd
   * = new Point(); getRectangleCoordinates(display, rectStart, rectEnd);
   * 
   * int dx = (rectEnd.x - rectStart.x) / 2; int dy = (rectEnd.y - rectStart.y)
   * / 2;
   * 
   * // Check bounds. int fixedX = point.x; int fixedY = point.y;
   * 
   * if (rectStart.x < boundsStart.x) { fixedX = boundsStart.x + dx; } if
   * (rectStart.y < boundsStart.y) { fixedY = boundsStart.y + dy; } if
   * (rectEnd.x > boundsEnd.x) { fixedX = boundsEnd.x - dx; } if (rectEnd.y >
   * boundsEnd.y) { fixedY = boundsEnd.y - dy; }
   * 
  ...
  private void getRectangleCoordinates(Display display, Point rectangleStart,
      Point rectangleEnd)
  {
    // Gets visible rect...
    ...
    tmpEnd.translate(display.getVisibleRect().width,
        display.getVisibleRect().height);
  ...
  private void drawClearRectangle(Graphics2D g)
  {
    ...
    g.setClip(insets.left, insets.top, overview.getWidth() - insets.left
        - insets.right, overview.getHeight() - insets.bottom - insets.top);
    g.setColor(new Color(ColorLib.hex("#4682B47A"), true));

    // ...and draws it:
    // Top.
    g.fillRect(insets.left, insets.top, overview.getWidth() - insets.left
        - insets.right, rectangleStart.y - insets.top);

    // Bottom.
    g.fillRect(insets.left, rectangleEnd.y, overview.getWidth() - insets.left
        - insets.right, overview.getHeight() - insets.bottom - rectangleEnd.y);

    // Left.
    g.fillRect(insets.left, rectangleStart.y, rectangleStart.x - insets.left,
        rectangleEnd.y - rectangleStart.y);

    // Right.
    g.fillRect(rectangleEnd.x, rectangleStart.y, overview.getWidth()
        - rectangleEnd.x - insets.right, rectangleEnd.y - rectangleStart.y);

    // Draws the rectangle.
    g.setColor(new Color(ColorLib.hex("#A6A8FA"), true));
    g.drawRect(rectangleStart.x, rectangleStart.y, rectangleEnd.x
        - rectangleStart.x, rectangleEnd.y - rectangleStart.y);
  ...
  /*
   * private void drawCross(Graphics g, Point start, Point end) { Color anterior
   * = g.getColor(); g.setColor(Color.RED);
   * 
   * g.drawLine(start.x, start.y, end.x, end.y); g.drawLine(start.x, end.y,
   * end.x, start.y);
   * 
   * g.setColor(anterior); }
   */
