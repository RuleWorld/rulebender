...
public class ConfigurationManager 
{
	...
	/**
	 * Reads the config file located in the user's home directory.  This should only be called once
	 * when the tool is being loaded because closing the dialogue that lets the user choose
	 * a workspace will close the tool.
	 */
	public void readConfigFile()
	{
		...
		if(configFile.exists())
		{
			...
			// to define a new one if the file does not exists when they try to run
			// a simulation.
			...
			if(!workspace.exists())
			{
				...
				(new SetWorkspaceDialogue(BNGEditor.getMainEditorShell(), BNGEditor.getEditor(), false)).show();
			}	
		...
		{
			workspacePath = System.getProperty("user.home") + slash +"RuleBenderWorkspace";
	...
	public void saveConfigFile()
	{
		...
		System.out.println("Deleted old Config");
	...
	public void setDefaultBNGPath()
	{
		...
		if (getOSType() == 2) 
		{
			...
			if (parentDir.indexOf("/Contents/MacOS") != -1) 
			{
				...
				bngpath = appDir + slash + "BioNetGen-2.1.7"
				+ slash + "Perl2";
			}
			else 
			{
				bngpath = parentDir + slash + "BioNetGen-2.1.7"
				+ slash + "Perl2";
			}
	...
	public void setBNGFPath(String bngFPath) {
		...
		System.out.println("BNGFPath set to " + bngFPath);
		saveConfigFile();
	}
