...
/**
 * This class implements a shell window that allows the user to open
 * a file.  
 * @author ams292
 *
 */
public class NewFileDialogue {
	 ...
	 * 
	 * @param mainEditorShell The shell that will hold the created shell object
	 * @param editorIn The BNGEditor object for which this shell will open files
	 */
	public NewFileDialogue(Shell mainEditorShell, BNGEditor editorIn)
	{
		...
		// Create the shell
		newFileDialogue = new Shell(parentShell, SWT.DIALOG_TRIM | SWT.APPLICATION_MODAL);
		...
		okbutton.addMouseListener(new MouseListener()
		{
			...
			public void mouseUp(MouseEvent arg0) {
				...
				if(emptyFile.getSelection()) {
					tempfile = new CurrentFile(temp,newFileText.getText(), ConfigurationManager.getConfigurationManager().getOSType(),true, -1);
					editor.incrementEmptyFileCount();
				}
				else {
					tempfile = new CurrentFile(temp,newFileText.getText(), ConfigurationManager.getConfigurationManager().getOSType(),false, -1);
					editor.incrementEmptyFileCount();
				}
