...
public abstract class Path2D implements Shape, Cloneable {
	 ...
	 * 
	 * /** Constructs a new empty single precision {@code Path2D} object with
	 * the specified winding rule and the specified initial capacity to store
	 * path segments. This number is an initial guess as to how many path
	 * segments will be added to the path, but the storage is expanded as needed
	 * to store whatever path segments are added.
	 * 
	 * @param rule the winding rule
	 * 
	 ...
	 */
	/*
	 * public Float(int rule, int initialCapacity) { super(rule,
	 * initialCapacity); floatCoords = new float[initialCapacity * 2]; } //
	 */
	/**
	 ...
	 * = p2d.cloneCoordsFloat(at); } else { PathIterator pi =
	 * s.getPathIterator(at); setWindingRule(pi.getWindingRule());
	 * this.pointTypes = new byte[INIT_SIZE]; this.floatCoords = new
	 * float[INIT_SIZE * 2]; append(pi, false); } }
	 * 
	 * float[] cloneCoordsFloat(AffineTransform at) { float ret[]; if (at ==
	 * null) { ret = Arrays.copyOf(this.floatCoords, this.floatCoords.length); }
	 * else { ret = new float[floatCoords.length]; at.transform(floatCoords, 0,
	 * ret, 0, numCoords / 2); } return ret; }
	 * 
	 ...
	 * // "in path definition");
	 * 
	 * try { throw new Exception("missing initial moveto "+
	 * "in path definition"); } catch (Exception e) { // e.printStackTrace(); }
	 * } int size = pointTypes.length; if (numTypes >= size) { int grow = size;
	 * if (grow > EXPAND_MAX) { grow = EXPAND_MAX; } pointTypes =
	 * Arrays.copyOf(pointTypes, size+grow); } size = floatCoords.length; if
	 * (numCoords + newCoords > size) { int grow = size; if (grow > EXPAND_MAX *
	 * 2) { grow = EXPAND_MAX * 2; } if (grow < newCoords) { grow = newCoords; }
	 ...
	 * = (float) x; floatCoords[numCoords-1] = (float) y; } else {
	 * needRoom(false, 2); pointTypes[numTypes++] = SEG_MOVETO;
	 * floatCoords[numCoords++] = (float) x; floatCoords[numCoords++] = (float)
	 * y; } } //
	 */
	/**
	 ...
	 */
	/*
	 * public final synchronized void moveTo(float x, float y) { if (numTypes >
	 * 0 && pointTypes[numTypes - 1] == SEG_MOVETO) { floatCoords[numCoords-2] =
	 * x; floatCoords[numCoords-1] = y; } else { needRoom(false, 2);
	 ...
	 * needRoom(true, 2); pointTypes[numTypes++] = SEG_LINETO;
	 * floatCoords[numCoords++] = (float) x; floatCoords[numCoords++] = (float)
	 * y; } //
	 */
	/**
	 ...
	 * (float) x2; floatCoords[numCoords++] = (float) y2;
	 * floatCoords[numCoords++] = (float) x3; floatCoords[numCoords++] = (float)
	 * y3; } //
	 */
	/**
	 ...
	 * float y2, float x3, float y3) { needRoom(true, 6); pointTypes[numTypes++]
	 * = SEG_CUBICTO; floatCoords[numCoords++] = x1; floatCoords[numCoords++] =
	 * y1; floatCoords[numCoords++] = x2; floatCoords[numCoords++] = y2;
	 ...
	 * numTypes; i++) { switch (pointTypes[i]) { case PathIterator.SEG_MOVETO:
	 * if (cury != movy) { crossings += Curve.pointCrossingsForLine(px, py,
	 * curx, cury, movx, movy); } movx = curx = coords[ci++]; movy = cury =
	 ...
	 * PathIterator.SEG_CUBICTO: crossings += Curve.pointCrossingsForCubic(px,
	 * py, curx, cury, coords[ci++], coords[ci++], coords[ci++], coords[ci++],
	 * endx = coords[ci++], endy = coords[ci++], 0); curx = endx; cury = endy;
	 * break; case PathIterator.SEG_CLOSE: if (cury != movy) { crossings +=
	 * Curve.pointCrossingsForLine(px, py, curx, cury, movx, movy); } curx =
	 * movx; cury = movy; break; } } if (cury != movy) { crossings +=
	 ...
	 * { float coords[] = floatCoords; double curx, cury, movx, movy, endx,
	 * endy; curx = movx = coords[0]; cury = movy = coords[1]; int crossings =
	 * 0; int ci = 2; for (int i = 1; crossings != Curve.RECT_INTERSECTS && i <
	 * numTypes; i++) { switch (pointTypes[i]) { case PathIterator.SEG_MOVETO:
	 * if (curx != movx || cury != movy) { crossings =
	 * Curve.rectCrossingsForLine(crossings, rxmin, rymin, rxmax, rymax, curx,
	 * cury, movx, movy); } // Count should always be a multiple of 2 here. //
	 * assert((crossings & 1) != 0); movx = curx = coords[ci++]; movy = cury =
	 * coords[ci++]; break; case PathIterator.SEG_LINETO: crossings =
	 * Curve.rectCrossingsForLine(crossings, rxmin, rymin, rxmax, rymax, curx,
	 * cury, endx = coords[ci++], endy = coords[ci++]); curx = endx; cury =
	 * endy; break; case PathIterator.SEG_QUADTO: crossings =
	 * Curve.rectCrossingsForQuad(crossings, rxmin, rymin, rxmax, rymax, curx,
	 * cury, coords[ci++], coords[ci++], endx = coords[ci++], endy =
	 * coords[ci++], 0); curx = endx; cury = endy; break; case
	 * PathIterator.SEG_CUBICTO: crossings =
	 ...
	 * case PathIterator.SEG_CLOSE: if (curx != movx || cury != movy) {
	 * crossings = Curve.rectCrossingsForLine(crossings, rxmin, rymin, rxmax,
	 * rymax, curx, cury, movx, movy); } curx = movx; cury = movy; // Count
	 * should always be a multiple of 2 here. // assert((crossings & 1) != 0);
	 * break; } } if (crossings != Curve.RECT_INTERSECTS && (curx != movx ||
	 * cury != movy)) { crossings = Curve.rectCrossingsForLine(crossings, rxmin,
	 * rymin, rxmax, rymax, curx, cury, movx, movy); } // Count should always be
	 * a multiple of 2 here. // assert((crossings & 1) != 0); return crossings;
	 * } //
	 */
	/**
	 * {@inheritDoc}
	 ...
	 */
	/*
	 * public final void append(PathIterator pi, boolean connect) { float
	 * coords[] = new float[6]; while (!pi.isDone()) { switch
	 * (pi.currentSegment(coords)) { case SEG_MOVETO: if (!connect || numTypes <
	 * 1 || numCoords < 1) { moveTo(coords[0], coords[1]); break; } if
	 * (pointTypes[numTypes - 1] != SEG_CLOSE && floatCoords[numCoords-2] ==
	 * coords[0] && floatCoords[numCoords-1] == coords[1]) { // Collapse out
	 * initial moveto/lineto break; } // NO BREAK; case SEG_LINETO:
	 * lineTo(coords[0], coords[1]); break; case SEG_QUADTO: quadTo(coords[0],
	 * coords[1], coords[2], coords[3]); break; case SEG_CUBICTO:
	 ...
	 */
	/*
	 * public final synchronized Rectangle2D getBounds2D() { float x1, y1, x2,
	 * y2; int i = numCoords; if (i > 0) { y1 = y2 = floatCoords[--i]; x1 = x2 =
	 * floatCoords[--i]; while (i > 0) { float y = floatCoords[--i]; float x =
	 * floatCoords[--i]; if (x < x1) x1 = x; if (y < y1) y1 = y; if (x > x2) x2
	 * = x; if (y > y2) y2 = y; } } else { x1 = y1 = x2 = y2 = 0.0f; } return
	 ...
	 * { return new CopyIterator(this); } else { return new TxIterator(this,
	 * at); } } //
	 */
	/**
	 ...
	 */
	/*
	 * public final Object clone() { // Note: It would be nice to have this
	 * return Path2D // but one of our subclasses (GeneralPath) needs to //
	 * offer "public Object clone()" for backwards // compatibility so we cannot
	 * restrict it further. // REMIND: Can we do both somehow? if (this
	 * instanceof GeneralPath) { return new GeneralPath(this); } else { return
	 * new Path2D.Float(this); } }
	 */
	/*
	 ...
	 * path.pointTypes[typeIdx]; int numCoords = curvecoords[type]; if
	 * (numCoords > 0) { System.arraycopy(floatCoords, pointIdx, coords, 0,
	 * numCoords); } return type; }
	 * 
	 * public int currentSegment(double[] coords) { int type =
	 * path.pointTypes[typeIdx]; int numCoords = curvecoords[type]; if
	 * (numCoords > 0) { for (int i = 0; i < numCoords; i++) { coords[i] =
	 ...
	 * public int currentSegment(float[] coords) { int type =
	 * path.pointTypes[typeIdx]; int numCoords = curvecoords[type]; if
	 * (numCoords > 0) { affine.transform(floatCoords, pointIdx, coords, 0,
	 * numCoords / 2); } return type; }
	 * 
	 * public int currentSegment(double[] coords) { int type =
	 * path.pointTypes[typeIdx]; int numCoords = curvecoords[type]; if
	 * (numCoords > 0) { affine.transform(floatCoords, pointIdx, coords, 0,
	...
	public static class Double extends Path2D {// implements Serializable {
		...
		public Double(Shape s, AffineTransform at) {
			if (s instanceof Path2D) {
				...
				this.pointTypes = copyByteArray(p2d.pointTypes,
						p2d.pointTypes.length);
		...
		double[] cloneCoordsDouble(AffineTransform at) {
			...
			if (at == null) {
				...
				ret = copyDoubleArray(this.doubleCoords,
		...
		void needRoom(boolean needMove, int newCoords) {
			if (needMove && numTypes == 0) {
				try {
					throw new Exception("missing initial moveto "
							+ "in path definition");
				} catch (Exception e) {
	 ...
	 * 
	 * if (isdbl) { dCoords = ((Path2D.Double) this).doubleCoords; fCoords =
	 * null; } else { fCoords = ((Path2D.Float) this).floatCoords; dCoords =
	 * null; }
	 * 
	 ...
	 * s.writeDouble(dCoords[cindex++]); } else {
	 * s.writeFloat(fCoords[cindex++]); s.writeFloat(fCoords[cindex++]); } } }
	 ...
	 * INIT_SIZE * 2; } if (storedbl) { ((Path2D.Double) this).doubleCoords =
	 * new double[nC]; } else { ((Path2D.Float) this).floatCoords = new
	 * float[nC]; }
	 * 
	 ...
	 * SEG_LINETO; break; case SERIAL_SEG_FLT_QUADTO: isdbl = false; npoints =
	 * 2; segtype = SEG_QUADTO; break; case SERIAL_SEG_FLT_CUBICTO: isdbl =
	 * false; npoints = 3; segtype = SEG_CUBICTO; break;
	 * 
	 ...
	 * needRoom(segtype != SEG_MOVETO, npoints * 2); if (isdbl) { while
	 * (--npoints >= 0) { append(s.readDouble(), s.readDouble()); } } else {
	 * while (--npoints >= 0) { append(s.readFloat(), s.readFloat()); } }
	 * pointTypes[numTypes++] = segtype; } if (nT >= 0 && s.readByte() !=
	 * SERIAL_PATH_END) { throw new
