package j15support.sun.awt.geom;
...
public abstract class Curve
{
   ...
   * 
   * public static void insertLine(Vector curves, double x0, double y0, double
   * x1, double y1) { if (y0 < y1) { curves.add(new Order1(x0, y0, x1, y1,
   * INCREASING)); } else if (y0 > y1) { curves.add(new Order1(x1, y1, x0, y0,
   * DECREASING)); } else { // Do not add horizontal lines } }
   * 
   * public static void insertQuad(Vector curves, double x0, double y0, double
   * coords[]) { double y1 = coords[3]; if (y0 > y1) { Order2.insert(curves,
   * coords, coords[2], y1, coords[0], coords[1], x0, y0, DECREASING); } else if
   * (y0 == y1 && y0 == coords[1]) { // Do not add horizontal lines return; }
   * else { Order2.insert(curves, coords, x0, y0, coords[0], coords[1],
   * coords[2], y1, INCREASING); } }
   * 
   * public static void insertCubic(Vector curves, double x0, double y0, double
   * coords[]) { double y1 = coords[5]; if (y0 > y1) { Order3.insert(curves,
   * coords, coords[4], y1, coords[2], coords[3], coords[0], coords[1], x0, y0,
   * DECREASING); } else if (y0 == y1 && y0 == coords[1] && y0 == coords[3]) {
   * // Do not add horizontal lines return; } else { Order3.insert(curves,
   * coords, x0, y0, coords[0], coords[1], coords[2], coords[3], coords[4], y1,
   * INCREASING); } }
   */
  /**
   ...
   * where the Y coordinate is increasing -1 is added for each crossing where
   * the Y coordinate is decreasing The return value is the sum of all crossings
   * for every segment in the path. The path must start with a SEG_MOVETO,
   * otherwise an exception is thrown. The caller must check p[xy] for NaN
   * values. The caller may also reject infinite p[xy] values as well.
   */
    ...
    while (!pi.isDone())
    {
      switch (pi.currentSegment(coords))
      {
        ...
        case PathIterator.SEG_QUADTO:
          endx = coords[2];
          endy = coords[3];
          crossings += pointCrossingsForQuad(px, py, curx, cury, coords[0],
        ...
        case PathIterator.SEG_CUBICTO:
          endx = coords[4];
          endy = coords[5];
          crossings += pointCrossingsForCubic(px, py, curx, cury, coords[0],
   ...
   * ray extending to the right from (px,py). If the point lies on the line,
   * then no crossings are recorded. +1 is returned for a crossing where the Y
   * coordinate is increasing -1 is returned for a crossing where the Y
      ...
      double y0, double xc, double yc, double x1, double y1, int level)
  {
    ...
    return (pointCrossingsForQuad(px, py, x0, y0, x0c, y0c, xc, yc, level + 1) + pointCrossingsForQuad(
   ...
   * the ray extending to the right from (px,py). If the point lies on a part of
   * the curve, then no crossings are counted for that intersection. the level
   * parameter should be 0 at the top-level call and will count up for each
   * recursion level to prevent infinite recursion +1 is added for each crossing
   * where the Y coordinate is increasing -1 is added for each crossing where
   * the Y coordinate is decreasing
  ...
  public static int pointCrossingsForCubic(double px, double py, double x0,
      double y0, double xc0, double yc0, double xc1, double yc1, double x1,
      double y1, int level)
  {
    ...
    return (pointCrossingsForCubic(px, py, x0, y0, xc0, yc0, xc0m, yc0m, xmid,
        ymid, level + 1) + pointCrossingsForCubic(px, py, xmid, ymid, xmc1,
        ymc1, xc1, yc1, x1, y1, level + 1));
  }
   ...
   * the bottom, then the count is incremented again and a full pass through the
   * shadow is indicated by the winding count having been incremented by 2.
   * 
   ...
   * which can then produce an answer directly. For intersection tests, the
   * answer is always "true" if the path intersects the rectangle. For
   * containment tests, the answer is always "false" if the path intersects the
   * rectangle. Thus, no further processing is ever needed if an intersection
   * occurs.
   */
   ...
   * right of the rectangle. See the comment for the RECT_INTERSECTS constant
   * for more complete details. The return value is the sum of all crossings for
   * both the top and bottom of the shadow for every segment in the path, or the
      ...
      switch (pi.currentSegment(coords))
      {
        ...
        case PathIterator.SEG_QUADTO:
          ...
          crossings = rectCrossingsForQuad(crossings, rxmin, rymin, rxmax,
              rymax, curx, cury, coords[0], coords[1], endx, endy, 0);
          curx = endx;
          cury = endy;
          break;
        case PathIterator.SEG_CUBICTO:
          ...
          crossings = rectCrossingsForCubic(crossings, rxmin, rymin, rxmax,
              rymax, curx, cury, coords[0], coords[1], coords[2], coords[3],
              endx, endy, 0);
          curx = endx;
          cury = endy;
          break;
      ...
      double xc, double yc, double x1, double y1, int level)
  {
    ...
    if (level > 52)
    {
      return rectCrossingsForLine(crossings, rxmin, rymin, rxmax, rymax, x0,
          y0, x1, y1);
    }
    double x0c = (x0 + xc) / 2;
    double y0c = (y0 + yc) / 2;
    ...
    crossings = rectCrossingsForQuad(crossings, rxmin, rymin, rxmax, rymax, x0,
        y0, x0c, y0c, xc, yc, level + 1);
    if (crossings != RECT_INTERSECTS)
    {
      crossings = rectCrossingsForQuad(crossings, rxmin, rymin, rxmax, rymax,
          xc, yc, xc1, yc1, x1, y1, level + 1);
    }
    return crossings;
  }
  ...
  public static int rectCrossingsForCubic(int crossings, double rxmin,
      double rymin, double rxmax, double rymax, double x0, double y0,
      double xc0, double yc0, double xc1, double yc1, double x1, double y1,
      int level)
  {
  ...
  /*
   * public boolean accumulateCrossings(Crossings c) { double xhi = c.getXHi();
   * if (getXMin() >= xhi) { return false; } double xlo = c.getXLo(); double ylo
   * = c.getYLo(); double yhi = c.getYHi(); double y0 = getYTop(); double y1 =
   * getYBot(); double tstart, ystart, tend, yend; if (y0 < ylo) { if (y1 <=
   * ylo) { return false; } ystart = ylo; tstart = TforY(ylo); } else { if (y0
   * >= yhi) { return false; } ystart = y0; tstart = 0; } if (y1 > yhi) { yend =
   * yhi; tend = TforY(yhi); } else { yend = y1; tend = 1; } boolean hitLo =
   * false; boolean hitHi = false; while (true) { double x = XforT(tstart); if
   * (x < xhi) { if (hitHi || x > xlo) { return true; } hitLo = true; } else {
   * if (hitLo) { return true; } hitHi = true; } if (tstart >= tend) { break; }
   * tstart = nextVertical(tstart, tend); } if (hitLo) { c.record(ystart, yend,
  ...
  public int compareTo(Curve that, double yrange[])
  {
    ...
    while (s0 < s1 && t0 < t1)
    {
      ...
      try
      {
        if (findIntersect(that, yrange, ymin, 0, 0, s0, xs0, ys0, sh, xsh, ysh,
  ...
  public boolean findIntersect(Curve that, double yrange[], double ymin,
      int slevel, int tlevel, double s0, double xs0, double ys0, double s1,
      double xs1, double ys1, double t0, double xt0, double yt0, double t1,
      double xt1, double yt1)
  {
    ...
    if (s1 - s0 > TMIN)
    {
      ...
      if (t1 - t0 > TMIN)
      {
        ...
        if (ys >= yt0 && yt >= ys0)
        {
          if (findIntersect(that, yrange, ymin, slevel + 1, tlevel + 1, s0,
              xs0, ys0, s, xs, ys, t0, xt0, yt0, t, xt, yt))
          {
            return true;
          }
        }
        ...
        {
          if (findIntersect(that, yrange, ymin, slevel + 1, tlevel + 1, s0,
              xs0, ys0, s, xs, ys, t, xt, yt, t1, xt1, yt1))
          {
            return true;
          }
        }
        if (yt >= ys)
        {
          if (findIntersect(that, yrange, ymin, slevel + 1, tlevel + 1, s, xs,
              ys, s1, xs1, ys1, t0, xt0, yt0, t, xt, yt))
          {
            return true;
          }
        }
        ...
        {
          if (findIntersect(that, yrange, ymin, slevel + 1, tlevel + 1, s, xs,
              ys, s1, xs1, ys1, t, xt, yt, t1, xt1, yt1))
          {
            return true;
          }
        }
      ...
      else
      {
        if (ys >= yt0)
        {
          if (findIntersect(that, yrange, ymin, slevel + 1, tlevel, s0, xs0,
              ys0, s, xs, ys, t0, xt0, yt0, t1, xt1, yt1))
          {
            return true;
          }
        }
        if (yt1 >= ys)
        {
          if (findIntersect(that, yrange, ymin, slevel + 1, tlevel, s, xs, ys,
              s1, xs1, ys1, t0, xt0, yt0, t1, xt1, yt1))
          {
            return true;
          }
        }
    ...
    else if (t1 - t0 > TMIN)
    {
      ...
      if (yt >= ys0)
      {
        if (findIntersect(that, yrange, ymin, slevel, tlevel + 1, s0, xs0, ys0,
            s1, xs1, ys1, t0, xt0, yt0, t, xt, yt))
        {
          return true;
        }
      }
