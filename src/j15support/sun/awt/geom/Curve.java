...
public abstract class Curve {
	 ...
	 * coords, coords[2], y1, coords[0], coords[1], x0, y0, DECREASING); } else
	 * if (y0 == y1 && y0 == coords[1]) { // Do not add horizontal lines return;
	 ...
	 * double coords[]) { double y1 = coords[5]; if (y0 > y1) {
	 * Order3.insert(curves, coords, coords[4], y1, coords[2], coords[3],
	 * coords[0], coords[1], x0, y0, DECREASING); } else if (y0 == y1 && y0 ==
	 * coords[1] && y0 == coords[3]) { // Do not add horizontal lines return; }
	 * else { Order3.insert(curves, coords, x0, y0, coords[0], coords[1],
	...
	/*
	 * public boolean accumulateCrossings(Crossings c) { double xhi =
	 * c.getXHi(); if (getXMin() >= xhi) { return false; } double xlo =
	 * c.getXLo(); double ylo = c.getYLo(); double yhi = c.getYHi(); double y0 =
	 * getYTop(); double y1 = getYBot(); double tstart, ystart, tend, yend; if
	 * (y0 < ylo) { if (y1 <= ylo) { return false; } ystart = ylo; tstart =
	 * TforY(ylo); } else { if (y0 >= yhi) { return false; } ystart = y0; tstart
	 * = 0; } if (y1 > yhi) { yend = yhi; tend = TforY(yhi); } else { yend = y1;
	 * tend = 1; } boolean hitLo = false; boolean hitHi = false; while (true) {
	 * double x = XforT(tstart); if (x < xhi) { if (hitHi || x > xlo) { return
	 * true; } hitLo = true; } else { if (hitLo) { return true; } hitHi = true;
	 * } if (tstart >= tend) { break; } tstart = nextVertical(tstart, tend); }
	 * if (hitLo) { c.record(ystart, yend, direction); } return false; }
	 */
