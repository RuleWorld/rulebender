...
public class Console implements IStartup, ISelectionListener, IPartListener2 {
	...
	private void addEditor(BNGLEditor editor) {
		...
		Logger.log(LOG_LEVELS.INFO, this.getClass(), "Added editor to hashmap: "
		    + path);
	}
	...
	private void focusResults(IWorkbenchPart part, ISelection selection) {
		...
		if (part instanceof DATMultiPageEditor) {
			osString = ((FileEditorInput) ((DATMultiPageEditor) part)
			    .getEditorInput()).getFile().getFullPath().toOSString();
		}
	...
	public void partClosed(IWorkbenchPartReference partRef) {
		...
		// If it's an editor
		if (part instanceof BNGLEditor) {
			String path = ((FileEditorInput) ((BNGLEditor) part).getEditorInput())
			    .getPath().toOSString();
