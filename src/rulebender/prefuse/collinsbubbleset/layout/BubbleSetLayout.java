package rulebender.prefuse.collinsbubbleset.layout;
...
import rulebender.prefuse.collinsbubbleset.layout.BubbleSetLayout.Intersection.State;
import rulebender.prefuse.collinsbubbleset.lib.MathLib;
...
public class BubbleSetLayout extends Layout {
	...
	private static final Logger LOGGER = Logger.getLogger(BubbleSetLayout.class.getName());
	
	static { Logger.getLogger(BubbleSetLayout.class.getName()).setLevel(Level.WARNING); }
     ...
     */
	public BubbleSetLayout(String aggrGroup, String sourceGroup, String nodeGroup, double nodeSurfaceDistance, double nodeInfluenceDistance, 
    ...
    /**
     * Set the superpixel size (resolution) used to calculate set boundaries.  Larger is faster but less accurate.
     * 
     * @param pixelGroup
     */
    ...
    public void run(double frac) {
        ...
        if (aggregateLabelLayout != null) {
        	...
        	while(labelIterator.hasNext()) {
        		...
        		Iterator<? extends VisualItem> itemIterator = aitem.items(VisiblePredicate.TRUE); // would VALIDATED false be better?
        	...
        	while ( aggrs.hasNext() ) {
        		...
        		// find if any items in the aggregate have moved
        		Iterator<? extends VisualItem> itemIterator = aitem.items(VisiblePredicate.TRUE);
        		if (itemIterator.hasNext()) {
        			// something changed, recalculate aggregate
        			run(aitem);
        		} 
    ...
    private void run(DecoratorItem di, AggregateItem aitem) {
        ...
        while (aggregateMembersIterator.hasNext() ) 
        {
        	...
        	if (item.get(VIRTUAL_EDGES) != null) 
        	{
        		...
        		ArrayList<Line2D> virtualEdges = (ArrayList<Line2D>) item.get(VIRTUAL_EDGES);
        		
        		//System.out.println("item " + item.get(VisualItem.LABEL) + " has virtual edges: ");
    	...
    	for(int i = 0 ; i < fhull.length-1; i+=2)
    		System.out.print(fhull[i] + ", " + fhull[i+1]);
    	System.out.println("]");
    	*/
    }
    ...
    public void fillPotentialArea(Rectangle2D activeArea, AggregateItem aitem, double[][] potentialArea) {
		...
		if (nodeInfluenceFactor != 0) {
			Iterator<? extends VisualItem> aNodeItems = aitem.items(visibleNodePredicate);
			while (aNodeItems.hasNext()) {
				...
				//
				// It seems like edges should get the line influence so I am changing this. 
				if(item instanceof EdgeItem)
				{
					//System.out.println("Using rectangle influence on Edge " + item.get("id"));
					//Deque<Line2D> singleLineDeque = (Deque<Line2D>) new ArrayDeque<Line2D>();
					ArrayList<Line2D> singleLineArrayList = (ArrayList<Line2D>) new ArrayList<Line2D>();
					
					Line2D line = new Line2D.Double(((EdgeItem) item).getSourceItem().getX(),((EdgeItem) item).getSourceItem().getY(),
							((EdgeItem) item).getTargetItem().getX(),((EdgeItem) item).getTargetItem().getY());
					
					//singleLineDeque.add(line);
					singleLineArrayList.add(line);
					
					calculateLinesInfluence(potentialArea, a*influenceFactor, edgeR1, singleLineArrayList, activeArea);
				}
				else
				{
				...
				//ArrayList<Line2D> scannedLines = (Deque<Line2D>) item.get(VIRTUAL_EDGES);
				ArrayList<Line2D> scannedLines = (ArrayList<Line2D>) item.get(VIRTUAL_EDGES);
		...
		Iterator<? extends VisualItem> aEdgeItems = aitem.items(VISIBLE_EDGE_PREDICATE);
            ...
            while (sourceItems.hasNext()) {
	        	...
	        	if (aitem.containsItem(item)) { 
		        	...
		        	if (activeArea.intersects(item.getBounds())) {
		        		if ((item.isInGroup(nodeGroup)) && item instanceof NodeItem &&(negativeNodeInfluenceFactor != 0)) {
		        			// subtract influence
     ...
     *  
     * @param activeArea the screen coordinates of the region to fill
     * @param aggregateDecorator the aggregate decorator (e.g. label) to calculate energy for 
     * @param potentialArea the energy field to fill in
     */
	...
	/**
	 * Visit all items in the aggregate, connecting them to their nearest explicitly connected neighbour, or
	 * the best neighbour which is also in the set (see connectItem(aitem, item, visited) for detail on selection
	 * of best neighbour.  Stores the connections in the VIRTUAL_EDGES deque for each item in the aggregate.
	 * 
	 * getAdjacentItem
	 * @param aitem
	 */
	private void calculateVirtualEdges(AggregateItem aitem) {
		...
		//Deque<VisualItem> visited = (Deque<VisualItem>) new ArrayDeque<VisualItem>();
		...
		while (aNodeItems.hasNext()) 
		{
			...
			//DEBUG
			//if(item instanceof EdgeItem)
				//System.out.println("\t\tItem (" + item.getX() + ", " + item.getY() + ") is Edge " + item.get("id"));
//			else if(item instanceof NodeItem)
	//			System.out.println("\t\tItem (" + item.getX() + ", " + item.getY() + ") is Node " + item.getString(VisualItem.LABEL));
			
			// check for visible edge structure connecting this node to others 
			if ((useStructuralEdges) && (item instanceof NodeItem)) {
				NodeItem nodeItem = (NodeItem) item;
				Iterator<? extends VisualItem> edgeIterator = aitem.items(VISIBLE_EDGE_PREDICATE);
	...
	/**
	 * Find the sequence of virtual edges which connect a given item to it's best unvisited neighbour within the set. 
	 * Considerations for selection of best neighbour include distance and number of intervening non-set items on 
	 * the straight line between the item and the candidate neighbour.
	 *  
	 *  
	 *  NodeItems and EdgeItems are both treated equally here, but EdgeItems do not have their X and Y fields set.
	 *  I will add the appropriate calls to set them before they are used.
	 *  ams292 Aug 4, 2010. 
	 *  
	 * @param aitem the set to search
	 * @param item the item to find the best neighbour for
	 * @param visited the already connected items within the set
	 * @return a deque containing the virtual edges which connect the item to its best neighbour 
	 */
	// 
	//private Deque<Line2D> connectItem(AggregateItem aitem, VisualItem item, Collection<VisualItem> visited)
	private ArrayList<Line2D> connectItem(AggregateItem aitem, VisualItem item, Collection<VisualItem> visited)
	{
		double itemX = 0;
		double itemY = 0;
		double nItemX = 0;
		...
		{
			//System.out.println("\n\tconnectItem called on Node "+ item.get(VisualItem.LABEL)+" ("+ item.getX() + ", " + item.getY()+")");
			itemX = item.getX();
			itemY = item.getY();
		}
		...
		//	System.out.println("\n\tconnectItem called on Edge "+ item.get("id")+" ("+ item.getX() + ", " + item.getY()+")");
			...
			//System.out.println("Setting itemX and itemY (" + itemX +", " + itemY + ")");
		}
		
		VisualItem closestNeighbour = null;
		//Deque<Line2D> scannedLines = (Deque<Line2D>) new ArrayDeque<Line2D>();
		//Deque<Line2D> linesToCheck = (Deque<Line2D>) new ArrayDeque<Line2D>();
		...
		while (neighbourIterator.hasNext()) 
		{
			...
			{
				//System.out.println("\t\t***************************************\n"+
					//	"\t\t* Neighbour Item is EdgeItem " + neighbourItem.get("id"));
			//	System.out.println("\t\t* Original Position: (" + neighbourItem.getX() + ", " + neighbourItem.getY() + ")");
				
				nItemX = neighbourItem.getBounds().getCenterX();
				nItemY = neighbourItem.getBounds().getCenterY();
				
				//System.out.println("\t\t* Bounds Center: ("+neighbourItem.getBounds().getCenterX() + ", " + neighbourItem.getBounds().getCenterY()+")");
				//System.out.println("\t\t* Using Center: ("+nItemX + ", " + nItemY+")");
				//System.out.println("\t\t***************************************");
			}
			
			else if (neighbourItem instanceof NodeItem)
			{
			//	System.out.println("\t\tNeighbour Item is NodeItem " + neighbourItem.get(VisualItem.LABEL) +"(" + neighbourItem.getX() + ", " + neighbourItem.getY() + ")");
				nItemX = item.getX();
				nItemY = item.getY();
			}
		...
		if ((closestNeighbour != null) && (edgeInfluenceFactor != 0)) 
		{
			...
			if(closestNeighbour instanceof EdgeItem)
				completeLine = new Line2D.Double(itemX, itemY, closestNeighbour.getBounds().getCenterX(), closestNeighbour.getBounds().getCenterY());
			else
				completeLine = new Line2D.Double(itemX, itemY, closestNeighbour.getX(), closestNeighbour.getY());
			...
			if (doMorph) 
			{
				...
				while (hasIntersection && iterations < MAX_ROUTING_ITERATIONS) {
					...
					while (!hasIntersection && !linesToCheck.isEmpty()) {
						...
						if (!hasIntersection) scannedLines.add(line);
	...
	/*	
		System.out.println();
		for(Line2D l : scannedLines)
		{
			System.out.println("\tReturned Line:\t(" + l.getX1() + ", " + l.getY1() + ", " + l.getX2() + ", " + l.getY2()+")");
		}
		// */
		return scannedLines;
	}
   ...
   // public void calculateLinesInfluence(double[][]potentialArea, double influenceFactor, double r1, Deque<Line2D> lines, Rectangle2D bounds) {
    public void calculateLinesInfluence(double[][]potentialArea, double influenceFactor, double r1, ArrayList<Line2D> lines, Rectangle2D bounds) {
		double tempX, tempY, distance = 0;
		double minDistance = Double.MAX_VALUE;
			...
			while (aNodeItems.hasNext()) {
				...
				for (Line2D edge : edges) {
					...
					if (length/2 >= maxLength) {
						...
						Point2D testPoint = interpolateLine2D(edge, 0.5 - ((labelWidth / length) / 2.0));
						if (!item.getBounds().contains(testPoint)) {
							while (!item.getBounds().contains(testPoint)  && (labelFont.getSize() < MAX_LABEL_SIZE)) {
								...
								fm = g.getFontMetrics(labelFont);
								labelWidth = fm.stringWidth(di.getString(titleField));
								testPoint = interpolateLine2D(edge, 0.5 - ((labelWidth / length) / 2.0));
							}
