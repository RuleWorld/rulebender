...
/**
 * This class tests the BNGASTReader.  It should test that the reader 
 * is returning the correct information with respect to the TestModel.bngl file. 
...
public class TestBNGASTReader 
{
	...
	public void setup()
	{
		...
		try 
		{
			...
			ast = BNGParserCommands.getASTForFileName("/Users/mr_smith22586/Documents/workspace/rulebender/testFiles/bngl/TestModel.bngl");
	...
	{	
		...
		BNGLModelBuilderInterface mockBuilder = new BNGLModelBuilderInterface()
		{
			...
			/**
			 * Implementation checks for presence of (some of the) molecules specific to this model.
			 */
		...
		BNGLModelBuilderInterface mockBuilder = new BNGLModelBuilderInterface()
		{
			...
			public void foundRule(RuleData ruleData)
			{
				...
				if(ruleData.getName().equals("SuperR1"))
				{
					...
					for(RulePatternData rpd : ruleData.getReactantPatternData())
					{
						...
						for(MoleculePatternData mpd : rpd.getMoleculePatterns())
						{
							...
							if(mpd.getName().equals("Rec"))
							{
								...
								assertTrue(mpd.getComponentPatterns().get(0).getComponent().equals("a"));
							...
							else if(mpd.getName().equals("Lig"))
							{
								...
								assertTrue(mpd.getComponentPatterns().get(0).getComponent().equals("l"));
								assertTrue(mpd.getComponentPatterns().get(0).getState() == null);
					...
					assertTrue(ruleData.getProductPatternData().get(0).getMoleculePatterns().size() == 2);

					// For each produce molecule pattern
					for(MoleculePatternData mpd : ruleData.getProductPatternData().get(0).getMoleculePatterns())
					{	
						// If it's the Rec pattern, then the molecule is Rec
						if(mpd.getName().equals("Rec"))
						{
				...
				if(ruleData.getName().equals("R3"))
				{
					...
					for(int i = 0; i < ruleData.getReactantPatternData().size(); i++)
					{
						...
						if(i == 0)
						{
							...
							assertTrue(mpd.getComponentPatterns().get(0).getComponent().equals("b"));
							assertTrue(mpd.getComponentPatterns().get(0).getState().equals("Y"));
						...
						if(i == 1)
						{
							...
							// The component is "b" and is in the "Y" state
							assertTrue(mpd.getComponentPatterns().get(0).getComponent().equals("U"));
							assertTrue(mpd.getComponentPatterns().get(0).getState() == null);
							assertTrue(mpd.getComponentPatterns().get(1).getComponent().equals("SH2"));
							assertTrue(mpd.getComponentPatterns().get(1).getState() == null);
						}
					}
