...
/**
 * This class creates the data structure required for prefuse and interacts with
 * the NetworkView class in order to show a visualization of the contact map
 * data structure constructed by the parser.
 * 
 * @author ams
 * 
 */
...
public class ContactMapVisual {
	...
	// "<parent molecule index>.<component index>.<state index>".
	private final Hashtable<String, Node> m_nodes;

	// This is so I can get an edge, based on an string value: bondID.<type>
	// (either component or state)
	private final Hashtable<String, Edge> m_edges;
	...
	public ContactMapVisual(ContactMapView view, ContactMapModel model_in,
		...
		// Graphs (and all other data structures in prefuse) are table-based data
		// structures. Each node is a row in the
		...
		// The VisualRule object that can create an edge. Arraylist of VisualRule
		// objects.
		...
		// add a column to decide which edge to show in different display mode (show
		// states)
		...
		AggregateTable at_compartment = m_networkViewer
		    .addAggregateTable("compartments");
		...
		CMapClickControlDelegate cctrldelegate = new CMapClickControlDelegate(view,
		    model_in.getSourcePath(), m_networkViewer.getVisualization());
		...
		// This is how to get a hub node based on a Node set (all the involved nodes
		// in a rule)
		...
		for (int i = 0; i < m_model.getMolecules().size(); i++) {
			...
			if (tmole.getComponents().size() == 0) {
				...
				if (compartment != null) {
					if (compartmentsFirstNode.containsKey(compartment)) {
						...
						EdgeItem ei = (EdgeItem) m_vis.getVisualItem(COMPONENT_GRAPH
						    + ".edges", te);
						ei.setVisible(false);
					} else {
			...
			// It has components
				...
				aggregateForMolecule.setString("molecule_expression",
				    tmole.getExpression());
				// Set its compartment
				String compartment = tmole.getFirstCompartment();
				...
				// Get the aggregate so you can add nodes to it.
				AggregateItem aggregateForMoleculeCompartment = (AggregateItem) at_compartment
				    .addItem();
				aggregateForMoleculeCompartment.setString("type", "compartment");
				// DEBUG
				// System.out.println("365: Setting compartment POSSIBLE MOLE NAME: " +
				// tmole.getName());
				aggregateForMoleculeCompartment.setString("compartment",
				    tmole.getName());
				...
				// set invisible
				VisualItem nItem = m_vis
				    .getVisualItem(COMPONENT_GRAPH, specialCompNode);
				...
				for (int j = 0; j < tmole.getComponents().size(); j++) {
					...
					aggregateForMoleculeCompartment.addItem(m_vis.getVisualItem(
					    COMPONENT_GRAPH, n));
					...
					if (tcomp.getStates().size() != 0) {
						...
						AggregateItem aggregateForComponent = (AggregateItem) at_comp
						    .addItem();
						aggregateForComponent.setString(AGG_COMP_LABEL, tmole.getName()
						    + "." + tcomp.getName());
						aggregateForComponent.addItem(m_vis.getVisualItem(COMPONENT_GRAPH,
						    n));
						...
						for (int k = 0; k < tcomp.getStates().size(); k++) {
							...
							aggregateForMolecule.addItem(m_vis.getVisualItem(COMPONENT_GRAPH,
							    n_state));
							aggregateForMoleculeCompartment.addItem(m_vis.getVisualItem(
							    COMPONENT_GRAPH, n_state));
							// add it to the component
							aggregateForComponent.addItem(m_vis.getVisualItem(
							    COMPONENT_GRAPH, n_state));

							((ArrayList<Node>) n.get("state_nodes")).add(n_state);

							// set invisible
							NodeItem nitem = (NodeItem) m_vis.getVisualItem(COMPONENT_GRAPH
							    + ".nodes", n_state);
							nitem.setVisible(false);
	...
	private Node makeComponentNode(Molecule tmole, Component tcomp,
	    int moleIndex, int compIndex) {
		Node n = m_componentGraph.addNode();
		// Set its name
		...
		// System.out.println("Molecule: (" + tmole.getName() + "," + moleIndex +
		// ")\t\tComponent: (" + tcomp.getName() + "," + compIndex +")");
	...
	private void createEdgesForBonds() {
		...
		for (int currentBondIndex = 0; currentBondIndex < m_model.getBonds().size(); currentBondIndex++) {
			// Declare two type of Edges, to show the CMAP in two mode: no states,
			// with states
			Edge e_comp, e_state;
			...
			Node leftnode = m_nodes.get(tbond.getMolecule1() + "."
			    + tbond.getComponent1());
			Node rightnode = m_nodes.get(tbond.getMolecule2() + "."
			    + tbond.getComponent2());
			...
			m_edges.put(Integer.toString(currentBondIndex) + "." + "component",
			    e_comp);
			// instantiate the ArrayList of Rule objects that will be stored with the
			// edge.
			...
			// System.out.println("Check " + tbond.getMolecule1() + "." +
			// tbond.getComponent1());

			// If the state of the first component is not -1 (there is one)
			if (tbond.getState1() != -1) {
				// Set the leftparentNode as the leftnode. The leftnode is the component
				// level.
				leftparentnode = leftnode;

				// Set the the leftnode as the state level node.
				leftnode = m_nodes.get(tbond.getMolecule1() + "."
				    + tbond.getComponent1() + "." + tbond.getState1());
				...
				// Do nothing.
					...
					// tbond.getMolecule1()+"."+tbond.getComponent1()+"."+tbond.getState1());
				}
			} else {
				// System.out.println("\tDid not consider " + tbond.getMolecule1() + "."
				// + tbond.getComponent1() + "(no state)");
			}

			// System.out.println("Check " + tbond.getMolecule2() + "." +
			// tbond.getComponent2());
			...
			if (tbond.getState2() != -1) {
				...
				rightnode = m_nodes.get(tbond.getMolecule2() + "."
				    + tbond.getComponent2() + "." + tbond.getState2()); // state node
			...
			// Do nothing if there is no state associated with the node.
				...
				// System.out.println("\tDid not consider " + tbond.getMolecule2() + "."
				// + tbond.getComponent2() + "(no state)");
			}

			// If either of the parent nodes are not null, then there is a bond that
			// requires a state.
			if (leftnode != null && rightnode != null
			    && (leftparentnode != null || rightparentnode != null)) {
				...
				// Put the edge into the hashtable.
				m_edges
				    .put(Integer.toString(currentBondIndex) + "." + "state", e_state);

				// instantiate the ArrayList of Rule objects that will be stored with
				// the edge.
	...
	private void identifyBonds(Rule thisRule, VisualRule r_comp,
	    VisualRule r_state) {
		// DEBUG
		...
		for (BondAction ba : thisRule.getBondactions()) {
			...
			if (e_state != null) {
				...
				ArrayList<VisualRule> e_state_rules = (ArrayList<VisualRule>) e_state
				    .get("rules");
				// System.out.println("\t\tExisting Visual Rules: " +
				// e_state_rules.size());
				e_state_rules.add(r_state);
				// System.out.println("\t\tNow Visual Rules: " +
				// ((ArrayList<VisualRule>) e_state.get("rules")).size());
			} else {
				// DEBUG
			...
			if (e_comp != null) {
				...
				// Add the Visual rule to the Edge.
				ArrayList<VisualRule> e_comp_rules = (ArrayList<VisualRule>) e_comp
				    .get("rules");
				// System.out.println("\t\tExisting Visual Rules: " +
				// e_comp_rules.size());
				e_comp_rules.add(r_state);
				// System.out.println("\t\tNow Visual Rules: " +
				// ((ArrayList<VisualRule>) e_comp.get("rules")).size());
			} else {
				// DEBUG
				// System.out.println("\te_state was null");
			}
	...
	private void identifyReactants(Rule thisRule, VisualRule r_comp,
	    VisualRule r_state) {
			...
			for (MoleculePattern mp : rp.getMolepatterns()) {
				...
				for (ComponentPattern cp : mp.getComppatterns()) {
					if (cp.getStateindex() != -1) {
						// has certain state
						r_state.addReactantCompNode(m_nodes.get(mp.getMoleIndex() + "."
						    + cp.getCompIndex() + "." + cp.getStateindex()));
					}

					r_comp.addReactantCompNode(m_nodes.get(mp.getMoleIndex() + "."
					    + cp.getCompIndex()));
					r_state.addReactantCompNode(m_nodes.get(mp.getMoleIndex() + "."
					    + cp.getCompIndex()));
				}
	...
	private void identifyProducts(Rule thisRule, VisualRule r_comp,
	    VisualRule r_state) {
		...
		for (RulePattern pp : thisRule.getProductpatterns()) {
			...
			for (MoleculePattern mp : pp.getMolepatterns()) {
				...
				for (ComponentPattern cp : mp.getComppatterns()) {
					if (cp.getStateindex() != -1) {
						// has certain state
						r_state.addProductCompNode(m_nodes.get(mp.getMoleIndex() + "."
						    + cp.getCompIndex() + "." + cp.getStateindex()));
					}

					r_comp.addProductCompNode(m_nodes.get(mp.getMoleIndex() + "."
					    + cp.getCompIndex()));
					r_state.addProductCompNode(m_nodes.get(mp.getMoleIndex() + "."
					    + cp.getCompIndex()));
				}
								...
								for (ComponentPattern pcp : pmp.getComppatterns()) {
									...
									if (rcompId == pcompId) {
										...
										if (rstateId != pstateId) {
											Node pstateNode = m_nodes.get(pmp.getMoleIndex() + "."
											    + pcp.getCompIndex() + "." + pcp.getStateindex());
											...
											if (r_state.getExpression().indexOf(">") != -1) {
												...
												ArrayList<VisualRule> tmplist = (ArrayList<VisualRule>) pstateNode
												    .get("rules");
											...
											if (r_state.getExpression().indexOf("<") != -1) {
												...
												// add VisualRule to state node
												ArrayList<VisualRule> tmplist = (ArrayList<VisualRule>) rstateNode
												    .get("rules");
												if (!tmplist.contains(r_state)) {
													tmplist.add(r_state);
												}
	...
	private void identifyMoleLevelReaction(Rule thisRule, VisualRule r_comp) {
		...
		// for two different rules, if they have same NodeSet, there will be only
		// one hub node
			...
			for (RulePattern rp : rpl) {
				for (MoleculePattern mp : rp.getMolepatterns()) {
					if (mp.getComppatterns().size() != 1) {
						...
						// component node
						compNode = m_nodes.get("" + mp.getMoleIndex() + "."
						    + cp.getCompIndex());
						if (cp.getStateindex() != -1) {
							// state node
							stateNode = m_nodes.get(mp.getMoleIndex() + "."
							    + cp.getCompIndex() + "." + cp.getStateindex());
							if (stateNode != null) {
		...
		if (hubNode == null) {
			...
			hubNode = m_componentGraph.addNode();
			// set its name
			hubNode.setString(VisualItem.LABEL, " ");
			// set its type
			hubNode.setString("type", "hub");
			// set VisualRule list
			ArrayList<VisualRule> tmplist = new ArrayList<VisualRule>();
			tmplist.add(r_comp);
			hubNode.set("rules", tmplist);
			m_hubNodes.put(moleSet, hubNode);
		}
		// exists, add current rule to the hub node
		else {
			ArrayList<VisualRule> tmplist = (ArrayList<VisualRule>) hubNode
			    .get("rules");
			tmplist.add(r_comp);
		}
		...
		addHubEdges(thisRule.getReactantpatterns(), hubNode, true);
		addHubEdges(thisRule.getProductpatterns(), hubNode, false);
		m_vis.getVisualItem(COMPONENT_GRAPH, hubNode).setVisible(true);
		// addMoleculeEdges(thisRule.getReactantpatterns(),
		// thisRule.getProductpatterns());
	}

	private void addHubEdges(ArrayList<RulePattern> patterns, Node hubNode,
	    boolean forward) {
	...
	private void addMoleculeEdges(ArrayList<RulePattern> reactantPatterns,
	    ArrayList<RulePattern> productPatterns) {
		for (RulePattern rp : reactantPatterns) {
			for (RulePattern pp : productPatterns) {
				addMoleculeEdge(rp, pp);
			}
		}
	}

	private void addMoleculeEdge(RulePattern reactantPattern,
	    RulePattern productPattern) {
		for (MoleculePattern rmp : reactantPattern.getMolepatterns()) {
			for (MoleculePattern pmp : productPattern.getMolepatterns()) {
				addMoleculeEdge(rmp, pmp);
			}
		}
	}

	private void addMoleculeEdge(MoleculePattern reactantMolePattern,
	    MoleculePattern productMolePattern) {
		// molecule level, no components or more than one components
		// if (pmp.getComppatterns().size() != 1) {
		boolean reactantSingleComponent, productSingleComponent;
		reactantSingleComponent = productSingleComponent = false;
		Node reactantNode, productNode;
		ComponentPattern rcp = null;
		ComponentPattern pcp = null;
		if (reactantMolePattern.getComppatterns().size() == 1) {
			reactantNode = m_nodes.get("" + reactantMolePattern.getMoleIndex() + "."
			    + reactantMolePattern.getComppatterns().get(0).getCompIndex());
			rcp = reactantMolePattern.getComppatterns().get(0);
			reactantSingleComponent = true;
		} else {
			reactantNode = m_nodes.get("" + reactantMolePattern.getMoleIndex());
		}

		if (productMolePattern.getComppatterns().size() == 1) {
			productNode = m_nodes.get("" + productMolePattern.getMoleIndex() + "."
			    + productMolePattern.getComppatterns().get(0).getCompIndex());
			pcp = productMolePattern.getComppatterns().get(0);
			productSingleComponent = true;
		} else {
			productNode = m_nodes.get("" + productMolePattern.getMoleIndex());
		}

		if (reactantNode == null || productNode == null) {
			return;
		}

		// set visible
		VisualItem reactantItem = m_vis
		    .getVisualItem(COMPONENT_GRAPH, reactantNode);
		VisualItem productItem = m_vis.getVisualItem(COMPONENT_GRAPH, reactantNode);

		reactantItem.setVisible(true);
		...
		// mode += "reactant";
		// }
		// if (productSingleComponent) {
		// mode += "product";
		// }
		if (reactantSingleComponent || productSingleComponent) {
			e.set("displaymode", "component");
			Node reactantState = reactantNode, productState = productNode;

			if (rcp != null && rcp.getStateindex() != -1) {
				// has certain state
				reactantState = m_nodes.get(reactantMolePattern.getMoleIndex() + "."
				    + rcp.getCompIndex() + "." + rcp.getStateindex());
				reactantState.set("hasedge", true);
			}

			if (pcp != null && pcp.getStateindex() != -1) {
				// has certain state
				productState = m_nodes.get(productMolePattern.getMoleIndex() + "."
				    + pcp.getCompIndex() + "." + pcp.getStateindex());
			...
			m_vis.getVisualItem(COMPONENT_GRAPH + ".edges", state).setVisible(false);
			// if (compNode != null) {
			// // connect to component node
			// Edge e_comp = m_componentGraph.addEdge(hubNode, compNode);
			// e_comp.set("type", "hubMoleConnection");
			// e_comp.set("displaymode", "component");
			//
			// m_vis.getVisualItem(COMPONENT_GRAPH, e_comp).setVisible(false);
			// // connect to state node
			// if (hasState && stateNode != null) {
			// Edge e_state = m_componentGraph.addEdge(hubNode, stateNode);
			// e_state.set("type", "hubMoleConnection");
			...
			e.set("displaymode", "both");
		}
		m_vis.getVisualItem(COMPONENT_GRAPH, e).setVisible(true);
	}

	private void initCompartmentAggregates(AggregateTable at_compartment,
	    ArrayList<AggregateItem> aggList) {

		CompartmentTable cmptTable = m_model.getCompartments();
		ArrayList<Compartment> cmptList = (ArrayList<Compartment>) cmptTable
		    .getCompartmentsList(true);
	 ...
	 * @param cmptStr
	 */
	private void addItemToCompartmentAgg(ArrayList<AggregateItem> aggList,
	    Node node, String cmptStr) {
		// Get the a table of information about the compartments in the model.
		CompartmentTable cmptTable = m_model.getCompartments();

		// Get a list of all of the compartments.
		ArrayList<Compartment> cmptList = (ArrayList<Compartment>) cmptTable
		    .getCompartmentsList(false);
		...
		for (int i = cmptList.size() - 1; i >= 0; i--) {
			...
			// If the passed in compartment name is the same as the current name, or
			// if
