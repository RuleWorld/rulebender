...
public class ChartComposite extends Composite implements ISelectionProvider {
	private final SashForm split;

	private CustomizedChartComposite curChartPanel; // chart panel for CDAT, GDAT,
	                                                // SCAN files

	private CTabFolder textFolder, outlineFolder;
	// private FileNode curFileNode;
	private final DATFileData m_datFileData;
	...
	public ChartComposite(Composite composite, IEditorInput editorInput,
				...
				if (selected == true) {
					...
					for (Object element : elements) {
						...
						if (node.getNodeType().equalsIgnoreCase("SpeciesNode")) {
							m_datFileData.addCheckedSpecies((SpeciesNode) element);
						}
					}
					...
					for (Object element : elements) {
						TreeNode node = (TreeNode) element;
						if (node.getNodeType().equalsIgnoreCase("SpeciesNode")) {
							m_datFileData.removeCheckedSpecies((SpeciesNode) element);
						}
					}
		...
		element_tv_cdat.addCheckStateListener(new ICheckStateListener() {
			...
			public void checkStateChanged(CheckStateChangedEvent event) {
				...
				if (labelName != null) {
					...
					element_tv_cdat.setSelection(
					    new StructuredSelection(findSpeciesNode(element_tv_cdat,
					        labelName)), true);
				}
			}
		});
		...
		element_tv_cdat
		    .addSelectionChangedListener(new ISelectionChangedListener() {
			    ...
			    public void selectionChanged(SelectionChangedEvent event) {
				    ...
				    String key = m_datFileData.getSpeciesID(labelName.split(" ")[0]); // index
	...
	private void createGDATOutline() {
				...
				if (selected == true) {
					...
					for (Object element : elements) {
						...
						if (node.getNodeType().equalsIgnoreCase("ObservableNode")) {
							m_datFileData.addCheckedObservable((ObservableNode) element);
						}
					}
					// set allChecked be true
					m_datFileData.setAllChecked(true);
					// set some nodes be checked
					element_tv_gdat.setCheckedElements(m_datFileData
					...
					for (Object element : elements) {
						TreeNode node = (TreeNode) element;
						if (node.getNodeType().equalsIgnoreCase("ObservableNode")) {
							m_datFileData.removeCheckedObservable((ObservableNode) element);
						}
					}
					// set allChecked be false
					m_datFileData.setAllChecked(false);
		...
		element_tv_gdat.addTreeListener(new ITreeViewerListener() {
			...
			public void treeExpanded(TreeExpansionEvent event) {
				TreeNode node = (TreeNode) event.getElement();
				if (node.getNodeType().equalsIgnoreCase("ObservableNode")
				    || node.getNodeType().equalsIgnoreCase("SpeciesFolderNode")) {
					Object[] children = node.getChildrenArray();
					for (Object element : children) {
						element_tv_gdat.setGrayChecked(element, true);
					}
				}
		...
		element_tv_gdat.addCheckStateListener(new ICheckStateListener() {
			...
			public void checkStateChanged(CheckStateChangedEvent event) {
				...
				if (event.getChecked() == true) {
					...
					} else if (node.getNodeType().equalsIgnoreCase("ObservableNode")) {
						// ObservableNode
						m_datFileData.addCheckedObservable((ObservableNode) event
						    .getElement());
						// setStatus("Checked:" + node.getName());
					} else if (node.getNodeType().equalsIgnoreCase("SpeciesFolderNode")) {
						// SpeciesFolderNode
						element_tv_gdat.setGrayChecked(node, true);
					}
				...
				if (labelName != null) {
					DATChart.markSelectedLine(curChartPanel.getChart(), labelName);

					// set selection
					element_tv_gdat.setSelection(new StructuredSelection(
					    findObservableNode(element_tv_gdat, labelName)), true);
				}
			}
		});
		    ...
		    .addSelectionChangedListener(new ISelectionChangedListener() {
			    ...
			    public void selectionChanged(SelectionChangedEvent event) {
				    ...
				    if (node.getNodeType().equalsIgnoreCase("ObservableNode")) {
					    ...
					    if (!labelName.equals(m_datFileData.getSelectedObservableName())) {
						    // store the name of the selected species
					    ...
					    String str = "species\t";
					    if (node.getNodeType().equalsIgnoreCase("SpeciesFolderNode")) {
						    str = "pattern\t-1 ";
					    }
	...
	/*
	 * private void createCompareOutline() { outlineCmp_compare = new
	 * Composite(outlineFolder, SWT.NONE); outlineCmp_compare.setLayout(new
	 * GridLayout(2, false));
	 * 
	 ...
	 * filePath.setText(""); GridData gridData = new
	 * GridData(GridData.FILL_HORIZONTAL); gridData.horizontalSpan = 2;
	 * filePath.setLayoutData(gridData);
	 * 
	 * // element tree viewer final CheckboxTreeViewer cur_compare = new
	 * CheckboxTreeViewer( outlineCmp_compare); gridData = new
	 * GridData(GridData.FILL_BOTH); gridData.horizontalSpan = 2;
	 ...
	 * 
	 * // nothing in the tree at the beginning cur_compare.setInput(null);
	 * 
	 * // select all button final Button selectAllBnt = new
	 * Button(outlineCmp_compare, SWT.CHECK);
	 * selectAllBnt.setText("Check/Uncheck All"); gridData = new GridData();
	 * 
	 * selectAllBnt.setLayoutData(gridData); gridData.horizontalSpan = 2;
	 * chartTypeList.add("line");
	 * 
	 * // chart type new Label(outlineCmp_compare, SWT.NONE).setText("Chart");
	 * 
	 * final Combo chartTypeBnt = new Combo(outlineCmp_compare, SWT.DROP_DOWN |
	 * SWT.READ_ONLY); chartTypeBnt.add("line"); chartTypeBnt.add("point");
	 * chartTypeBnt.add("line & point");
	 * chartTypeBnt.setText(chartTypeList.get(i)); chartTypeBnt.setData("index",
	 * i);
	 * 
	 * // set index for cur_compare cur_compare.setData("index", i);
	 * 
	 * element_tv_compare.add(cur_compare);
	 * 
	 * // add listener chartTypeBnt.addSelectionListener(new SelectionAdapter() {
	 * public void widgetSelected(SelectionEvent e) { chartTypeList.set((Integer)
	 * chartTypeBnt.getData("index"), chartTypeBnt.getText());
	 * 
	 * DATComparisonData data = (DATComparisonData) curFileNode .getFileData(); //
	 * plot curChartPanel.setChart(DATComparisonChart.plotChart(data,
	 * chartTypeList));
	 * curChartPanel.setChart(DATComparisonChart.plotChart(curChartPanel
	 * .getChart(), xAxisType, yAxisType)); } });
	 * 
	 * // add listener for selectAllBnt selectAllBnt.addSelectionListener(new
	 * SelectionAdapter() {
	 * 
	 * public void widgetSelected(SelectionEvent arg0) { boolean selected =
	 * selectAllBnt.getSelection(); if (selected == true) { // select all
	 * 
	 * cur_compare.setAllChecked(true);
	 * 
	 * Object[] elements = cur_compare.getCheckedElements();
	 * 
	 * // Compare DATComparisonData data = (DATComparisonData) curFileNode
	 * .getFileData();
	 * 
	 * // check all the ObservableNode for (int i = 0; i < elements.length; i++) {
	 * TreeNode node = (TreeNode) elements[i]; if
	 * (node.getNodeType().equalsIgnoreCase( "ObservableNode")) {
	 * data.addCheckedObservable( (Integer) cur_compare.getData("index"),
	 * (ObservableNode) elements[i]); } if (node.getNodeType().equalsIgnoreCase(
	 * "SpeciesNode")) { data.addCheckedSpecies( (Integer)
	 * cur_compare.getData("index"), (SpeciesNode) elements[i]); } } // set
	 * allChecked be true data.setAllChecked( (Integer)
	 * cur_compare.getData("index"), true); // set some nodes be checked
	 * cur_compare.setCheckedElements(data .getCheckedElements((Integer)
	 * cur_compare .getData("index"))); // plot the chart
	 * curChartPanel.setChart(DATComparisonChart.plotChart( data, chartTypeList));
	 ...
	 * .getFileData();
	 * 
	 * // uncheck all the ObservableNode for (int i = 0; i < elements.length; i++)
	 * { TreeNode node = (TreeNode) elements[i]; if
	 * (node.getNodeType().equalsIgnoreCase( "ObservableNode")) {
	 * data.removeCheckedObservable( (Integer) cur_compare.getData("index"),
	 * (ObservableNode) elements[i]); } if (node.getNodeType().equalsIgnoreCase(
	 * "SpeciesNode")) { data.removeCheckedSpecies( (Integer)
	 * cur_compare.getData("index"), (SpeciesNode) elements[i]); } }
	 * 
	 * // set allChecked be false data.setAllChecked( (Integer)
	 * cur_compare.getData("index"), false); // set some nodes be checked
	 * cur_compare.setCheckedElements(data .getCheckedElements((Integer)
	 * cur_compare .getData("index"))); // plot the chart
	 * curChartPanel.setChart(DATComparisonChart.plotChart( data, chartTypeList));
	 * curChartPanel.setChart(DATComparisonChart.plotChart(
	 * curChartPanel.getChart(), xAxisType, yAxisType));
	 * 
	 * cur_compare.setAllChecked(false); cur_compare.setSelection(null); } }
	 * 
	 * });
	 * 
	 * // set selection be empty when user click on empty area
	 ...
	 * public void mouseDown(MouseEvent event) {
	 * 
	 * Point p = Display.getCurrent().getCursorLocation(); p =
	 * cur_compare.getTree().toControl(p); // get the item corresponding the click
	 * point TreeItem item = cur_compare.getTree().getItem(p); if (item == null) {
	 * int cur_index = (Integer) cur_compare.getData("index");
	 * 
	 * cur_compare.setSelection(null);
	 * DATComparisonChart.resetChart(curChartPanel.getChart(), cur_index); //
	 * reset chart } }
	 * 
	 * public void mouseUp(MouseEvent arg0) {
	 * 
	 ...
	 * }
	 * 
	 * public void treeExpanded(TreeExpansionEvent event) { TreeNode node =
	 * (TreeNode) event.getElement(); if
	 * (node.getNodeType().equalsIgnoreCase("ObservableNode") ||
	 * node.getNodeType().equalsIgnoreCase( "SpeciesFolderNode")) { Object[]
	 * children = node.getChildrenArray(); for (int i = 0; i < children.length;
	 * i++) { cur_compare.setGrayChecked(children[i], true); } } }
	 * 
	 * });
	 * 
	 ...
	 * public void checkStateChanged(CheckStateChangedEvent event) {
	 * 
	 * //index for current compare tree viewer int cur_index = (Integer)
	 * cur_compare.getData("index"); //get node TreeNode node = (TreeNode)
	 * event.getElement(); //get data DATComparisonData data = (DATComparisonData)
	 * curFileNode .getFileData();
	 * 
	 * if (event.getChecked() == true) { if
	 * (node.getNodeType().equalsIgnoreCase("SpeciesNode")) { // SpeciesNode if
	 * (((TreeNode) node.getParent()).getParent() != null) { // GDAT
	 * cur_compare.setGrayChecked(node, true);
	 * 
	 * } else { // CDAT data.addCheckedSpecies(cur_index, (SpeciesNode)
	 * event.getElement()); //setStatus("Checked:" + node.getName()); }
	 * 
	 * } else if (node.getNodeType().equalsIgnoreCase( "ObservableNode")) { //
	 * ObservableNode data.addCheckedObservable(cur_index, (ObservableNode)
	 * event.getElement()); //setStatus("Checked:" + node.getName()); } else if
	 * (node.getNodeType().equalsIgnoreCase( "SpeciesFolderNode")) { //
	 * SpeciesFolderNode cur_compare.setGrayChecked(node, true); }
	 * 
	 * } // not check else if (event.getChecked() == false) { if
	 * (node.getNodeType().equalsIgnoreCase("SpeciesNode")) { // SpeciesNode if
	 * (((TreeNode) node.getParent()).getParent() != null) { // GDAT
	 * cur_compare.setGrayChecked(node, true);
	 * 
	 * } else { // CDAT data.removeCheckedSpecies(cur_index, (SpeciesNode)
	 * event.getElement()); // setStatus("Unchecked:" + node.getName()); }
	 * 
	 * } else if (node.getNodeType().equalsIgnoreCase( "ObservableNode")) { //
	 * ObservableNode data.removeCheckedObservable(cur_index, (ObservableNode)
	 * event.getElement());
	 * 
	 * //setStatus("Unchecked:" + node.getName()); } else if
	 * (node.getNodeType().equalsIgnoreCase( "SpeciesFolderNode")) { //
	 * SpeciesFolderNode cur_compare.setGrayChecked(node, true);
	 * 
	 * } } curChartPanel.setChart(DATComparisonChart .plotChart(data,
	 * chartTypeList)); curChartPanel.setChart(DATComparisonChart.plotChart(
	 * curChartPanel.getChart(), xAxisType, yAxisType));
	 * 
	 * // mark line if (data.getSelectedSpeciesName(cur_index) != null) { String
	 * labelName = data.getSelectedSpeciesName(cur_index); String key =
	 * labelName.split(" ")[0]; if (key != null) {
	 * DATComparisonChart.markSelectedLine( curChartPanel.getChart(), cur_index,
	 * key);
	 * 
	 * cur_compare.setSelection(new StructuredSelection(
	 * findSpeciesNode(cur_compare, labelName)), true);
	 * 
	 * } } } });
	 * 
	 * // add selection changed listener
	 ...
	 * TreeNode node = (TreeNode) selection.getFirstElement();
	 * 
	 * // empty selection if (selection.isEmpty()) { return; }
	 * 
	 * String labelName = node.getName();
	 * 
	 * int cur_index = (Integer) cur_compare.getData("index");
	 * 
	 * if (node.getNodeType().equalsIgnoreCase("ObservableNode")) { // if selected
	 * observable changed if
	 * (!labelName.equals(data.getSelectedObservableName(cur_index))) { // store
	 * the name of the selected species data.setSelectedObservableName(cur_index,
	 * labelName); }
	 * 
	 * // mark selected line
	 * DATComparisonChart.markSelectedLine(curChartPanel.getChart(), cur_index,
	 * labelName); //setStatus("Selected: " // + data.getFileName(cur_index) + " "
	 * // + labelName); }
	 * 
	 * else { // SpeciesFolderNode or SpeciesNode String str = "species\t"; if
	 * (node.getNodeType().equalsIgnoreCase( "SpeciesFolderNode")) { str =
	 * "pattern\t-1 "; }
	 * 
	 * if (labelName.equals(data .getSelectedSpeciesName(cur_index))) { //
	 * selected species not change return; }
	 * 
	 * // store the name of the selected species
	 * data.setSelectedSpeciesName(cur_index, labelName);
	 * 
	 * if (data.getFileName(cur_index).endsWith( ".cdat")) { // CDAT
	 * 
	 * String key = labelName.split(" ")[0]; // index
	 * 
	 * // mark selected line DATComparisonChart.markSelectedLine(
	 * curChartPanel.getChart(), cur_index, key); }
	 * 
	 * if (labelName.contains(" ")) { // show the species graph in a JFrame
	 * ArrayList<String> itemList = new ArrayList<String>(); String item = str +
	 * labelName; itemList.add(item);
	 * 
	 * showGraph(itemList); ////setStatus("Selected: " // +
	 * data.getFileName(cur_index) + " " // + labelName); } } }
	 * 
	 * }); }
	 * 
	 * }
	...
	private void createChartItem(IEditorInput editorInput) {
		...
		final Combo xAxisBnt = new Combo(chartCmp, SWT.DROP_DOWN | SWT.READ_ONLY);
		xAxisBnt.add("linear");
		xAxisBnt.add("log");
		...
		final Combo yAxisBnt = new Combo(chartCmp, SWT.DROP_DOWN | SWT.READ_ONLY);
		yAxisBnt.add("linear");
		yAxisBnt.add("log");
		...
		// DATFileData datData = new DATFileData(new
		// File(iFile.getLocation().toOSString()));

		// create the chart using JFreeChart
		JFreeChart chart = DATChart.plotChart(m_datFileData, xAxisType, yAxisType,
		    chartType);
		...
		/*
		 * This code selects all of the observables from the start. This incurs high
		 * overhead and is not always necessary, so if the user wants to load all of
		 * the data points they can easily manually select it.
		 */
		...
		/*
		 * // This was doing nothing, but might have something to do with
		 * comparisons. // Most likely it can be deleted, if comparisons are
		 * possible, then this // definitely can be deleted.
		 * 
		 * chartItem.addDisposeListener(new DisposeListener() { public void
		 * widgetDisposed(DisposeEvent event) { CTabItem item = (CTabItem)
		 * event.getSource();
		 * 
		 * String filePath = (String) item.getData("path");
		 * 
		 * // remove the fileNode from curFileList removeFileNode(filePath);
		 * 
		 * if (curFileList.size() == 0) { curFileNode = null; if (elementOutline !=
		 * null&& elementOutline.isDisposed() == false) {
	...
	private void updateOutline() {
		/*
		 * if (curFileNode == null) { elementOutline.setControl(null); return; } //
		 */

		/*
		 * if (curFileNode.getName().contains("Compare")) { // Compare
		 * elementOutline.setControl(outlineCmp_compare);
		 * 
		 * DATComparisonData data = (DATComparisonData) curFileNode .getFileData();
		 * for (int i = 0; i < element_tv_compare.size(); i++) { CheckboxTreeViewer
		 * cur_compare = element_tv_compare.get(i);
		 * 
		 * // set input cur_compare.setInput(data.getFolderNode(i));
		 * 
		 * // set selection on element_tv if (data.getFileName(i).endsWith(".cdat"))
		 * { String selectedSpeciesName = data.getSelectedSpeciesName(i); if
		 * (selectedSpeciesName != null) { cur_compare.setSelection(new
		 * StructuredSelection( findSpeciesNode(cur_compare, selectedSpeciesName)),
		 * true); } else { cur_compare.setSelection(null); } } else { String
		 * selectedObservableName = data .getSelectedObservableName(i); if
		 * (selectedObservableName != null) { cur_compare.setSelection(new
		 * StructuredSelection( findObservableNode(cur_compare,
		 * selectedObservableName)), true); } else { cur_compare.setSelection(null);
		 * } }
		 * 
		 * // set some nodes be checked
		 * cur_compare.setCheckedElements(data.getCheckedElements(i)); } return; }
		 * 
		 * //
		 */
...

		{
			...
			if (m_datFileData.getExpandedElements() != null) {
				element_tv_gdat
				    .setExpandedElements(m_datFileData.getExpandedElements());
			}

			// set grayed nodes
			Object[] toSetGray = m_datFileData.getGrayedElements();
			if (toSetGray != null) {
				for (Object element : toSetGray) {
					element_tv_gdat.setGrayChecked(element, true);
				}
			}
			...
			((Combo) outlineCmp_gdat.getChildren()[index]).setText(m_datFileData
			    .getChartType());
		}
	 ...
	 * 
	 * @param file
	 *          The file for which we need an IEditorInput.
	...
	public boolean isComparable() {
		/*
		 * if(curFileNode == null) { return false; } //
		 */

		String curFileName = m_datFileData.getFileName();

		if (curFileName.endsWith(".cdat") || curFileName.endsWith(".gdat")
	...
	/*
	 * TODO Figure out how to do comparisons when the charts are in multipage
	 * editors. public void compareWith(FileNode node) {
	 * 
	 * String curFileName = m_datFileData.getFileName(); //curFileNode.getName();
	 * 
	 * // get file type String type1 = curFileName.substring(
	 * curFileName.indexOf(".") + 1); String type2 = node.getName().substring(
	 * node.getName().indexOf(".") + 1);
	 * 
	 * // get DATFileData DATFileData data1 = (DATFileData)
	 * curFileNode.getFileData(); DATFileData data2 = (DATFileData) ((FileNode)
	 * node) .getFileData();
	 * 
	 * // create a compare data object DATComparisonData compData = new
	 * DATComparisonData(type1, data1, type2, data2);
	 * 
	 * if (compData.isComparable() == false) { MessageBox msgBox = new
	 * MessageBox(this.getShell(), SWT.ICON_ERROR | SWT.OK);
	 * msgBox.setText("Error"); msgBox.setMessage("Different X axis");
	 * msgBox.open(); return; }
	 * 
	 * // create a new compare tab item createComparisonItem(curFileNode,
	 * (FileNode) node, compData); }
	 */
	 ...
	 * DATComparisonData data) { curFileNode = new FileNode(null, null);
	 * curFileNode.setfData(data);
	 * 
	 * // tab item CTabItem compItem = new CTabItem(textFolder, SWT.CLOSE); String
	 * fileName1 = fNode1.getName().substring( fNode1.getName().indexOf(":") + 2);
	 * String fileName2 = fNode2.getName().substring(
	 * fNode2.getName().indexOf(":") + 2); String fileName = "Compare " +
	 * fileName1 + " with " + fileName2; compItem.setText(fileName);
	 * curFileNode.setName(fileName); data.setFileName(fileName);
	 * 
	 * // set tool tip String filePath1 = fNode1.getPath(); String filePath2 =
	 * fNode2.getPath(); String filePath = "Compare " + filePath1 + " with " +
	 * filePath2; compItem.setToolTipText(filePath);
	 ...
	 * chartCmp.setLayout(new GridLayout(6, false));
	 * 
	 * // control options xAxisType = "linear"; yAxisType = "linear";
	 * 
	 * // x axis scale new Label(chartCmp, SWT.NONE).setText("\tX Axis");
	 * 
	 * final Combo xAxisBnt = new Combo(chartCmp, SWT.DROP_DOWN | SWT.READ_ONLY);
	 * xAxisBnt.add("linear"); xAxisBnt.add("log"); xAxisBnt.setText(xAxisType);
	 * 
	 * // y axis scale new Label(chartCmp, SWT.NONE).setText("\tY Axis");
	 * 
	 * final Combo yAxisBnt = new Combo(chartCmp, SWT.DROP_DOWN | SWT.READ_ONLY);
	 * yAxisBnt.add("linear"); yAxisBnt.add("log"); yAxisBnt.setText(yAxisType);
	 * 
	 * xAxisBnt.addSelectionListener(new SelectionAdapter() { public void
	 * widgetSelected(SelectionEvent e) { xAxisType = xAxisBnt.getText();
	 * 
	 * DATComparisonData data = (DATComparisonData) curFileNode .getFileData(); //
	 * plot curChartPanel.setChart(DATComparisonChart.plotChart(data,
	 * chartTypeList)); curChartPanel.setChart(DATComparisonChart.plotChart(
	 * curChartPanel.getChart(), xAxisType, yAxisType)); updateOutline(); } });
	 * yAxisBnt.addSelectionListener(new SelectionAdapter() { public void
	 ...
	 * curChartPanel.getChart(), xAxisType, yAxisType)); updateOutline(); } });
	 * 
	 * // create the chart using JFreeChart JFreeChart chart =
	 * DATComparisonChart.plotChart(data, chartTypeList); CustomizedChartComposite
	 * chartPanel = new CustomizedChartComposite(chartCmp, SWT.NONE, chart, true);
	 * GridData griddata = new GridData(GridData.FILL_BOTH);
	 * griddata.horizontalSpan = 6; chartPanel.setLayoutData(griddata);
	 * curChartPanel = chartPanel;
	 * 
	 * // set control compItem.setControl(chartCmp);
	 * textFolder.setSelection(compItem);
	 * 
	 * updateOutline();
	 * 
	 * for (int i = 0; i < data.getDATDataCount(); i++) { // update the
	 * "check/uncheck all" button data.setAllChecked(i, true); ((Button)
	 * outlineCmp_compare.getChildren()[2 + 5 * i]) .setSelection(true);
	 * 
	 * // update checkbox in the tree
	 ...
	 * 
	 * for (int j = 0; j < elements.length; j++) { TreeNode node = (TreeNode)
	 * elements[j]; if (node.getNodeType().equalsIgnoreCase("SpeciesNode")) { //
	 * SpeciesNode data.addCheckedSpecies(i, (SpeciesNode) elements[j]); } else if
	 * (node.getNodeType() .equalsIgnoreCase("ObservableNode")) { //
	 * ObservableNode data.addCheckedObservable(i, (ObservableNode) elements[j]);
	 * } else { } } }
	 * 
	 * // set file path for each tree ((Text)
	 * outlineCmp_compare.getChildren()[0]).setText(fileName1); ((Text)
	 * outlineCmp_compare.getChildren()[0]).setToolTipText(filePath1); ((Text)
	 * outlineCmp_compare.getChildren()[5]).setText(fileName2); ((Text)
	 * outlineCmp_compare.getChildren()[5]).setToolTipText(filePath2);
	 * 
	 * // plot chart curChartPanel.setChart(DATComparisonChart .plotChart(data,
	 * chartTypeList));
	 * curChartPanel.setChart(DATComparisonChart.plotChart(curChartPanel
	 * .getChart(), xAxisType, yAxisType));
	 * 
	 * // add close listener for tabItem compItem.addDisposeListener(new
	 * DisposeListener() {
	 * 
	 * public void widgetDisposed(DisposeEvent event) { CTabItem item = (CTabItem)
	 * event.getSource();
	 * 
	 * String filePath = (String) item.getData("path");
	 * 
	 * // remove the fileNode from curFileList // removeFileNode(filePath);
	 * 
	 * //if (curFileList.size() == 0) //{ //curFileNode = null;
	 * 
	 * //} } }); }
	 */
