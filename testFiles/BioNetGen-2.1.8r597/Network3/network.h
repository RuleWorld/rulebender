...
#define ELEMENTARY 0
#define SATURATION 1
#define MICHAELIS_MENTEN  2
#define HILL 3
#define FUNCTION 4 

enum {ES, POISSON, LANGEVIN, DETERMINISTIC}; 
enum {VERY_SLOW, SLOW, MEDIUM, FAST};
enum {PRELEAP_PROPENSITIES, PRELEAP_SPECIES, POSTLEAP};

/* Basic network structures */

/* Structure used to for species and rate constants.  For species, val is the concentration in a consistent set of units
 * (up to user).*/
typedef struct ELT{
	char*			name;
    double      	val;
...
struct NETWORK{
	...
	// testing variables: don't need for simulation
	//  vector<int> rxn_count;
	//  ofstream rxns_out;
